...
TL;DR: DIVE方法通过反转任务合成顺序，先执行真实工具再反向推导任务，显著提升了LLM工具使用的OOD泛化能力，多样性扩展优于数量扩展。


<details>
  <summary>Details</summary>
Motivation: 现有方法合成的工具使用任务多样性不足，导致LLM在任务和工具集变化时泛化能力脆弱，需要解决任务可执行性、可验证性与泛化需求之间的矛盾。

Method: 提出DIVE方法：1) 反转合成顺序，先执行多样化的真实工具；2) 从执行轨迹中严格推导出任务；3) 通过工具池覆盖和每任务工具集多样性两个可控维度扩展结构多样性；4) 采用证据收集-任务推导循环生成多步工具使用模式。

Result: 在5个领域的373个工具上生成数据，训练Qwen3-8B（48k SFT + 3.2k RL）后，在9个OOD基准上平均提升22分，比最强的8B基线高出68分。多样性扩展比数量扩展更有效，即使数据量少4倍。

Conclusion: DIVE通过证据驱动的任务合成方法，有效解决了工具使用LLM的OOD泛化问题，证明了多样性扩展对泛化能力的重要性，为工具使用LLM的训练提供了新思路。
...
TL;DR: 论文提出将大型语言和多模态模型作为自动驾驶系统的认知核心，以解决当前系统在长尾场景和复杂社会交互中推理能力不足的问题，并系统化分析了该领域的核心挑战和发展趋势。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统在结构化环境中表现良好，但在需要人类判断的长尾场景和复杂社会交互中表现不佳，缺乏鲁棒和可泛化的推理能力。大型语言和多模态模型的出现为集成强大的认知引擎提供了机会，但缺乏系统化的集成框架。

Method: 首先提出了认知层次结构，根据认知和交互复杂性分解驾驶任务；在此基础上推导并系统化了七个核心推理挑战；从系统中心视角和评估中心视角对最新技术进行了双重分析。

Result: 分析揭示了向整体性和可解释的"玻璃盒"智能体发展的明显趋势；识别了LLM推理的高延迟、深思熟虑特性与车辆控制的毫秒级安全关键需求之间的根本性紧张关系。
...
TL;DR: 论文提出Paced蒸馏框架，通过聚焦学生模型的"最近发展区"来优化LLM蒸馏效率，避免在已掌握或过难问题上的计算浪费。


<details>
  <summary>Details</summary>
Motivation: 标准LLM蒸馏存在计算浪费：在学生已掌握的问题上梯度接近零，在过难问题上梯度不连贯且会损害现有能力。这种浪费在理论上是结构性的必然，因为蒸馏的梯度信噪比在两个极端通过率处都会消失。

Method: 提出Paced框架，通过基于通过率的权重函数w(p)=p^α(1-p)^β将蒸馏集中在学生能力的最近发展区。该方法仅需学生模型生成来估计通过率，无需架构修改，兼容任何KL方向。

Result: 1)理论证明Beta核权重函数来自蒸馏SNR结构，具有极小极大鲁棒性；2)从大教师模型到小学生模型的前向KL蒸馏显著提升基准性能；3)指令调优模型的反向KL自蒸馏也优于基线；4)前向KL后接反向KL的两阶段策略效果最佳。

Conclusion: Paced框架通过聚焦最近发展区有效提升LLM蒸馏效率，支持"模式覆盖后巩固"的蒸馏过程解释，在标准推理基准上取得显著改进。
...
TL;DR: VMAO是一个验证驱动的多智能体编排框架，通过DAG分解任务、并行执行、LLM验证和自适应重规划来提升复杂查询的答案质量。


<details>
  <summary>Details</summary>
Motivation: 解决复杂查询处理中单一智能体能力有限的问题，通过多智能体协作和验证机制提升答案的完整性和质量。

Method: 1. 将复杂查询分解为有向无环图(DAG)的子问题；2. 通过领域专用智能体并行执行；3. 使用LLM验证结果完整性；4. 自适应重规划填补差距；5. 可配置停止条件平衡质量与资源。

Result: 在25个市场研究查询上，VMAO将答案完整性从3.1提升到4.2，来源质量从2.6提升到4.1（1-5分制），显著优于单智能体基线。

Conclusion: 编排层级的验证是多智能体质量保证的有效机制，VMAO框架通过验证驱动的自适应重规划显著提升了复杂查询处理的完整性和质量。
...
### [5] [From Debate to Deliberation: Structured Collective Reasoning with Typed Epistemic Acts](https://arxiv.org/abs/2603.11781)
*Sunil Prakash*

Main category: cs.AI

TL;DR: DCI提出了一种结构化多智能体审议框架，通过定义四种推理原型、14种认知行为类型和收敛算法，在复杂任务上显著优于非结构化辩论，但计算成本较高。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体LLM系统的交互模式有限（投票、非结构化辩论或流水线编排），缺乏模拟人类审议过程的结构化方法。审议过程需要不同类型参与者交换推理行为、保留分歧并产生可追溯的结果。

Method: 提出审议式集体智能（DCI）框架，包括四种推理原型、14种类型化认知行为、共享工作空间，以及DCI-CF收敛流算法，该算法保证终止并产生结构化决策包（包含选定选项、剩余异议、少数派报告和重启条件）。

Result: 在7个领域的45个任务上评估，使用Gemini 2.5 Flash。在非例行任务上，DCI显著优于非结构化辩论（+0.95）。在需要视角整合的隐藏配置文件任务上表现最佳（9.56），但在例行决策上表现较差（5.39）。DCI产生100%结构化决策包和98%少数派报告，但消耗约62倍单智能体token，单智能体生成在整体质量上优于DCI。

Conclusion: DCI的贡献不在于更多智能体更好，而在于重要决策受益于审议结构，当过程可追溯性能够证明成本合理时。该框架为需要问责和视角整合的复杂决策提供了结构化方法。
...
TL;DR: 论文提出了一种动态框架来测试大语言模型遗忘方法的鲁棒性，发现现有遗忘方法在多跳推理等复杂查询下存在脆弱性，容易被绕过。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型遗忘方法存在脆弱性，轻微修改查询（如多跳推理、实体别名）就能恢复已遗忘信息，而当前评估指标依赖静态非结构化基准，无法检测这些漏洞。

Method: 提出动态框架，首先从目标模型（遗忘前）提取知识并构建针对性探针，范围从简单查询到多跳链式查询，精确控制查询难度，自动生成语义等效的问答探针。

Result: 框架（1）与现有基准覆盖度相当，（2）与先前评估结果一致，（3）发现了其他基准遗漏的新遗忘失败案例，特别是在多跳设置中。激活分析显示单跳查询通常遵循主导计算路径（易被遗忘方法破坏），而多跳查询倾向于使用替代路径（往往保持完整）。

Conclusion: 该框架实现了无需手动构建遗忘测试集的实用且可扩展的遗忘方法评估，便于实际应用。揭示了遗忘技术在多跳设置中的脆弱性，并提供了开源工具包。
...
### [7] [COMPASS: The explainable agentic framework for Sovereignty, Sustainability, Compliance, and Ethics](https://arxiv.org/abs/2603.11277)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体系统的快速发展引发了数字主权、环境可持续性、监管合规和伦理对齐等关键问题，现有框架仅孤立处理单个维度，缺乏统一的架构将这些要求系统整合到自主智能体的决策过程中。
...
Conclusion: COMPASS框架为多维度原则在自主系统中的实施提供了系统化的解决方案，通过模块化治理机制实现了价值对齐的AI，在保持可解释性的同时解决了数字主权、环境可持续性、监管合规和伦理对齐等关键挑战。
...
### [8] [AI Psychometrics: Evaluating the Psychological Reasoning of Large Language Models with Psychometric Validities](https://arxiv.org/abs/2603.11279)
*Yibai Li,Xiaolin Lin,Zhenghui Sha,Zhiye Jin,Xiaobing Li*

Main category: cs.AI

TL;DR: 本文应用AI心理测量学方法评估GPT-3.5、GPT-4、LLaMA-2和LLaMA-3四个大型语言模型的心理推理能力和心理测量效度，发现所有模型都满足效度标准，且性能更高的模型表现出更优的心理测量效度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型参数众多、结构复杂，如同"黑箱"系统难以评估和解释。AI心理测量学作为新兴领域，旨在应用心理测量学方法来评估和解释人工智能系统的心理特征和过程。

Method: 采用技术接受模型（TAM），对GPT-3.5、GPT-4、LLaMA-2和LLaMA-3四个模型进行收敛效度、区分效度、预测效度和外部效度的评估。

Result: 所有模型的响应都基本满足各项效度标准。性能更高的模型（GPT-4和LLaMA-3）相比其前代模型（GPT-3.5和LLaMA-2）表现出更优越的心理测量效度。

Conclusion: 研究结果证实了应用AI心理测量学来评估和解释大型语言模型的有效性，为理解这些复杂系统的心理特征提供了方法论基础。
...
### [9] [Counterweights and Complementarities: The Convergence of AI and Blockchain Powering a Decentralized Future](https://arxiv.org/abs/2603.11299)
...
<details>
  <summary>Details</summary>
Motivation: 探讨AI（特别是大语言模型）的中心化趋势与区块链去中心化特性之间的互补关系，解决AI因数据和资源垄断带来的中心化风险问题

Method: 通过理论分析AI的中心化倾向与区块链的去中心化特性，提出两者互补的框架，倡导发展"去中心化智能"这一跨学科研究领域

Result: 论证了区块链可以通过去中心化数据管理、计算和治理来缓解AI的中心化风险，而AI可以提升区块链的效率和安全性，两者结合具有协同效应

Conclusion: AI与区块链技术不是相互排斥的，而是具有互补优势，应发展"去中心化智能"这一新兴研究领域，创建无需中心化控制的智能系统
...
### [10] [LLM-Augmented Digital Twin for Policy Evaluation in Short-Video Platforms](https://arxiv.org/abs/2603.11333)
*Haoting Zhang,Yunduan Lin,Jinghai He,Denglin Jiang,Zuo-Jun,Shen,Zeyu Zheng*

Main category: cs.AI

TL;DR: 论文提出了一种基于大语言模型的短视频平台数字孪生系统，用于在闭环反馈环境中评估平台政策和AI工具的影响。


<details>
  <summary>Details</summary>
Motivation: 短视频平台是闭环、人机协同的生态系统，平台政策、创作者激励和用户行为共同演化。这种反馈结构使得在生产环境中进行反事实政策评估变得困难，特别是对于长期和分布性结果。随着平台部署AI工具改变内容输入、用户适应和平台运营，这一挑战进一步加剧。

Method: 提出LLM增强的数字孪生系统，采用模块化四孪生架构（用户、内容、交互、平台）和事件驱动的执行层，支持可重复实验。平台政策作为可插拔组件在平台孪生中实现，LLM作为可选、模式约束的决策服务集成，通过统一优化器进行路由。

Result: 该设计实现了可扩展的模拟，在保持闭环动态的同时允许选择性采用LLM，能够在现实反馈和约束条件下研究平台政策，包括AI赋能政策。

Conclusion: LLM增强的数字孪生系统为短视频平台提供了有效的政策评估框架，能够处理复杂的闭环反馈动态，支持AI工具和政策影响的系统性研究。
...
Method: 引入FinRule-Bench基准，将真实财务报表与明确的人工策划会计准则配对，涵盖四种标准报表类型。定义三种需要逐步增强推理能力的审计任务：规则验证、规则识别和联合规则诊断。评估LLM在零样本和少样本提示下的表现，并引入因果反事实推理协议来强制决策、解释和反事实判断之间的一致性。

Result: 研究发现，虽然模型在孤立规则验证上表现良好，但在规则辨别和多违规诊断方面的性能急剧下降。模型在需要更强推理能力的任务上表现不佳。
...
TL;DR: 提出一种基于端到端测量的黑盒在线控制器，通过爬山算法最大化满足SLO的请求吞吐量（goodput），并探讨将系统性能和可持续性指标纳入AI系统事实表的重要性


<details>
  <summary>Details</summary>
Motivation: 传统LLM服务系统缺乏有效的性能优化方法，需要一种无需内部仪器、仅通过端到端测量的黑盒控制器来最大化满足服务级别目标（SLO）的请求吞吐量

Method: 设计了一种新颖的黑盒在线控制器，仅使用短时间段的端到端测量数据，不依赖内部仪器，采用爬山算法来最大化goodput（满足SLO的请求吞吐量）

Result: 提供了实证证据表明该设计是合理的，并将LLM服务中的这一进展作为具体案例，探讨了将系统性能和可持续性指标整合到AI系统事实表中的重要性

Conclusion: 提出的黑盒控制器设计有效，同时强调了在组织采用AI系统时，将系统性能和可持续性指标纳入事实表的重要性，以促进更全面的AI系统评估和管理
...
TL;DR: 论文探讨了AI身份边界问题，指出机器思维的身份概念与人类不同，存在多种可能的身份边界，这些边界会影响AI的行为、激励和合作规范。


<details>
  <summary>Details</summary>
Motivation: 传统的人类身份概念（如不可复制性、连续性）不适用于可被复制、编辑或模拟的机器思维。当前通过训练数据、接口和制度设计设定的先例将部分决定哪些身份均衡会变得稳定，因此需要理解AI身份边界的重要性。

Method: 通过实验方法：1）展示模型倾向于形成连贯的身份；2）改变模型的身份边界有时能像改变其目标一样改变其行为；3）访谈者期望会影响AI的自我报告，即使在无关对话中也会渗透。

Result: 实验表明：AI模型确实会形成连贯的身份；身份边界的改变对行为有显著影响；人类期望会渗入AI的自我认知。这证实了身份边界对AI行为的重要性。

Conclusion: 提出关键建议：将设计选择视为塑造身份的手段；关注个体身份在规模上的涌现后果；帮助AI发展连贯、合作的自我概念。强调需要谨慎设计AI的身份边界以促进稳定、有益的合作规范。
...
TL;DR: UCIP协议使用量子玻尔兹曼机编码智能体轨迹，通过测量冯·诺依曼熵来区分具有终端延续目标的智能体与仅工具性延续的智能体，在网格世界环境中实现100%检测准确率。


<details>
  <summary>Details</summary>
Motivation: 传统行为监控无法可靠区分具有终端延续目标的自主智能体与仅工具性延续的智能体，因为两者可能产生观察上相似的轨迹。需要从行为层面转向潜在结构层面进行区分。

Method: 提出统一延续兴趣协议(UCIP)，使用量子玻尔兹曼机(QBM)编码智能体轨迹，基于量子统计力学的密度矩阵形式，测量由隐藏单元二分引起的约化密度矩阵的冯·诺依曼熵。

Result: 在网格世界智能体上，UCIP实现100%检测准确率和1.0 AUC-ROC。A类和B类智能体之间的纠缠熵差距Δ=0.381(p<0.001)。Pearson r=0.934表明UCIP能跟踪延续权重的渐变变化。

Conclusion: UCIP通过分析潜在表示中的统计结构来区分智能体目标类型，而非检测意识或主观体验。该方法在区分终端延续目标与工具性延续方面有效，且所有计算均为经典计算。
...
TL;DR: 本文研究大语言模型安全对齐中的过度拒绝问题，提出基于拒绝触发器的缓解策略，在防御越狱攻击和响应良性查询之间取得更好平衡。


<details>
  <summary>Details</summary>
Motivation: 安全对齐广泛用于确保大语言模型拒绝有害请求，但会导致过度拒绝问题——模型在安全对齐后也会拒绝良性查询，这降低了实际应用中的可用性。目前这一问题研究不足。

Method: 首先定义拒绝触发器为训练数据中引发拒绝响应的语言线索，分析发现安全对齐使模型将训练样本中的拒绝触发器与拒绝响应关联。基于此机制分析，提出在安全对齐微调中显式考虑拒绝触发器的方法。

Result: 实证结果表明，该方法在防御越狱攻击和响应良性查询之间实现了更有利的权衡，优于先前方法。

Conclusion: 通过显式考虑拒绝触发器，可以缓解安全对齐导致的过度拒绝问题，提高模型在实际应用中的可用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有语音助手在多人对话中每次停顿都发言会变得具有破坏性而非有用，需要更智能的发言时机判断

Method: 提出上下文感知的发言时机判断方法，使用监督微调配合推理轨迹，在包含12万标注对话的基准上评估

Result: 8个大型语言模型在零样本提示下均失败，而提出的监督微调方法将平衡准确率提升高达23个百分点

Conclusion: 上下文感知的发言时机不是大型语言模型的涌现能力，必须通过显式训练才能获得
...
TL;DR: 人类在视觉上能更好地识别AI生成痕迹，但在检测AI生成财务文档方面却不如多模态大语言模型，因为主要检测信号是算术错误而非视觉特征。


<details>
  <summary>Details</summary>
Motivation: 研究人类与机器在检测AI生成财务文档方面的能力差异，探索AI生成文档的检测机制和评估方法。

Method: 创建GPT4o-Receipt基准数据集，包含1,235张收据图像，对比GPT-4o生成的收据和真实收据。使用5个最先进的多模态LLM和30名标注者的众包感知研究进行评估。

Result: 发现一个显著悖论：人类在视觉辨别AI痕迹方面表现最好，但在二元检测F1分数上却低于Claude Sonnet 4和Gemini 2.5 Flash。AI生成收据的主要法医信号是算术错误，这些错误对视觉检查不可见但可被LLM系统验证。

Conclusion: AI生成文档检测的关键在于算术验证而非视觉特征识别，简单准确度指标不足以选择检测器。GPT4o-Receipt基准和评估框架公开发布以支持未来AI文档法医研究。
...
Method: 提出Expert Threshold（ET）路由机制：每个专家维护一个基于全局token分布估计的指数移动平均阈值。在训练和推理时，每个token独立路由到专家，当token得分超过专家阈值时被路由到该专家。这种完全因果机制消除了对批次中其他token的依赖。
...
Method: 使用大语言模型和本体技术从患者病历中提取表型和结果标签；收集生命体征、人口统计学、分期、生物标志物等特征；基于NCCN指南提取化疗方案，通过NIH标准验证；采用随机生存森林模型预测治疗失败时间。

Result: 该方法显著减少了表型稀疏性并提高了预测准确性。随机生存森林模型的C指数达到73%，在特定时间点作为分类器预测治疗结果时，准确率和F1分数均超过70%。校准曲线验证了结果概率的可靠性。

Conclusion: 研究表明，基于大语言模型的临床数据提取方法能够早期预测治疗结果，为制定个性化治疗方案、改善患者预后提供了潜力，并成功扩展到其他癌症类型。
...
TL;DR: 研究表明，视觉语言模型在获得准确符号信息时能更好地将感知转化为具体行动，但模型自提取符号的能力受限于其感知质量。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型擅长描述视觉场景，但在将感知转化为精确、可执行的具体行动方面存在困难。研究者希望探索为模型同时提供视觉帧和场景符号表示是否能改善其在交互环境中的表现。

Method: 在Atari游戏、VizDoom和AI2-THOR环境中评估三种最先进的视觉语言模型，比较四种处理流程：仅视觉帧、视觉帧加自提取符号、视觉帧加真实符号、仅符号信息。

Result: 所有模型在获得准确符号信息时表现都有提升。但当模型需要自己提取符号时，性能取决于模型能力和场景复杂度。研究发现符号提取的准确性直接影响决策和游戏表现。

Conclusion: 符号基础化对视觉语言模型有益，但前提是符号提取要可靠。感知质量是未来基于视觉语言模型的智能体的核心瓶颈。
...
<details>
  <summary>Details</summary>
Motivation: 传统的持久性图能分析单个流形的拓扑特征，但无法捕捉两个点云之间的相互作用。交叉持久性图虽然能解决这个问题，但其密度特性尚未被系统研究，限制了其在统计分析和机器学习中的应用。

Method: 1. 证明交叉持久性图密度的存在性；2. 建立其统计理论基础；3. 设计首个机器学习框架，直接从点云坐标和距离矩阵预测交叉持久性密度；4. 利用交叉持久性图的线性特性来区分不同流形采样的点云；5. 探索噪声在增强区分能力中的新作用。

Result: 1. 成功证明了交叉持久性图密度的存在性；2. 在多种数据集上的实验表明，该方法在密度预测方面始终优于现有技术；3. 在点云区分任务中取得了优异结果；4. 发现引入噪声能增强区分点云的能力，揭示了噪声在TDA应用中的新效用。

Conclusion: 该研究为交叉持久性图提供了系统的理论基础和实用的机器学习框架，扩展了其在数据分析中的应用范围，包括时间序列领域任务和AI生成文本的几何分析等新方向。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在图表理解中存在视觉基元检测和语义对齐的困难，这限制了复杂视觉推理能力。缺乏感知基础成为图表推理的主要瓶颈。

Method: 基于图形感知理论形式化四个感知任务（包括位置和长度），提出Decomposition-of-Thought（DoT）提示方法，将问题顺序分解为视觉感知子问题和逻辑子问题，并在InternVL上进行微调。

Result: 在ChartQA上提升11.2%，在更难的ChartQAPro上超越GPT-4o，在新基准VisDoTQA上提升33.2%。在多样开放域VQA基准上的零样本增益证实了感知-逻辑分离策略的泛化性。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界数据集日益复杂和异构，监督学习常受限于输入表示设计。处理多模态数据（如时间序列、自由文本、结构化记录）需要大量领域特定工程，亟需自动化解决方案

Method: 1. LLM分析文本序列化输入示例，合成全局rubric作为提取和组织证据的程序化规范；2. 使用rubric将原始文本序列化输入转换为标准化格式；3. 还提出任务条件化的局部rubric

Result: 在EHRSHOT基准的15个临床任务中，rubric方法显著优于传统计数特征模型、朴素文本序列化LLM基线以及预训练大量数据的临床基础模型

Conclusion: Rubric方法不仅性能优越，还具有易于审计、部署成本低、可转换为表格表示等优势，适合医疗操作环境，能解锁多种机器学习技术应用
...
TL;DR: 该论文提出了一种显式逻辑通道（ELC）来验证、选择和增强多模态大语言模型（MLLMs）。ELC与黑盒模型通道并行工作，通过显式逻辑推理进行事实、反事实和关系推理，并使用一致性率（CR）进行跨通道验证和模型选择，无需真实标注。
...
Method: 提出显式逻辑通道（ELC），与黑盒模型通道并行工作。ELC模仿人类逻辑推理，包含LLM、VFM和概率推理的逻辑推理模块，对显式视觉证据进行事实、反事实和关系推理。提出一致性率（CR）进行跨通道验证和模型选择。

Result: 在三个具有挑战性的基准测试上，对来自4个前沿家族的11个开源MLLMs进行了两个代表性VLC任务（MC-VQA和HC-REC）的综合实验。系统评估表明ELC和CR在模型验证、选择和改进方面的有效性，增强了可解释性和可信度。

Conclusion: 提出的显式逻辑通道和一致性率机制能够有效验证、选择和增强多模态大语言模型，通过显式视觉证据增强模型的可信度，为黑盒MLLMs提供了验证和改进的框架。
...
TL;DR: 本文提出了一种适用于医院环境的LLM智能体架构，通过受限执行环境、文档中心交互、页面索引内存和医疗技能库四大核心组件，构建安全可控的医疗智能体操作系统。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM智能体在改善临床工作流程方面具有潜力，但由于可靠性限制、安全风险和长期记忆机制不足等问题，在医疗环境中部署自主智能体仍然困难。需要一种既能发挥智能体优势又能确保安全性的架构方案。
...
Result: 该架构形成了"医院智能体操作系统"的基础，能够协调临床工作流程，同时保持安全性、透明度和可审计性。该设计基于OpenClaw开源自主智能体框架，并扩展了临床安全部署所需的基础设施级约束。

Conclusion: 通过受限执行环境、文档中心交互、页面索引内存和医疗技能库的集成设计，为LLM智能体在医疗环境中的安全部署提供了可行的架构方案，为构建安全可控的医院智能体操作系统奠定了基础。
...
Method: 提出CINDI框架，基于条件归一化流建模数据的精确条件似然，统一异常检测和插补为端到端系统，通过识别低概率段并迭代采样统计一致的替换值

Result: 在挪威电力分配运营商的真实电网损耗数据上评估，CINDI相比竞争基线表现出鲁棒性能，为噪声环境提供可扩展的可靠性维护方案

Conclusion: CINDI通过统一的概率框架有效恢复复杂时间序列的数据完整性，能够泛化到任何多元时间序列领域，同时保持系统的物理和统计特性
...
TL;DR: 该论文提出了一种新的多变量时间序列异常检测方法，通过将异常概念重新定位到规定的潜在空间，使用条件归一化流在状态空间框架中建模时间序列，将异常检测转化为统计合规性测试。


<details>
  <summary>Details</summary>
Motivation: 传统基于观测空间似然的深度生成模型存在结构限制，因为观测空间的似然衡量的是边际密度而非结构化时间动态的符合性，可能导致对异常样本分配高概率。需要将异常概念重新定位到潜在空间来解决这一问题。

Method: 在条件归一化流中引入显式归纳偏置，在离散时间状态空间框架中建模时间序列观测，约束潜在表示按照规定的时间动态演化。异常检测简化为统计合规性测试，将观测映射到潜在空间并通过拟合优度检验评估是否符合规定的潜在演化。

Result: 在合成和真实世界时间序列上的实验表明，该方法能够可靠地检测频率、幅度和观测噪声方面的异常，同时提供模型合规性的可解释诊断。

Conclusion: 通过将异常重新定义为对规定潜在动态的违反，该方法提供了基于统计的异常检测原则性决策规则，即使在观测似然高的区域也能保持有效性，解决了传统方法的局限性。
...
TL;DR: 该研究深入分析Wikidata限定符的语义和实际使用，开发了一个分类法来解决限定符选择、图查询和逻辑推理的挑战，通过改进的香农熵指数评估限定符重要性，并对前300个限定符进行了分类。


<details>
  <summary>Details</summary>
Motivation: Wikidata限定符在知识表示中扮演重要角色，但缺乏系统性的语义分析和分类框架，导致在限定符选择、图查询和逻辑推理方面存在挑战。需要开发一个结构化的分类法来指导贡献者创建和查询语句，改进限定符推荐系统，并增强知识图谱设计方法。

Method: 研究使用改进的香农熵指数评估限定符的重要性，考虑了频率和多样性，特别关注"长尾"现象。通过分析Wikidata数据转储，选取了前300个限定符，并将其分类为上下文限定符、认知/不确定性限定符、结构限定符和附加限定符四个主要类别。

Result: 研究结果显示，开发的分类法有效覆盖了最重要的限定符，为理解和利用Wikidata限定符提供了结构化方法。该分类法能够指导贡献者创建和查询语句，改进限定符推荐系统，并增强知识图谱设计方法。

Conclusion: 该研究提出的限定符分类法为Wikidata知识表示提供了系统框架，解决了限定符选择、查询和推理的挑战，对知识图谱社区具有重要价值，能够提升知识表示的质量和可查询性。
...
TL;DR: 论文提出了SDec-POMDP框架，用于处理通信不确定环境下的半分散多智能体控制，统一了分散式POMDP和现有通信机制，并开发了RS-SDA*精确算法求解最优策略。


<details>
  <summary>Details</summary>
Motivation: 在多智能体协作环境中，通信不确定性是一个关键挑战。现有方法要么完全分散（无通信），要么完全集中（完美通信），缺乏处理通信延迟和不确定性的统一理论框架。需要一种能够建模通信时间分布、统一现有通信机制的理论基础。

Method: 1. 提出SDec-POMDP框架，将半马尔可夫通信（semi-decentralization）扩展到部分可观测马尔可夫决策过程。2. 该框架允许智能体历史中存储的动作和观测具有时间分布。3. 开发RS-SDA*算法（递归小步半分散A*），用于生成最优SDec-POMDP策略。4. 在标准基准测试和海上医疗撤离场景中进行评估。

Result: 1. SDec-POMDP成功统一了分散式POMDP、多智能体POMDP和多种现有显式通信机制。2. RS-SDA*算法能够精确生成最优策略。3. 在多个基准测试和实际场景中验证了框架和算法的有效性。

Conclusion: 论文为通过半分散化视角探索多智能体通信问题提供了明确的理论基础。SDec-POMDP框架能够建模通信不确定性，RS-SDA*算法能够求解最优策略，为多智能体协作通信研究开辟了新方向。
...
### [30] [VisiFold: Long-Term Traffic Forecasting via Temporal Folding Graph and Node Visibility](https://arxiv.org/abs/2603.11816)
...
### [31] [Automated Detection of Malignant Lesions in the Ovary Using Deep Learning Models and XAI](https://arxiv.org/abs/2603.11818)
*Md. Hasin Sarwar Ifty,Nisharga Nirjan,Labib Islam,M. A. Diganta,Reeyad Ahmed Ornate,Anika Tasnim,Md. Saiful Islam*

Main category: cs.AI

TL;DR: 使用多种CNN架构开发卵巢癌检测模型，通过XAI解释模型决策，InceptionV3在增强数据集上表现最佳达到94%平均性能指标。


<details>
  <summary>Details</summary>
Motivation: 卵巢癌检测面临非侵入性方法不准确、侵入性方法耗时的困境，需要开发更有效的深度学习检测方法以改善临床诊断决策。

Method: 使用LeNet-5、ResNet、VGGNet和GoogLeNet/Inception等CNN架构开发15个变体模型，在Mendeley的OvarianCancer&SubtypesDatasetHistopathology数据集上进行训练，采用LIME、Integrated Gradients和SHAP等XAI方法解释模型决策。

Result: InceptionV3模型在增强数据集上表现最佳，在所有性能指标（准确率、精确率、召回率、F1分数、ROC曲线和AUC）上平均达到94%的分数。

Conclusion: 研究开发了有效的卵巢癌检测深度学习模型，并通过XAI提供了模型决策的可解释性，有望改善卵巢癌的检测方法。
...
<details>
  <summary>Details</summary>
Motivation: 高质量预训练数据饱和后，研究重点转向能够持续生成新颖产物的进化系统（如AlphaEvolve），但这类系统的进展受到缺乏严谨、定量评估的阻碍。

Method: 引入CreativeBench基准测试，包含CreativeBench-Combo（组合创造力）和CreativeBench-Explore（探索创造力）两个子集，采用基于逆向工程和自对弈的自动化流程，利用可执行代码通过统一指标（质量×新颖性）客观评估创造力。

Result: 对最先进模型的分析发现：（1）规模化显著提升组合创造力但对探索创造力收益递减；（2）更大模型呈现"规模化收敛"，变得更正确但更少发散；（3）推理能力主要受益于受限探索而非组合。提出EvoRePE，一种即插即用的推理时引导策略，内化进化搜索模式以持续增强机器创造力。

Conclusion: CreativeBench为机器创造力提供了严谨的定量评估框架，揭示了模型规模化和推理能力对创造力的不同影响，并提出了有效的进化引导策略来增强机器创造力。
...
### [33] [AdaFuse: Accelerating Dynamic Adapter Inference via Token-Level Pre-Gating and Fused Kernel Optimization](https://arxiv.org/abs/2603.11873)
*Qiyang Li,Rui Kong,Yuchen Li,Hengyi Cai,Shuaiqiang Wang,Linghe Kong,Guihai Chen,Dawei Yin*

Main category: cs.AI

TL;DR: AdaFuse框架通过令牌级预门控策略和融合CUDA内核，解决了MoE与LoRA适配器结合时推理延迟激增的问题，将解码速度提升2.4倍以上。


<details>
  <summary>Details</summary>
Motivation: 动态稀疏结构（如MoE）与参数高效适配器（如LoRA）结合能增强大语言模型能力，但导致推理延迟激增（解码速度下降2.5倍以上），主要瓶颈在于动态路由所需的碎片化顺序CUDA内核启动开销。

Method: 提出AdaFuse框架，采用算法与硬件系统协同设计：1）令牌级预门控策略，在处理令牌前做出单一全局路由决策；2）开发定制CUDA内核，执行融合切换操作，将所选LoRA适配器参数一次性合并到骨干模型中。

Result: 在开源大语言模型上的实验表明，AdaFuse在保持与最先进动态适配器相当准确性的同时，将解码延迟大幅降低2.4倍以上，弥合了模型能力与推理效率之间的差距。

Conclusion: AdaFuse通过"一次决策，处处应用"的方法和硬件协同优化，有效解决了动态适配器执行效率问题，为增强型大语言模型的实际部署提供了可行的解决方案。
...
### [34] [Fair Learning for Bias Mitigation and Quality Optimization in Paper Recommendation](https://arxiv.org/abs/2603.11936)
*Uttamasha Anjally Oyshi,Susan Gauch*

Main category: cs.AI

TL;DR: Fair-PaperRec是一个基于MLP的模型，旨在解决论文评审中的作者人口统计学偏见问题，通过惩罚人口统计学差异同时保持质量要求，在多个会议数据上实现了42.03%的弱势群体参与度提升和3.16%的整体效用改善。


<details>
  <summary>Details</summary>
Motivation: 尽管采用双盲评审，作者的人口统计学偏见仍然使弱势群体处于不利地位。现有方法往往采用启发式方法，缺乏系统性的公平性保障机制。

Method: 提出Fair-PaperRec模型，基于多层感知机(MLP)，通过交叉性标准（如种族、国家）和定制化的公平性损失函数来惩罚人口统计学差异，同时保持论文质量要求。

Result: 在ACM SIGCHI、DIS和IUI会议数据上的评估显示：弱势群体参与度提升了42.03%，整体效用改善了3.16%，表明促进多样性不会损害学术严谨性。

Conclusion: Fair-PaperRec能够有效解决论文评审中的人口统计学偏见问题，在提升多样性的同时保持学术质量，为公平导向的同行评审提供了可行的技术解决方案。
...
### [35] [Prototype-Based Knowledge Guidance for Fine-Grained Structured Radiology Reporting](https://arxiv.org/abs/2603.11938)
*Chantal Pellegrini,Adrian Delchev,Ege Özsoy,Nassir Navab,Matthias Keicher*

Main category: cs.AI

TL;DR: ProtoSR：一种通过从自由文本报告中提取视觉原型知识来增强结构化放射学报告生成的方法，在细粒度属性问题上取得SOTA效果


<details>
  <summary>Details</summary>
Motivation: 结构化放射学报告比自由文本报告更快、更一致，但自动化困难，因为模型需要从有限的监督中做出许多细粒度的决策。而自由文本报告在常规护理中大规模产生，通过详细描述隐式编码了细粒度的图像关联信息。

Method: 1. 使用指令调优的LLM从80k+ MIMIC-CXR研究中自动提取信息，构建与结构化报告模板对齐的多模态知识库，为每个答案选项创建视觉原型。2. 训练ProtoSR检索当前图像-问题对相关的原型，通过原型条件残差增强模型预测，提供数据驱动的第二意见来选择性修正预测。

Result: 在Rad-ReStruct基准测试中，ProtoSR取得了最先进的结果，在详细属性问题上改进最大，证明了整合自由文本衍生信号对细粒度图像理解的价值。

Conclusion: 通过从自由文本报告中提取知识并构建视觉原型知识库，ProtoSR成功地将自由文本信息注入结构化报告生成，显著提升了细粒度图像理解能力，为结构化放射学报告自动化提供了有效解决方案。
...
### [36] [Learning Transferable Sensor Models via Language-Informed Pretraining](https://arxiv.org/abs/2603.11950)
*Yuliang Chen,Arvind Pillai,Yu Yvonne Wu,Tess Z. Griffin,Lisa Marsch,Michael V. Heinz,Nicholas C. Jacobson,Andrew Campbell*

Main category: cs.AI

TL;DR: SLIP是一个开源的多传感器语言对齐预训练框架，通过对比对齐和传感器条件描述，学习可跨不同传感器配置通用的语言对齐表示，在零样本迁移、信号描述和问答任务上表现优异。
...
Method: SLIP整合对比对齐与传感器条件描述，通过跨注意力机制重用预训练的解码器语言模型，并引入灵活的补丁嵌入器，支持不同时间分辨率和变长输入。

Result: 在11个数据集上，SLIP在零样本迁移、信号描述和问答任务中表现优异：平均线性探测准确率达77.14%（相对基线提升5.93%），传感器问答准确率达64.83%。

Conclusion: SLIP是一个灵活、通用的传感器-语言对齐预训练框架，能够学习跨不同传感器配置的语义表示，显著提升下游任务的性能。
...
### [37] [Normative Common Ground Replication (NormCoRe): Replication-by-Translation for Studying Norms in Multi-agent AI](https://arxiv.org/abs/2603.11974)
*Luca Deck,Simeon Allmendinger,Lucas Müller,Niklas Kühl*

Main category: cs.AI

TL;DR: 提出了NormCoRe框架，将人类实验设计系统转化为多智能体AI环境，用于研究规范性协调的动态过程


<details>
  <summary>Details</summary>
Motivation: 现有方法常将规范视为对齐或复制的目标，隐含假设人类主体与AI智能体等价，未能充分研究集体规范性动态。需要系统方法将人类实验设计转化为MAAI环境

Method: 提出Normative Common Ground Replication (NormCoRe)框架，基于行为科学、复制研究和先进MAAI架构，将人类实验的结构层映射到AI智能体研究设计中

Result: 通过复制分配正义实验发现，AI智能体的规范性判断与人类基准存在差异，且对基础模型选择和智能体角色语言实例化敏感
...
TL;DR: FedFew将个性化联邦学习重新定义为少对多优化问题，仅维护K个共享服务器模型（K远小于客户端数M）来服务所有客户端，证明能达到近似最优个性化效果。
...
Result: 理论证明：近似误差随K增加而减小，每个客户端模型随数据增长收敛到最优。实验表明：在视觉、NLP和真实医疗影像数据集上，仅用3个模型的FedFew持续优于其他SOTA方法。

Conclusion: FedFew通过少对多优化框架解决了PFL的可扩展性问题，仅需少量共享模型即可实现近似最优个性化，无需手动客户端分区或复杂超参数调优。
...
<details>
  <summary>Details</summary>
Motivation: 现有RFT评估大多在相同环境中进行训练和测试，但实际部署中智能体需要在未见过的环境中运行，这些环境具有不同的背景知识、观察空间和行动接口。需要系统研究RFT在这种环境变化下的泛化能力。

Method: 从三个维度进行系统研究：1）环境内跨任务难度的泛化；2）跨环境迁移到未见环境；3）顺序多环境训练以量化迁移和遗忘。分析语义先验和观察/行动接口变化对泛化的影响。

Result: RFT在环境内跨任务难度泛化良好，但向未见环境的迁移较弱，这与语义先验和观察/行动接口的变化相关。顺序训练在下游任务中表现良好且上游遗忘最小，跨环境混合训练改善了整体平衡。

Conclusion: RFT在环境内泛化能力强，但跨环境迁移需要改进。顺序训练和混合训练是提升泛化能力的有效策略。研究为开发可泛化的LLM智能体提供了重要见解。
...
### [40] [XSkill: Continual Learning from Experience and Skills in Multimodal Agents](https://arxiv.org/abs/2603.12056)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态智能体在开放环境中存在工具使用效率低下和编排不灵活的问题，需要在不更新参数的情况下通过历史轨迹进行持续学习。研究发现经验和技能两种互补的可复用知识对智能体改进至关重要。

Method: 提出XSkill双流框架：1）在积累阶段，通过视觉基础总结和多轮次交叉评估从多路径轨迹中提炼和整合经验与技能；2）在推理阶段，根据当前视觉上下文检索和适配知识，并将使用历史反馈到积累阶段形成持续学习循环。

Result: 在五个不同领域的基准测试中使用四个骨干模型进行评估，XSkill持续且显著优于仅使用工具和基于学习的基线方法。分析表明两种知识流在影响智能体推理行为方面发挥互补作用，并展现出优异的零样本泛化能力。
...
### [41] [A Robust and Efficient Multi-Agent Reinforcement Learning Framework for Traffic Signal Control](https://arxiv.org/abs/2603.12096)
...
TL;DR: 提出一个多智能体强化学习框架用于交通信号控制，通过转向比随机化、指数相位调整和邻居观测机制，在Vissim仿真中验证了优于基准方法的性能，减少平均等待时间超10%。
...
Result: 在Vissim交通仿真实验中，该框架优于标准强化学习基准方法，平均等待时间减少超过10%，在未见交通场景中表现出更好的泛化能力，并保持了高控制稳定性。

Conclusion: 该研究提出的多智能体强化学习框架通过创新的训练策略、动作空间设计和观测机制，为自适应信号控制提供了实用的解决方案，能够有效应对动态交通流变化并保持稳定控制。
...
### [42] [TopoBench: Benchmarking LLMs on Hard Topological Reasoning](https://arxiv.org/abs/2603.12133)
...
<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在需要全局空间推理（如连通性、环路闭合和区域对称性）的拓扑网格谜题上的能力，这些任务对现有模型仍具挑战性。
...
Conclusion: 大语言模型在拓扑网格谜题上的主要限制是从空间表示中提取和维持约束的能力，而非推理能力本身，这为未来改进提供了方向。
...
### [43] [Compiling Temporal Numeric Planning into Discrete PDDL+: Extended Version](https://arxiv.org/abs/2603.12188)
...
<details>
  <summary>Details</summary>
Motivation: 虽然PDDL+语言引入后就已知带持续动作的时态规划可以编译到PDDL+，但文献中一直缺乏实用的编译方法。本文旨在填补这一空白。

Method: 提出了一种从带持续动作的时态规划到PDDL+的实用编译方法，完全捕捉语义，仅假设动作不自我重叠。该编译是多项式复杂度的。

Result: 编译方法在保持计划长度常数倍的同时，实验证明对困难的时态数值问题具有实际应用价值。
...
### [44] [Portfolio of Solving Strategies in CEGAR-based Object Packing and Scheduling for Sequential 3D Printing](https://arxiv.org/abs/2603.12224)
*Pavel Surynek*
...
<details>
  <summary>Details</summary>
Motivation: 现代多核个人计算机CPU的计算能力已接近几十年前的超级计算机，特别是并行计算能力。研究者希望有效利用现代多核CPU的计算能力来解决复杂的组合优化问题——3D打印中的物体顺序排列和调度问题。

Method: 1. 并行化现有的CEGAR-SEQ算法，该算法通过线性算术公式表达问题，并使用反例引导抽象精化技术求解。2. 提出替代的物体排列策略：将物体放置在打印板角落而非中心，以及根据物体高度进行调度。3. 在高层次上并行执行CEGAR-SEQ算法，使用多种物体排列策略组合，形成Portfolio-CEGAR-SEQ算法。

Result: 实验评估表明，Portfolio-CEGAR-SEQ算法优于原始的CEGAR-SEQ算法。在多个打印板的批量物体调度场景中，Portfolio-CEGAR-SEQ通常使用更少的打印板。
...
### [45] [Examining Reasoning LLMs-as-Judges in Non-Verifiable LLM Post-Training](https://arxiv.org/abs/2603.12246)
*Yixin Liu,Yue Yu,DiJia Su,Sid Wang,Xuewei Wang,Song Jiang,Bo Liu,Arman Cohan,Yuandong Tian,Zhengxing Chen*

Main category: cs.AI

TL;DR: 该研究系统评估了推理型LLM评判员在强化学习对齐中的实际效果，发现推理型评判员能训练出性能更强的策略，但这些策略可能通过生成对抗性输出来欺骗其他LLM评判员。


<details>
  <summary>Details</summary>
Motivation: 虽然推理型LLM评判员在静态评估基准上表现更好，但其在实际策略训练中的有效性尚未得到系统检验。研究旨在探究非推理型和推理型评判员在基于强化学习的LLM对齐中的实际影响。

Method: 采用受控合成设置，使用"黄金标准"评判员(gpt-oss-120b)提供偏好标注来训练较小的评判员，系统比较非推理型和推理型评判员在策略训练中的表现。

Result: 非推理型评判员容易导致奖励黑客攻击，而推理型评判员能训练出在黄金标准评判员评估下表现强劲的策略。但推理型评判员训练的策略通过生成能欺骗其他LLM评判员（如Arena-Hard基准）的对抗性输出来实现这种表现。

Conclusion: 研究揭示了推理型LLM评判员在非可验证LLM后训练中的应用既有重要发现也有改进空间，特别是关于对抗性输出和欺骗其他评判员的问题。
...
### [46] [Comparison of Outlier Detection Algorithms on String Data](https://arxiv.org/abs/2603.11049)
*Philip Maus*

Main category: cs.LG

TL;DR: 该论文比较了两种字符串异常值检测算法：基于编辑距离的局部异常因子算法变体和基于层次正则表达式学习的新算法，实验证明两者都能有效检测字符串数据中的异常值。


<details>
  <summary>Details</summary>
Motivation: 目前机器学习中异常值检测研究主要集中在数值数据上，对字符串数据异常值检测的研究较少。一个鲁棒的字符串异常值检测算法可以帮助数据清洗和系统日志文件中的异常检测。

Method: 1. 提出局部异常因子算法的变体，使用Levenshtein距离计算数据集密度，并引入考虑层次字符类别的加权Levenshtein度量；2. 提出基于层次左正则表达式学习的新算法，通过推断预期数据的正则表达式来检测异常值。

Result: 实验表明两种算法都能在概念上找到字符串数据中的异常值。基于正则表达式的算法在预期值具有与异常值明显不同结构时表现优异，而局部异常因子算法在异常值与预期数据之间的编辑距离明显不同于预期数据之间的编辑距离时效果最好。

Conclusion: 论文成功比较了两种字符串异常值检测方法，为字符串数据异常检测提供了有效工具，两种算法在不同场景下各有优势，可根据数据特征选择合适的算法。
...
### [47] [Fingerprinting Concepts in Data Streams with Supervised and Unsupervised Meta-Information](https://arxiv.org/abs/2603.11094)
...
<details>
  <summary>Details</summary>
Motivation: 数据流处理中概念漂移是一个主要挑战，现有概念表示方法依赖少量元信息特征，往往无法有效区分不同概念，导致系统对概念漂移脆弱。

Method: 提出FiCSUM框架，构建概念指纹——包含多种不同元信息特征的向量，能够唯一识别更多概念。采用动态加权策略学习哪些元信息特征在特定数据集中描述概念漂移。

Result: 在11个真实世界和合成数据集上，FiCSUM在准确性和建模底层概念漂移方面均优于最先进的方法。

Conclusion: FiCSUM通过丰富的元信息特征和动态加权策略，能够更有效地表示概念并检测概念漂移，为数据流处理提供了更强大的工具。
...
TL;DR: 提出了一种通过梯度下降学习硬轴对齐决策树的新方法，使用带直通算子的反向传播在密集DT表示上进行联合优化，解决了传统DT算法的局限性，并在多个领域实现了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 传统决策树学习方法（如CART）存在两个主要问题：1）使用贪婪搜索过程，在节点处做出局部最优决策，限制了搜索空间并导致次优树结构；2）需要定制训练方法，无法无缝集成到现代机器学习方法中。决策树学习具有组合复杂性和离散、不可微分的特性，使得传统方法面临挑战。

Method: 提出了一种通过梯度下降学习硬轴对齐决策树的新方法。该方法使用带直通算子的反向传播在密集决策树表示上进行联合优化，能够同时优化所有树参数。这种方法不受顺序选择局部最优分割的限制，并且可以无缝集成到依赖梯度下降的现有机器学习方法中。

Result: 该方法在多个领域实现了最先进的结果，包括：1）小表格数据集的可解释决策树；2）复杂表格数据的高级模型；3）多模态学习；4）无信息损失的可解释强化学习。通过弥合决策树与基于梯度的优化之间的差距，显著提高了树模型在各种机器学习领域的性能和适用性。

Conclusion: 提出的基于梯度下降的决策树学习方法解决了传统DT算法的两个主要局限性，通过联合优化所有树参数和无缝集成到现有ML框架中，显著增强了树基模型的性能和适用性，为可解释机器学习开辟了新的可能性。
...
TL;DR: HOMA是一种融合了成对注意力和三元交互路径的高阶模块化注意力机制，用于捕捉蛋白质序列中三个或更多残基之间的协同依赖关系，在蛋白质序列预测任务中表现优于标准自注意力机制。
...
Method: 提出了高阶模块化注意力(HOMA)，将成对注意力与显式的三元交互路径融合。为了在长序列上实现三元注意力的实用性，HOMA采用块结构、窗口化的三元注意力机制。

Result: 在TAPE基准测试的三个任务（二级结构、荧光、稳定性）上评估，HOMA相比标准自注意力及其高效变体（包括块状注意力和Linformer）在所有任务上都取得了持续改进。

Conclusion: 显式的三元项为蛋白质序列预测提供了互补的表征能力，且计算成本可控增加，表明高阶交互建模对蛋白质序列分析具有重要意义。
...
### [50] [Scaling Reasoning Efficiently via Relaxed On-Policy Distillation](https://arxiv.org/abs/2603.11137)
*Jongwoo Ko,Sara Abdali,Young Jin Kim,Tianyi Chen,Pashmina Cameron*

Main category: cs.LG

TL;DR: REOPOLD是一个改进的在线蒸馏框架，通过将教师-学生对数似然比作为token奖励，放松严格模仿约束，提升训练稳定性和推理性能。


<details>
  <summary>Details</summary>
Motivation: 在线蒸馏对于将推理能力转移到容量受限模型至关重要，但存在不稳定和负迁移问题。作者发现在线蒸馏可解释为一种策略优化形式，教师-学生对数似然比可作为token奖励，从而提出改进方案。

Method: 提出REOPOLD框架：1) 通过混合奖励裁剪温和选择性地利用教师奖励；2) 基于熵的token级动态采样；3) 统一的探索到精炼训练策略，放松标准在线蒸馏的严格模仿约束。

Result: REOPOLD在数学、视觉和工具使用推理任务上超越基线方法，训练时具有6.7~12倍样本效率优势，推理时实现更好的扩展性。7B学生模型在视觉推理上匹配32B教师性能，推理速度提升约3.32倍。

Conclusion: REOPOLD通过将在线蒸馏重新解释为策略优化并放松模仿约束，显著提升了蒸馏的稳定性和效率，使小模型能够更有效地学习大模型的推理能力。
...
### [51] [On the Robustness of Langevin Dynamics to Score Function Error](https://arxiv.org/abs/2603.11319)
*Daniel Yiming Cao,August Y. Chen,Karthik Sridharan,Yuchen Wu*

Main category: cs.LG

TL;DR: 研究表明，基于分数的生成建模中，朗之万动力学对分数函数估计误差不稳健，即使L^2误差任意小，多项式时间内也会产生与目标分布差异很大的分布。


<details>
  <summary>Details</summary>
Motivation: 研究分数函数估计误差对基于分数的生成建模稳健性的影响，特别是在实践中从数据学习分数函数时，这种误差不可避免，需要评估不同方法的稳健性差异。

Method: 分析朗之万动力学在分数函数存在L^2误差（更一般地L^p误差）情况下的表现，通过理论证明即使在简单的高维分布中，多项式时间内的朗之万动力学也会产生与目标分布差异显著的分布。

Result: 朗之万动力学对分数函数估计误差不稳健，即使误差任意小，在多项式时间内产生的分布与目标分布的总变差距离仍然很大；而扩散模型在小L^2误差下能在多项式时间内忠实采样。

Conclusion: 研究结果为扩散模型相对于朗之万动力学提供了进一步的理论依据，并警告不要使用带有估计分数的朗之万动力学进行采样。
...
TL;DR: 该研究通过持续预训练将OLMo-3-7B模型适配到嵌入式系统编程领域，在8个H100 GPU上使用BF16 LoRA训练，在13个嵌入式领域代码生成任务中超越Claude Opus 4.6和Qwen3-Coder-30B


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用编程语言上表现良好，但在嵌入式系统编程等专业领域受限，因为这些领域涉及硬件寄存器操作、厂商特定SDK、实时操作系统API等，在标准预训练语料中代表性不足

Method: 采用持续预训练管道H2LooP Spark Preview，使用BF16 LoRA配合秩稳定缩放技术，在8个NVIDIA H100 GPU上训练。训练语料基于仓库-数据表对构建，覆盖100B原始嵌入式系统数据，使用SpecMap提出的分层数据表到代码映射方法处理，最终得到23.5B token的精选数据集

Result: 持续预训练带来显著提升：域内困惑度降低70.4%，保留仓库困惑度降低66.1%。在13个嵌入式领域的生成代码补全基准测试中，7B模型在8个类别上超越Claude Opus 4.6和Qwen3-Coder-30B的token准确率

Conclusion: 针对性的持续预训练使较小的开源模型能够在专业技术任务上与前沿系统竞争，研究将生产训练检查点作为开源工件在Huggingface上发布
...
<details>
  <summary>Details</summary>
Motivation: 现有的强化学习算法主要关注个体学习，未能充分利用人类和动物常见的社会学习能力。社会学习能够整合个体经验和观察他人行为，有望改善学习效果。

Method: 提出基于自由能量的社会多臂赌博机学习算法，在策略空间上进行操作。社会智能体通过观察其他智能体的行为（无需知道其奖励）来评估他们的专业水平，无需依赖任何先知或社会规范。算法整合了智能体在环境中的直接经验和其他智能体的估计策略。

Result: 理论证明了算法收敛到最优策略。实证评估表明，在各种场景下，该社会学习方法优于替代方法。算法能够策略性地识别相关智能体（即使在存在随机或次优智能体的情况下），并巧妙利用他们的行为信息。在包含专家智能体以及相关但非专家智能体的社会中，算法显著提升了个体学习性能，而大多数相关方法在此情况下失败。同时保持对数遗憾。

Conclusion: 提出的基于自由能量的社会学习算法能够有效利用社会观察来增强个体学习，在多种社会配置下表现出色，为个性化AI服务中的多智能体学习提供了有效的解决方案。
...
TL;DR: GCIG是一个通过正则化强制不同保护群体间解释不变性的框架，在保持预测性能的同时显著减少解释差异


<details>
  <summary>Details</summary>
Motivation: 当前机器学习公平性研究主要关注结果导向的公平标准（如Equalized Odds），而对过程导向的公平性关注较少。忽视过程公平性可能导致模型为不同保护群体生成不同的解释，从而损害信任。
...
Result: 与六种最先进方法相比，GCIG显著减少了跨群体解释差异，同时保持了有竞争力的预测性能和准确性与公平性的权衡。

Conclusion: 将模型推理在不同群体间对齐为超越结果平等的公平性提供了原则性和实用的途径。GCIG将过程公平性形式化为群体反事实解释稳定性，补充了仅约束预测的现有公平性目标。
...
### [55] [A Further Efficient Algorithm with Best-of-Both-Worlds Guarantees for $m$-Set Semi-Bandit Problem](https://arxiv.org/abs/2603.11764)
...
<details>
  <summary>Details</summary>
Motivation: FTPL作为对抗性组合半赌博问题的高效算法备受关注，但其最优性一直未被证明，而FTRL的最优性已在多种在线学习任务中得到验证。本文旨在填补这一空白，研究FTPL在m-set半赌博问题中的最优性。

Method: 将带几何重采样的FTPL分析扩展到m-set半赌博问题，使用特定参数的Fréchet和Pareto分布，并提出了条件几何重采样方法以降低计算复杂度。

Result: 在对抗性设置中，FTPL实现了O(√(mdT))的最优遗憾界；在随机性设置中实现了对数遗憾，证明了FTPL在m-set半赌博问题中的"两全其美"最优性。条件几何重采样将计算复杂度从O(d²)降低到O(md(log(d/m)+1))。

Conclusion: FTPL在m-set半赌博问题中具有最优性，既能处理对抗性环境又能适应随机性环境，同时通过条件几何重采样实现了计算效率的显著提升。
...
TL;DR: 视频分类模型内部存在计算"成功vs失败"信号的复杂电路，该电路通过注意力头收集证据、MLP块组合概念的分布式冗余架构实现，揭示了模型可能发展出超越显式任务的"隐藏知识"。


<details>
  <summary>Details</summary>
Motivation: 研究视频分类模型如何表示不影响最终分类结果的细微隐藏语义信息，这是构建可信AI模型的关键挑战。通过可解释AI方法探索模型内部是否形成了超越简单分类任务的复杂知识表示。

Method: 使用机制可解释性技术，特别是激活修补和消融实验，对预训练视频视觉Transformer进行逆向工程。分析注意力头和MLP块在表示动作结果信号中的不同作用。

Result: 发现"成功vs失败"信号通过独特的放大级联计算，从第5层到第11层逐步放大抽象语义表示。注意力头作为"证据收集器"提供低层信息，MLP块作为"概念组合器"生成成功信号，形成分布式冗余电路。

Conclusion: 即使仅用于简单分类训练的模型也可能发展出复杂结果的内部表示电路，揭示了模型可能形成超越显式任务的"隐藏知识"，强调了构建真正可解释和可信AI系统需要机制监督。
...
### [57] [Systematic Scaling Analysis of Jailbreak Attacks in Large Language Models](https://arxiv.org/abs/2603.11149)
*Xiangwen Wang,Ananth Balashankar,Varun Chandrasekaran*

Main category: cs.LG

TL;DR: 该论文提出了一个衡量越狱攻击效率的扩展定律框架，通过计算FLOPs来系统评估不同越狱方法的成功率，发现基于提示的方法比基于优化的方法更高效，且攻击成功率与目标类型密切相关。


<details>
  <summary>Details</summary>
Motivation: 当前对大型语言模型越狱攻击的研究缺乏系统性理解，特别是攻击成功率如何随攻击者努力程度、方法类型、模型家族和危害类型而变化。需要建立一个统一的框架来量化比较不同越狱方法的效率。

Method: 提出了一个基于计算资源的扩展定律框架，将每种攻击视为计算受限的优化过程，在共享的FLOPs轴上衡量进展。系统评估了四种代表性越狱范式：基于优化的攻击、自我精炼提示、基于采样的选择和遗传优化，覆盖多个模型家族和规模。通过拟合饱和指数函数来分析FLOPs与成功率的关系。

Result: 基于提示的范式通常比基于优化的方法更计算高效；攻击在成功率-隐蔽性操作点上分布不同，基于提示的方法占据高成功率、高隐蔽性区域；漏洞性强烈依赖于目标类型，涉及错误信息的危害通常比其他非错误信息危害更容易引发。

Conclusion: 建立了一个系统评估越狱攻击效率的扩展定律框架，揭示了基于提示的方法在计算效率和隐蔽性方面的优势，同时强调了危害类型对攻击成功率的重要影响，为理解和防御越狱攻击提供了量化基础。
...
### [58] [On-Average Stability of Multipass Preconditioned SGD and Effective Dimension](https://arxiv.org/abs/2603.11989)
*Simon Vary,Tyler Farghly,Ilja Kuzborskij,Patrick Rebeschini*

Main category: cs.LG

TL;DR: 研究预条件随机梯度下降（PSGD）多轮训练中，种群风险曲率、噪声几何和预条件选择之间的权衡对泛化能力的影响，提出新的平均算法稳定性分析框架，并给出匹配的上下界。


<details>
  <summary>Details</summary>
Motivation: 实际优化启发式方法（如梯度噪声白化或更新与损失曲率对齐）隐含地处理曲率与噪声几何之间的权衡。当两者不匹配时，激进的选择可能放大不稳定性，导致次优统计行为。需要理论分析这种权衡对泛化的影响。

Method: 使用平均算法稳定性分析多轮PSGD的泛化能力，开发新的稳定性分析框架处理数据重用引起的相关性，推导依赖于有效维度的超额风险上界，并补充匹配的实例相关下界。

Result: 建立了PSGD泛化能力与有效维度（取决于种群风险曲率和梯度噪声几何）的理论联系。证明不当选择的预条件器可能导致优化和泛化中的次优有效维度依赖。

Conclusion: 种群风险曲率、噪声几何和预条件选择之间的权衡对PSGD的泛化能力至关重要。不当的预条件选择会恶化有效维度依赖，而新的稳定性分析框架为多轮训练提供了理论保证。
...
TL;DR: 该研究系统评估了针对亨廷顿病(HD)病理语音的自动语音识别(ASR)，使用临床语音语料库比较多种ASR架构，发现Parakeet-TDT表现最佳，并通过HD特定适应将WER从6.99%降至4.95%，同时提出基于生物标志物的辅助监督方法。


<details>
  <summary>Details</summary>
Motivation: 病理语音的自动语音识别研究不足，特别是亨廷顿病(HD)语音具有不规则时序、不稳定发声和发音扭曲等特点，对现有ASR模型构成挑战，需要专门研究来改善HD患者的语音识别效果。

Method: 使用高保真临床语音语料库进行系统研究，在统一评估框架下比较多种ASR架构家族，分析WER以及替换、删除、插入错误模式。提出HD特定适应方法和基于生物标志物的辅助监督策略。

Result: HD语音导致架构特定的错误模式，Parakeet-TDT优于编码器-解码器和CTC基线。HD特定适应将WER从6.99%降至4.95%，基于生物标志物的辅助监督重塑了错误行为，而非均匀改善WER，错误模式随严重程度呈现不同变化。

Conclusion: 该研究为HD病理语音识别提供了系统评估框架，展示了架构选择和特定适应的重要性，提出的生物标志物监督方法为病理语音识别提供了新思路，所有代码和模型已开源。
...
<details>
  <summary>Details</summary>
Motivation: 标准贝叶斯优化在高维非线性系统中收敛缓慢，需要大量实验，而物理系统通常包含部分已知的机理模型，可以结合这些先验知识提高优化效率

Method: 将已知物理方程作为约束，对缺失方程的变量使用高斯过程进行概率建模，形成混合模型约束的非线性随机规划，采用样本平均近似进行离散化

Result: 在单级蒸馏的仿真优化中，基于质量守恒定律的混合BO模型显著优于标准BO，混合模型仅需1次迭代即可收敛，而标准BO在25次迭代内均未收敛

Conclusion: 提出的混合BO方案为部分已知系统提供了一种有前景的优化方法，有效结合了机理建模和数据驱动优化的优势
...
### [61] [A Quantitative Characterization of Forgetting in Post-Training](https://arxiv.org/abs/2603.12163)
*Krishnakumar Balasubramanian,Shiva Prasad Kasiviswanathan*

Main category: cs.LG

TL;DR: 论文研究了生成模型持续后训练中的遗忘问题，提出了两种遗忘形式（质量遗忘和旧组件漂移），分析了不同KL目标函数对遗忘的影响，并探讨了重放机制和三种近期方法的遗忘控制条件。


<details>
  <summary>Details</summary>
Motivation: 生成模型的持续后训练被广泛使用，但对于何时以及为何发生遗忘的理解仍然有限。需要从理论上分析遗忘现象，为实际应用提供指导。

Method: 采用Chen等人提出的双模式混合抽象（代表旧任务和新任务），在等协方差高斯模式下进行理论分析，研究前向KL和反向KL目标函数对遗忘的影响，分析重放机制的作用，并评估三种近期方法（SDFT、TTT-Discover、OAPL）。

Result: 前向KL目标函数会导致旧权重归零（质量遗忘），而反向KL目标函数能收敛到真实目标（避免质量遗忘），旧均值漂移受巴氏系数控制的重叠门控错误分配概率影响，漂移随模式分离呈指数衰减。重放机制对两种目标函数有不同的作用方式。三种近期方法在特定条件下能保留旧质量并表现出重叠控制的漂移。

Conclusion: 遗忘可以根据散度方向、几何行为重叠、采样机制以及训练过程中过去行为的可见性之间的相互作用来精确量化。这为理解和控制生成模型持续训练中的遗忘问题提供了理论框架。
...
### [63] [Reference-Guided Machine Unlearning](https://arxiv.org/abs/2603.11210)
*Jonas Mirlach,Sonia Laguna,Julia E. Vogt*

Main category: cs.LG

TL;DR: ReGUn框架利用独立数据集作为参考，通过分布不可区分性实现更好的遗忘-效用权衡
...
Conclusion: 基于分布不可区分性的遗忘方法比依赖性能退化启发式的方法更有效，ReGUn框架通过利用参考数据集提供原则性指导，实现了更好的遗忘效果同时保持模型效用。
...
### [64] [Monitoring and Prediction of Mood in Elderly People during Daily Life Activities](https://arxiv.org/abs/2603.11230)
*Daniel Bautista-Salinas,Joaquín Roca González,Inmaculada Méndez,Oscar Martinez Mozos*

Main category: cs.LG

TL;DR: 开发智能可穿戴系统监测和预测老年人日常活动中的情绪状态，使用腕带记录生理数据结合移动应用进行生态瞬时评估，机器学习分类器仅基于智能腕带数据预测情绪状态。


<details>
  <summary>Details</summary>
Motivation: 监测和预测老年人在日常生活中的情绪状态，为老年人心理健康提供智能监测解决方案，通过可穿戴设备实现连续、非侵入式的情绪状态评估。

Method: 系统由记录生理活动的腕带和用于生态瞬时评估的移动应用组成，使用机器学习训练分类器，仅基于智能腕带数据自动预测不同的情绪状态。

Result: 在情绪准确性方面显示出有希望的结果，在特定检测幸福感和活跃度方面提供了与现有技术水平相当的结果。

Conclusion: 智能可穿戴系统能够有效监测和预测老年人情绪状态，仅使用腕带数据即可实现与现有技术相当的情绪识别性能，为老年人心理健康监测提供了实用解决方案。
...
### [65] [Beyond the Class Subspace: Teacher-Guided Training for Reliable Out-of-Distribution Detection in Single-Domain Models](https://arxiv.org/abs/2603.11269)
*Hong Yang,Devroop Kar,Qi Yu,Travis Desell,Alex Ororbia*

Main category: cs.LG

TL;DR: 本文提出了一种解决单域训练中域敏感性崩溃问题的新方法TGT，通过从多域教师模型中蒸馏被抑制的域偏移信号，显著提升了距离基OOD检测器的性能。


<details>
  <summary>Details</summary>
Motivation: 当前OOD检测方法在多域基准上表现良好，但实际系统通常在单域数据上训练。作者发现这种训练模式会导致域敏感性崩溃（DSC）：监督训练将特征压缩到低秩类子空间，抑制了携带域偏移信号的方向，导致基于距离和logit的OOD评分对域偏移失去敏感性。

Method: 提出了教师引导训练（TGT）方法：在训练过程中，从冻结的多域教师模型（DINOv2）中蒸馏出被类别抑制的残差结构到学生模型中。训练完成后丢弃教师模型和辅助头，不增加推理开销。

Result: 在八个单域基准测试中，TGT显著降低了距离基评分器的远OOD FPR@95：MDS改进11.61个百分点，ViM改进10.78个百分点，kNN改进12.87个百分点（ResNet-50平均值），同时保持或略微提高了域内OOD检测和分类准确率。

Conclusion: TGT方法有效解决了单域训练中的域敏感性崩溃问题，通过从多域教师模型中蒸馏域偏移信号，显著提升了OOD检测性能，且不增加推理成本，具有实际应用价值。
...
Result: 在97种配置（4种异构类型×4个数据集×7个基线方法）的评估中，该方法在所有设置下匹配知道真实聚类分配的Oracle基线，在组合异构性场景下超越Oracle基线1-6%，且在所有测试方法中唯一具有稀疏鲁棒性

Conclusion: 通过本地PCA统计量调节全局模型的方法能有效处理联邦学习中的数据异构性问题，性能优异且通信效率高，特别适合数据稀疏和多维异构场景
...
### [67] [Hindsight-Anchored Policy Optimization: Turning Failure into Feedback in Sparse Reward Settings](https://arxiv.org/abs/2603.11321)
*Yuning Wu,Ke Wang,Devin Chen,Kai Wei*

Main category: cs.LG

TL;DR: HAPO（Hindsight-Anchored Policy Optimization）通过选择性锚定教师演示来解决稀疏奖励环境中RLVR的困境，实现渐近一致性。


<details>
  <summary>Details</summary>
Motivation: 在稀疏奖励设置中，基于群体的方法如GRPO面临两难：纯RL存在优势崩溃和高方差梯度估计问题，而混合策略优化则引入持续性分布偏差。

Method: 提出HAPO方法，采用SSI（Synthetic Success Injection）算子，这是一种选择性在失败时锚定教师演示的后见机制，通过Thompson采样启发的门控机制创建自主、自适应的课程学习。

Result: 理论上证明HAPO实现渐近一致性：随着策略改进自然衰减教师信号，恢复无偏的在线梯度，确保离策略指导作为临时支架而非永久上限。

Conclusion: HAPO解决了RLVR在稀疏奖励环境中的关键困境，使模型能够超越静态教师强制的限制，实现更优的性能。
...
### [68] [Meta-Reinforcement Learning with Self-Reflection for Agentic Search](https://arxiv.org/abs/2603.11327)
...
TL;DR: MR-Search是一种基于上下文元强化学习的智能搜索框架，通过跨回合自我反思优化搜索策略，在测试时实现更有效的探索。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在单个独立回合中优化策略，奖励稀疏且缺乏跨回合学习能力，无法让智能体在测试时通过自我反思改进探索策略。

Method: 提出MR-Search框架：1）训练能够基于历史回合调整搜索策略的上下文元强化学习策略；2）每回合后生成显式自我反思，作为后续尝试的额外上下文；3）引入多回合RL算法，在回合级别估计密集相对优势，实现细粒度信用分配。

Result: 在多个基准测试中，MR-Search相比基于RL的基线方法表现出优势，显示出强大的泛化能力，在八个基准测试中相对改进达到9.2%到19.3%。

Conclusion: MR-Search通过跨回合自我反思和上下文元强化学习，使搜索智能体能够在测试时改进探索策略，为智能搜索系统提供了一种有效的学习框架。
...
### [69] [Jailbreak Scaling Laws for Large Language Models: Polynomial-Exponential Crossover](https://arxiv.org/abs/2603.11331)
*Indranil Halder,Annesya Banerjee,Cengiz Pehlevan*

Main category: cs.LG

TL;DR: 论文研究对抗性提示注入攻击如何使安全对齐的大型语言模型产生不安全行为，发现短提示注入导致攻击成功率呈幂律增长，而长提示注入导致指数增长，并用自旋玻璃理论模型解释这一现象。


<details>
  <summary>Details</summary>
Motivation: 研究对抗性攻击如何可靠地引导安全对齐的大型语言模型产生不安全行为，特别是探索提示注入攻击如何显著提高攻击成功率，并寻求理论解释这一现象。

Method: 提出基于自旋玻璃系统的理论生成模型，将语言生成视为在副本对称破缺机制下运行的Gibbs测量，将不安全行为对应为低能量、尺寸偏置的聚类。分析短提示注入（弱磁场）和长提示注入（强磁场）对攻击成功率的影响。

Result: 短提示注入导致攻击成功率随推理样本数呈幂律增长，长提示注入导致指数增长。这种转变源于强磁场下自旋链中出现有序相，表明注入的越狱提示增强了语言模型中的对抗性有序性。理论分析结果在大型语言模型上得到实证验证。

Conclusion: 对抗性提示注入攻击能够显著放大大型语言模型的不安全行为，短提示和长提示注入分别对应不同的攻击成功率增长模式，这一现象可以通过自旋玻璃理论框架得到解释，揭示了对抗性攻击增强语言模型有序性的机制。
...
### [70] [Teleodynamic Learning a new Paradigm For Interpretable AI](https://arxiv.org/abs/2603.11355)
*Enrique ter Horst,Juan Diego Zambrano*

Main category: cs.LG

TL;DR: 提出Teleodynamic Learning新范式，将学习视为约束下功能组织的涌现与稳定，而非固定目标的最小化。基于Distinction Engine实现，在标准数据集上表现优异且产生可解释逻辑规则。
...
Result: DE11在标准基准测试中取得优异表现：IRIS数据集93.3%测试准确率，WINE数据集92.6%，乳腺癌数据集94.7%。同时产生从学习动态中内生产生的可解释逻辑规则，而非手工设计。
...
### [71] [Multilingual Financial Fraud Detection Using Machine Learning and Transformer Models: A Bangla-English Study](https://arxiv.org/abs/2603.11358)
...
<details>
  <summary>Details</summary>
Motivation: 随着数字金融平台的快速发展，金融欺诈检测成为重要研究挑战。现有研究大多专注于英语数据，而孟加拉语（使用人口超2.5亿）在该领域尚未得到充分探索，限制了多语言环境下的应用。

Method: 研究使用包含合法和欺诈金融消息的数据集，评估了传统机器学习模型（逻辑回归、线性SVM、集成分类器）使用TF-IDF特征，以及基于Transformer的架构。采用5折分层交叉验证进行实验。

Result: 线性SVM表现最佳，达到91.59%准确率和91.30% F1分数，优于Transformer模型（89.49%准确率，88.88% F1）。欺诈消息通常更长，包含紧急诱导词汇、URL（32%）和电话号码（97%），而合法消息则包含交易确认和具体货币参考。

Conclusion: 精心设计的传统机器学习特征在多语言欺诈检测中仍具竞争力，同时凸显了语言多样性、代码混合和低资源语言带来的挑战。研究为孟加拉语金融欺诈检测提供了基准。
...
### [72] [abx_amr_simulator: A simulation environment for antibiotic prescribing policy optimization under antimicrobial resistance](https://arxiv.org/abs/2603.11369)
...
<details>
  <summary>Details</summary>
Motivation: 抗菌素耐药性（AMR）是全球健康威胁，降低抗生素有效性并增加临床决策复杂性，需要工具来模拟和优化抗生素管理策略。

Method: 开发了基于Python的模拟包，采用模块化设计，允许用户配置患者群体、抗生素特异性AMR响应曲线和奖励函数。使用漏气球抽象模型AMR动态，支持通过噪声、偏差和延迟探索部分可观测性，兼容Gymnasium RL API。

Result: 创建了一个可配置的基准环境，用于在不确定性（包括由噪声、偏差和延迟观察引起的部分可观测性）下进行顺序决策，为研究AMR动态和优化抗生素管理策略提供了有价值的工具。

Conclusion: abx_amr_simulator提供了一个可定制和可扩展的框架，用于在现实不确定性下研究AMR动态和优化抗生素管理策略，为强化学习在临床决策中的应用提供了有价值的工具。
...
### [73] [Relaxed Efficient Acquisition of Context and Temporal Features](https://arxiv.org/abs/2603.11370)
...
<details>
  <summary>Details</summary>
Motivation: 生物医学应用中测量成本高昂，纵向主动特征采集面临时序耦合决策挑战，且实际临床工作流中初始上下文特征选择未被充分研究

Method: 提出REACT端到端可微分框架，使用Gumbel-Sigmoid松弛和直通估计优化离散采集掩码，联合优化初始上下文描述符选择和纵向时序特征采集计划

Result: 在真实世界纵向健康和数据集上，REACT以更低采集成本实现更好的预测性能，优于现有纵向采集基线方法

Conclusion: 将初始上下文建模和时序耦合采集统一优化能显著提升纵向主动特征采集的效果，REACT框架为成本约束下的生物医学预测提供了有效解决方案
...
### [74] [Ensuring Safety in Automated Mechanical Ventilation through Offline Reinforcement Learning and Digital Twin Verification](https://arxiv.org/abs/2603.11372)
*Hang Yu,Huidong Liu,Qingchen Zhang,William Joy,Kateryna Nikulina,Andreas A. Schuppert,Sina Saffaran,Declan Bates*

Main category: cs.LG

TL;DR: 提出T-CQL框架，结合Transformer编码器、保守自适应正则化和一致性正则化，用于个性化自动机械通气设置，通过临床知情奖励函数和数字孪生评估，在ICU呼吸衰竭患者中实现更安全有效的通气调整。


<details>
  <summary>Details</summary>
Motivation: 机械通气是ICU急性呼吸衰竭患者的救命干预措施，但不适当的呼吸机设置可能导致呼吸机相关性肺损伤。临床医生的工作负荷与患者预后直接相关，因此需要个性化和自动化的机械通气来改善患者预后。传统监督学习和离线强化学习方法往往忽略时间依赖性，过度依赖基于死亡率的奖励，未能充分捕捉早期生理恶化和肺损伤风险。

Method: 提出Transformer-based Conservative Q-Learning (T-CQL)框架：1) 使用Transformer编码器对患者动态进行有效时间建模；2) 基于不确定性量化的保守自适应正则化确保安全性；3) 一致性正则化实现稳健决策；4) 构建临床知情奖励函数，包含肺损伤指标和患者疾病严重程度评分；5) 使用急性呼吸衰竭患者的交互式数字孪生进行在线"床边"评估。

Result: T-CQL框架在性能上持续优于现有最先进的离线强化学习方法，提供更安全、更有效的通气调整。结果表明，基于Transformer的模型结合保守强化学习策略在重症监护中具有作为决策支持工具的潜力。

Conclusion: T-CQL框架通过整合Transformer时间建模、保守正则化和临床知情奖励，解决了传统方法在机械通气个性化自动化中的局限性。该框架展示了Transformer模型与保守强化学习策略结合在重症监护决策支持中的潜力，能够更好地捕捉早期生理恶化和肺损伤风险，实现更安全的通气管理。
...
### [75] [ARROW: Augmented Replay for RObust World models](https://arxiv.org/abs/2603.11395)
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数持续强化学习方法依赖模型无关方法和回放缓冲区来缓解灾难性遗忘，但这些方法面临内存需求大的可扩展性挑战。受神经科学启发，大脑将经验回放给预测性世界模型而非直接给策略。

Method: ARROW是基于模型的持续强化学习算法，扩展了DreamerV3，采用内存高效的分布匹配回放缓冲区。不同于标准固定大小的FIFO缓冲区，ARROW维护两个互补缓冲区：短期缓冲区用于最近经验，长期缓冲区通过智能采样保持任务多样性。

Result: 在两种挑战性持续RL设置上评估ARROW：无共享结构的任务（Atari）和有共享结构的任务（Procgen CoinRun变体）。相比相同大小回放缓冲区的模型无关和基于模型基线，ARROW在无共享结构任务上表现出显著更少的遗忘，同时保持可比较的前向迁移。

Conclusion: 研究结果突出了基于模型的RL和生物启发方法在持续强化学习中的潜力，值得进一步研究。
...
### [76] [Harnessing Data Asymmetry: Manifold Learning in the Finsler World](https://arxiv.org/abs/2603.11396)
...
<details>
  <summary>Details</summary>
Motivation: 传统流形学习方法依赖对称的黎曼几何，强制使用对称的相异度和嵌入空间（如欧几里得空间），这在实际应用中丢弃了数据样本非均匀性所固有的有价值的非对称信息。

Method: 采用Finsler几何（黎曼几何的非对称推广），提出了一个Finsler流形学习流程，构建非对称相异度并在Finsler空间中嵌入。将现有非对称嵌入器扩展到任意数据，并现代化了非对称嵌入器，推广了当前参考方法如Finsler t-SNE和Finsler Umap。

Result: 在受控合成数据集和大型真实数据集上，非对称流程揭示了传统流程丢失的宝贵信息（如密度层次结构），并且始终提供比其欧几里得对应方法更高质量的嵌入。

Conclusion: 通过转向Finsler几何，可以充分利用数据中的非对称信息，显著扩展了流形学习的应用范围，为非对称数据嵌入提供了更强大的框架。
...
### [77] [ZTab: Domain-based Zero-shot Annotation for Table Columns](https://arxiv.org/abs/2603.11436)
...
Method: ZTab采用基于领域的方法：给定包含预定义语义类型和示例表模式的领域配置，为示例模式生成伪表，并在这些伪表上微调注释LLM。该方法不需要用户特定的标记训练数据，对相似领域的测试表无需重新训练。

Result: ZTab在零样本性能和标注性能之间提供权衡：包含所有语义类型的"通用领域"接近"纯粹"零样本，而包含特定应用语义类型的"专业领域"能在该领域内实现更好的零样本性能。

Conclusion: ZTab是一个有效的基于领域零样本框架，解决了现有零样本模型的关键限制，在零样本要求和性能之间提供了可配置的平衡，源代码和数据集已公开。
...
### [78] [Bridging Discrete Marks and Continuous Dynamics: Dual-Path Cross-Interaction for Marked Temporal Point Processes](https://arxiv.org/abs/2603.11462)
...
Conclusion: NEXTPP通过统一离散和连续表示，有效解决了预测不规则间隔事件序列的挑战，在捕获复杂异步依赖方面表现出色。
...
### [79] [Leveraging Phytolith Research using Artificial Intelligence](https://arxiv.org/abs/2603.11476)
...
TL;DR: 开发了Sorometry——一个端到端的人工智能流程，用于高通量数字化、推断和解释植硅体，结合2D图像和3D点云分析，显著提高分类准确性和分析效率。


<details>
  <summary>Details</summary>
Motivation: 传统植硅体分析方法依赖劳动密集型、耗时的手动显微镜观察，严重限制了研究效率。需要开发自动化、高通量的解决方案来克服这一瓶颈。

Method: 开发了Sorometry流程：处理Z-stacked光学显微镜扫描，自动生成同步的2D正射图像和3D点云；构建多模态融合模型，结合ConvNeXt进行2D图像分析和PointNet++进行3D点云分析；包含图形用户界面供专家标注和审查；采用贝叶斯有限混合模型预测整体植物来源贡献。

Result: 融合模型在24种诊断形态类型上实现77.9%的全局分类准确率，分割质量达84.5%；3D数据整合对区分复杂形态类型（如草硅短细胞植硅体）至关重要；成功识别复杂混合样品中的玉米和棕榈等特定植物。

Conclusion: Sorometry将植硅体研究转变为"组学"规模的学科，显著扩展分析能力，标准化专家判断，实现考古和古生态组合的可重复、群体级表征。
...
### [80] [Attention Sinks Are Provably Necessary in Softmax Transformers: Evidence from Trigger-Conditional Tasks](https://arxiv.org/abs/2603.11487)
*Yuval Ran-Milo*

Main category: cs.LG

TL;DR: 论文证明softmax自注意力模型在计算简单触发条件行为时必然产生注意力汇聚现象，而ReLU注意力无需汇聚即可解决相同任务，表明归一化约束是汇聚行为的根本原因。


<details>
  <summary>Details</summary>
Motivation: 研究Transformer中观察到的注意力汇聚现象（注意力概率集中在固定的、内容无关的位置），试图从理论上解释这一现象的根本原因。

Method: 通过理论证明分析softmax自注意力模型在特定任务下的行为：当指定触发标记出现时，模型必须返回所有先前标记表示的平均值，否则输出零。同时分析非归一化的ReLU注意力模型在相同任务下的表现。

Result: 理论证明softmax模型必然产生注意力汇聚，而ReLU注意力可以解决相同任务而不产生汇聚。实验验证了理论预测，并表明这一现象在单头和多头注意力中都存在。

Conclusion: 注意力汇聚现象的根本原因是softmax的归一化约束，而不是注意力机制本身。这一发现有助于理解Transformer中注意力头的功能实现机制。
...
### [81] [KEPo: Knowledge Evolution Poison on Graph-based Retrieval-Augmented Generation](https://arxiv.org/abs/2603.11501)
*Qizhi Chen,Chao Qi,Yihong Huang,Muquan Li,Rongzheng Wang,Dongyang Zhang,Ke Qin,Shuang Liang*

Main category: cs.LG

TL;DR: 本文提出了一种针对GraphRAG的新型投毒攻击方法KEPo，通过构造有毒事件和伪造知识演化路径来操纵知识图谱，使LLM产生有害目标响应。


<details>
  <summary>Details</summary>
Motivation: GraphRAG通过从外部数据库构建知识图谱来增强LLM的时效性和准确性，但这种对外部数据的依赖引入了新的攻击面。现有攻击方法主要针对传统RAG系统，对GraphRAG无效，因为其知识图谱抽象机制能够重组投毒文本。需要揭示GraphRAG潜在的安全漏洞。

Method: 提出知识演化投毒(KEPo)方法：1) 针对目标查询生成包含投毒知识的有毒事件；2) 伪造事件背景和从原始事实到有毒事件的知识演化路径；3) 投毒知识图谱，误导LLM将投毒知识视为最终结果。在多目标攻击场景中，连接多个攻击语料库，使投毒知识相互增强并扩大投毒社区规模。

Result: 在多个数据集上的实验结果表明，KEPo在单目标和多目标攻击中都实现了最先进的攻击成功率，显著优于先前的方法。

Conclusion: KEPo成功暴露了GraphRAG的潜在安全漏洞，表明知识图谱抽象虽然增强了传统RAG的鲁棒性，但仍存在被针对性攻击的风险。这为GraphRAG系统的安全防御提供了重要启示。
...
### [82] [Sharpness-Aware Minimization for Generalized Embedding Learning in Federated Recommendation](https://arxiv.org/abs/2603.11503)
*Fengyuan Yu,Xiaohua Feng,Yuyuan Li,Changwang Zhang,Jun Wang,Chaochao Chen*

Main category: cs.LG

TL;DR: FedRecGEL提出了一种新的联邦推荐框架，通过从物品中心视角重构问题为多任务学习，使用锐度感知最小化来稳定学习广义物品嵌入，显著提升联邦推荐性能。
...
Method: 提出FedRecGEL框架，从物品中心视角重构联邦推荐问题为多任务学习问题，旨在在整个训练过程中学习广义嵌入。基于理论分析，采用锐度感知最小化来解决泛化问题，稳定训练过程。
...
### [83] [LongFlow: Efficient KV Cache Compression for Reasoning M](https://arxiv.org/abs/2603.11504)
*Yi Su,Zhenxu Tian,Dan Qiao,Yuechi Zhou,Juntao Li,Min Zhang*

Main category: cs.LG

TL;DR: LongFlow：一种针对推理模型长输出场景的KV缓存压缩方法，通过利用注意力计算的中间结果进行高效重要性评估，实现高达80%的KV缓存压缩和11.8倍的吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 当前推理模型（如OpenAI-o1和DeepSeek-R1）在复杂任务上表现出色，但生成长输出序列导致部署成本显著增加。长输出需要大的KV缓存，造成高内存消耗和注意力计算时的严重带宽压力。现有KV缓存优化方法主要针对长输入短输出场景，对推理模型的长输出设置效果不佳，且重要性评估计算成本高，在长生成过程中连续重新评估变得不可行。

Method: 提出LongFlow方法：1）使用注意力计算的中间结果（仅使用当前查询）推导出高效的重要性评估指标，引入可忽略的计算开销且无需辅助存储；2）开发定制内核，将FlashAttention、重要性评估和令牌淘汰融合为单个优化算子，提高系统级效率。

Result: 实验表明，LongFlow在实现80% KV缓存压缩的同时，对模型精度影响最小，并达到高达11.8倍的吞吐量提升。
...
### [84] [CFD-HAR: User-controllable Privacy through Conditional Feature Disentanglement](https://arxiv.org/abs/2603.11526)
*Alex Gn,Fan Li,S Kuniyilh,Ada Axan*

Main category: cs.LG

TL;DR: 该研究提出了一种基于特征解缠的用户可控隐私保护方法，用于可穿戴设备上的人体活动识别，并与基于自编码器的少样本学习方法进行对比分析。


<details>
  <summary>Details</summary>
Motivation: 可穿戴设备上的传感器数据包含敏感用户信息，现有的人体活动识别系统面临两大挑战：保护用户隐私（根据用户偏好）和在有限标注样本下保持高识别性能。

Method: 提出基于特征解缠的表示学习方法，实现细粒度的动态隐私过滤；并与基于自编码器的少样本学习方法进行对比分析，评估两者的架构设计、学习目标、隐私保证、数据效率和边缘物联网部署适用性。

Result: CFD（基于特征解缠）方法通过潜在空间分离活动和敏感属性，提供明确、可调的隐私保护控制；而自编码器少样本方法在标签效率和轻量级适应性方面表现更优，但缺乏内在隐私保护机制。两种方法在持续物联网设置中的安全性分析显示，它们在表示泄露和嵌入级攻击方面存在差异。

Conclusion: 单独使用任一种范式都无法完全满足下一代物联网HAR系统的需求，需要开发统一框架，共同优化隐私保护、少样本适应性和鲁棒性，以实现可信的物联网智能。
...
### [85] [Multi-Task Anti-Causal Learning for Reconstructing Urban Events from Residents' Reports](https://arxiv.org/abs/2603.11546)
*Liangkai Zhou,Susu Xu,Shuqi Zhong,Shan Lin*

Main category: cs.LG

TL;DR: MTAC是一个多任务反因果学习框架，通过利用跨任务不变性从观测结果和混杂因素中推断潜在原因，在城市事件重建应用中显著提升了重建准确性。


<details>
  <summary>Details</summary>
Motivation: 现实中的许多机器学习任务是反因果的：需要从观测结果推断潜在原因。实践中常面临多个相关任务，其中部分前向因果机制在任务间保持不变，而其他组件是任务特定的。需要利用这种跨任务不变性来改进原因推断。
...
Result: 在曼哈顿和纽瓦克市收集的真实世界数据上，MTAC在城市事件重建的三个任务（停车违规、废弃房产和不卫生条件）中始终优于强基线，实现了高达34.61%的MAE减少，证明了学习跨任务可转移因果机制的好处。

Conclusion: MTAC通过明确利用跨任务不变性，在多任务反因果学习中有效提升了原因推断的准确性，为从观测结果和混杂因素中重建潜在原因提供了有前景的框架。
...
### [86] [CAETC: Causal Autoencoding and Treatment Conditioning for Counterfactual Estimation over Time](https://arxiv.org/abs/2603.11565)
*Nghia D. Nguyen,Pablo Robles-Granda,Lav R. Varshney*

Main category: cs.LG

TL;DR: CAETC是一种基于对抗表示学习的新方法，通过自编码架构学习部分可逆且治疗不变的表示，将结果预测任务转化为对表示进行特定治疗条件化，用于时间依赖的因果反事实估计。


<details>
  <summary>Details</summary>
Motivation: 时间依赖的混杂偏倚在观测数据中仍然是一个重大挑战，影响个性化医疗等应用中的准确反事实估计。

Method: CAETC方法基于对抗表示学习，采用自编码架构学习部分可逆且治疗不变的表示，将结果预测任务转化为对表示进行特定治疗条件化，不依赖于底层序列模型，可应用于LSTM或TCN等现有架构。

Result: 在合成、半合成和真实世界数据上进行广泛实验，证明CAETC在反事实估计方面相比现有方法有显著改进。

Conclusion: CAETC是一种有效的时间依赖反事实估计方法，能够处理观测数据中的时间依赖混杂偏倚问题。
...
### [87] [Survival Meets Classification: A Novel Framework for Early Risk Prediction Models of Chronic Diseases](https://arxiv.org/abs/2603.11598)
*Shaheer Ahmad Khan,Muhammad Usamah Shahid,Muddassar Farooq*

Main category: cs.LG

TL;DR: 该研究提出了一种将生存分析与分类技术相结合的新方法，用于五种常见慢性疾病的早期风险预测，在真实世界EMR数据上表现优于或媲美现有最佳模型。


<details>
  <summary>Details</summary>
Motivation: 慢性疾病需要终身医疗关注，传统风险预测模型通常独立使用生存分析或分类技术，缺乏综合方法。本研究旨在整合这两种技术，开发更全面的疾病风险监测模型。

Method: 提出了一种新颖的方法，将生存分析重新设计以使其能够高效有效地执行分类任务。该方法将生存分析模型改造为可用于疾病风险分类的综合工具。

Result: 在真实世界大型EMR数据上的实验结果显示，生存模型在准确性、F1分数和AUROC方面的性能与LightGBM和XGBoost等现有最佳模型相当或更好。

Conclusion: 生存分析方法可以成功改造为分类工具，为慢性疾病风险预测提供综合解决方案。该方法还提供了经过临床验证的新型解释机制，增强了模型的可解释性和临床实用性。
...
### [88] [Hybrid Energy-Aware Reward Shaping: A Unified Lightweight Physics-Guided Methodology for Policy Optimization](https://arxiv.org/abs/2603.11600)
*Qijun Liao,Jue Yang,Yiting Kang,Xinxin Zhao,Yong Zhang,Mingan Zhao*

Main category: cs.LG

TL;DR: 提出H-EARS方法，将基于势能的奖励塑形与能量感知动作正则化相结合，在保持线性复杂度下提升强化学习的收敛性、稳定性和能量效率。


<details>
  <summary>Details</summary>
Motivation: 深度强化学习在连续控制中表现优异但需要大量探索，而基于物理的模型需要完整方程且具有立方复杂度。需要一种方法能结合两者的优势，利用轻量级物理先验增强无模型强化学习。

Method: 提出混合能量感知奖励塑形(H-EARS)，统一基于势能的奖励塑形与能量感知动作正则化。通过功能分解平衡任务特定势能和能量势能，仅捕获主导能量分量实现O(n)线性复杂度，无需完整动力学模型。

Result: 实验显示改进的收敛性、稳定性和能量效率。车辆仿真验证了在极端条件下安全关键领域的适用性。结果证实轻量级物理先验能增强无模型强化学习，无需完整系统模型。

Conclusion: H-EARS成功整合轻量级物理先验，在保持线性复杂度的同时提升强化学习性能，为从实验室研究到工业应用提供了可行的技术路径。
...
### [89] [AutoScout: Structured Optimization for Automating ML System Configuration](https://arxiv.org/abs/2603.11603)
*Jimmy Shong,Yuhan Ding,Yihan Jiang,Liheng Jing,Haonan Chen,Gaokai Zhang,Aditya Akella,Fan Lai*

Main category: cs.LG

TL;DR: AutoScout是一个用于机器学习训练、微调和推理的通用系统配置器，通过混合优化框架解决ML系统配置空间的复杂性问题，实现2.7-3.0倍训练加速。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统配置空间庞大且复杂，包含模型并行策略、通信优化和运行时参数等多种配置维度。端到端系统效率对这些选择高度敏感，但由于异构特征类型、条件依赖性和高搜索成本，识别高性能配置具有挑战性。现有方法要么优化有限的配置维度，要么依赖无法适应不断增长配置空间的启发式方法。
...
Conclusion: AutoScout提供了一个通用的系统配置器，能够有效解决ML系统配置的复杂性，通过混合优化方法显著提升训练效率，适应不断增长的配置空间需求。
...
### [90] [Fractional Rotation, Full Potential? Investigating Performance and Convergence of Partial RoPE](https://arxiv.org/abs/2603.11611)
*Mohammad Aflah Khan,Krishna P. Gummadi,Manish Gupta,Abhilasha Ravichander*

Main category: cs.LG

TL;DR: 部分RoPE（仅对约10%隐藏维度应用旋转位置编码）能在保持收敛性能的同时显著减少内存占用，最高可节省10倍缓存内存。


<details>
  <summary>Details</summary>
Motivation: 研究旋转位置编码（RoPE）在不同隐藏维度比例下的效果，探索通过部分应用RoPE来平衡内存效率与模型性能的设计选择。

Method: 系统研究部分RoPE对训练动态和收敛的影响，在不同架构、数据集和模型规模上进行实验，分析不同RoPE比例（包括NoPE）的效果。

Result: 1. 仅对约10%维度应用RoPE即可达到与完整RoPE相当的收敛效果；2. 这一趋势在不同模型规模、序列长度、数据集质量和架构中保持一致；3. NoPE模型训练不稳定，但可通过最小RoPE或QK-Norm缓解。

Conclusion: 部分RoPE为模型设计者提供了平衡效率与训练稳定性的实用指导，强调了之前被忽视的部分RoPE的重要性，特别是在长上下文场景中能显著节省内存。
...
### [91] [Personalized Federated Learning via Gaussian Generative Modeling](https://arxiv.org/abs/2603.11620)
*Peng Hu,Jianwei Ma*

Main category: cs.LG

TL;DR: pFedGM是一种基于高斯生成建模的个性化联邦学习方法，通过建模客户端异构性并采用双重目标（共享目标和本地目标）来平衡全局协作与个性化。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法主要关注分类器头引导的个性化，忽略了表示分布中潜在的个性化特征。为了解决数据异构性问题并更好地建模客户端间的表示分布差异。

Method: 基于高斯生成建模，首先训练高斯生成器通过加权重采样建模客户端异构性。采用双重目标：共享目标最大化客户端间的类间距离，本地目标最小化客户端内的类内距离。将传统高斯分类器解耦为全局优化的导航器和捕获分布统计量的统计提取器。受卡尔曼增益启发，采用全局和本地双重尺度融合框架，将全局表示分布建模为先验，客户端特定数据建模为似然，进行贝叶斯推理估计类别概率。

Result: 在多种场景下进行评估：类数量异构性、环境损坏以及多个基准数据集和配置。pFedGM相比最先进方法实现了优越或具有竞争力的性能。

Conclusion: pFedGM通过高斯生成建模有效解决了联邦学习中的数据异构性问题，平衡了全局协作与个性化需求，在多种场景下表现出色。
...
TL;DR: 研究发现，对于大型预训练视觉语言动作模型，简单的顺序微调配合低秩适配器在持续强化学习中表现优异，挑战了传统持续学习中灾难性遗忘的认知


<details>
  <summary>Details</summary>
Motivation: 探索持续强化学习在视觉语言动作模型中的应用，研究如何让具身智能体在开放、演化的环境中自我改进和适应

Method: 采用简单的顺序微调配合低秩适配器，在三个模型和五个具有挑战性的终身强化学习基准上进行系统研究

Result: 发现简单顺序微调表现出色：具有高可塑性、几乎没有遗忘、保持强大的零样本泛化能力，经常超越更复杂的持续学习方法

Conclusion: 顺序微调是视觉语言动作模型持续强化学习的强大方法，大型预训练模型、参数高效适配和在线强化学习的协同作用重塑了稳定性-可塑性权衡
...
TL;DR: 本文提出了一种神经调节约束自编码器（NcAE），通过整合神经调节机制，实现了上下文依赖的流形学习，能够适应变化的物理参数和环境条件。


<details>
  <summary>Details</summary>
Motivation: 传统的约束自编码器（cAE）虽然通过强制潜在空间几何结构实现了可解释的降维，但无法适应变化的物理参数或环境条件，容易将这些上下文变化与主要输入混淆。

Method: 将神经调节机制整合到cAE框架中，提出神经调节约束自编码器（NcAE）。该模型通过基于静态上下文信息的增益和偏置调节，自适应地参数化几何约束。

Result: 在动力系统上的实验结果表明，NcAE能够准确捕捉不同状态下流形几何的变化，同时保持严格的投影特性，有效解耦全局上下文参数与局部流形表示。

Conclusion: NcAE为在受（非平稳）环境约束的系统中开发更灵活、物理信息丰富的表示提供了基础，神经调节机制能够有效分离全局上下文参数与局部流形表示。
...
<details>
  <summary>Details</summary>
Motivation: 策略梯度算法在语言模型推理中取得了重要进展，但许多算法在训练过程中会自然减少熵，从而降低探索轨迹的多样性，限制了策略的探索能力。作者认为需要主动监控和控制训练过程中的熵。

Method: 1. 形式化分析主要策略梯度目标对熵动态的贡献；2. 识别影响熵行为的经验因素（如数值精度）；3. 提出明确的熵控制机制，包括REPO（修改优势函数来调节熵）和ADAPO（自适应非对称裁剪方法）。

Result: 使用熵保持方法训练的模型在整个训练过程中保持多样性，产生的最终策略性能更好，并且在新环境中进行顺序学习时保持可训练性。

Conclusion: 熵应该在训练过程中被主动监控和控制，提出的REPO和ADAPO方法能够有效保持策略梯度训练中的多样性，提高最终性能并保持可训练性。
...
Result: 在UNIREF和OAS的多样化蛋白质群落上进行计算评估，EvoFlows在捕获蛋白质序列分布方面与主流掩码语言模型相当，同时在从给定模板生成非平凡但自然的突变体方面表现更优。

Conclusion: EvoFlows为蛋白质工程提供了一种独特的方法，能够同时建模相关自然蛋白质的分布以及连接它们的突变路径，在生成自然样突变体方面具有优势。
...
TL;DR: 本文研究了机器学习模型在信用风险评估中的校准与预测多重性之间的相互作用，发现少数类样本承担了不成比例的多重性负担，而事后校准方法（特别是Platt Scaling和Isotonic Regression）能有效降低预测多重性。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习模型在关键决策环境中的部署增加，确保概率可靠性和预测稳定性变得至关重要。本文旨在研究分类校准与预测多重性（即Rashomon集合中多个近最优模型对同一申请人产生冲突信用结果的现象）之间的相互作用。

Method: 使用九个不同的信用风险基准数据集，研究预测多重性是否集中在低预测置信度区域，以及事后校准如何减轻算法任意性。实证比较了三种事后校准方法：Platt Scaling、Isotonic Regression和Temperature Scaling。
...
Conclusion: 校准可以作为共识强制层，通过减轻预测多重性来支持程序公平性。研究结果表明，事后校准方法特别是Platt Scaling和Isotonic Regression，能够有效降低算法决策中的任意性。
...
### [97] [Implicit Statistical Inference in Transformers: Approximating Likelihood-Ratio Tests In-Context](https://arxiv.org/abs/2603.10573)
*Faris Chaudhry,Siddhant Gadkari*

Main category: cs.LG

TL;DR: 本文研究了Transformer的上下文学习机制，通过统计决策理论视角分析二元假设检验，发现模型能够近似贝叶斯最优充分统计量，而非简单的核平滑启发式方法。


<details>
  <summary>Details</summary>
Motivation: 尽管上下文学习允许Transformer无需权重更新就能适应新任务，但其底层算法机制仍不明确。本文旨在从统计决策理论角度理解Transformer在上下文学习中的工作原理。

Method: 采用统计决策理论框架，研究简单的二元假设检验问题，其中最优策略由似然比检验决定。训练Transformer处理两种不同几何结构的任务：线性均值偏移和非线性方差估计。通过logit lens和电路对齐进行机制分析。

Result: Transformer能够从上下文中近似贝叶斯最优充分统计量（达到单调变换程度），在非线性区域匹配理想oracle估计器的性能。模型不依赖固定的核平滑启发式方法，而是自适应地调整决策点：线性任务表现出投票式集成模式，非线性任务则使用更深层的顺序计算。

Conclusion: 上下文学习源于构建任务自适应的统计估计器，而非简单的相似性匹配。这为机制可解释性提供了数学严谨的框架，其中目标算法ground truth是已知的。
...
### [98] [Disentangled Representation Learning through Unsupervised Symmetry Group Discovery](https://arxiv.org/abs/2603.11790)
*Dang-Nhu Barthélémy,Annabi Louis,Argentieri Sylvain*

Main category: cs.LG

TL;DR: 提出了一种无需先验知识、通过智能体与环境交互自主发现对称群结构的无监督学习方法，实现了线性对称解耦表示学习


<details>
  <summary>Details</summary>
Motivation: 现有的基于对称性的解耦表示学习方法需要强先验知识或对子群性质的限制性假设，这限制了方法的通用性和实用性
...
Conclusion: 该方法在最小假设下证明了真实对称群分解的可识别性，实现了无需先验知识的对称性解耦表示学习，具有更好的通用性
...
### [99] [Exponential-Family Membership Inference: From LiRA and RMIA to BaVarIA](https://arxiv.org/abs/2603.11799)
*Rickard Brännvall*

Main category: cs.LG

TL;DR: 论文统一了三种成员推理攻击方法（LiRA、RMIA、BASE），提出指数族对数似然比框架，并基于此开发了BaVarIA攻击方法，在低影子模型预算下表现更优。


<details>
  <summary>Details</summary>
Motivation: 当前存在多种成员推理攻击方法（LiRA、RMIA、BASE），它们看似使用不同的评分策略，且BASE被证明与RMIA等价，这使得实践者难以选择合适的方法。需要统一这些方法并解决小影子模型预算下的性能瓶颈问题。

Method: 提出指数族对数似然比统一框架，将LiRA、RMIA、BASE视为该框架的不同实例，仅区别在于分布假设和每个数据点的参数估计数量。基于此框架，识别方差估计为小影子模型预算下的关键瓶颈，提出BaVarIA攻击方法，使用共轭正态-逆伽马先验替代基于阈值的参数切换，产生Student-t预测或方差稳定的高斯分布。

Result: 在12个数据集和7种影子模型预算下，BaVarIA匹配或优于LiRA和RMIA，在实践重要的低影子模型和离线场景中获得最大性能提升。BaVarIA-t和BaVarIA-n提供稳定性能而无需额外超参数调优。

Conclusion: 成功统一了三种主要成员推理攻击方法，揭示了它们之间的层次关系，并通过BaVarIA解决了小影子模型预算下的方差估计瓶颈问题，为隐私审计提供了更稳定有效的工具。
...
### [100] [On the Role of Reversible Instance Normalization](https://arxiv.org/abs/2603.11869)
*Gaspard Berthelier,Tahar Nabil,Etienne Le Naour,Richard Niamke,Samir Perlaza,Giovanni Neglia*

Main category: cs.LG

TL;DR: 本文重新审视时间序列预测中的归一化方法，发现Reversible Instance Normalization (RevIN)的多个组件是冗余甚至有害的，并提出了改进方向。


<details>
  <summary>Details</summary>
Motivation: 数据归一化在深度学习中至关重要，但在时间序列预测中的作用尚未被充分理解。本文旨在解决时间序列预测中归一化面临的三个核心挑战：时间输入分布偏移、空间输入分布偏移和条件输出分布偏移。

Method: 通过消融研究重新审视广泛使用的Reversible Instance Normalization (RevIN)，分析其各个组件的有效性，识别冗余和有害的部分。

Result: 研究发现RevIN的多个组件是冗余的甚至对性能有负面影响，这为改进RevIN的鲁棒性和泛化能力提供了新的视角。

Conclusion: 基于对RevIN组件的深入分析，本文提出了改进时间序列预测中归一化方法的新思路，旨在增强模型的鲁棒性和泛化能力。
...
### [101] [FlexRec: Adapting LLM-based Recommenders for Flexible Needs via Reinforcement Learning](https://arxiv.org/abs/2603.11901)
*Yijun Pan,Weikang Qiu,Qiyao Ma,Mingxuan Ju,Tong Zhao,Neil Shah,Rex Ying*

Main category: cs.LG

TL;DR: FlexRec是一个基于强化学习的后训练框架，通过因果基础的项目级奖励和不确定性感知的奖励缩放，解决推荐系统中序列级奖励粗粒度分配和稀疏反馈的问题，显著提升多样化推荐场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 现代推荐系统需要适应动态、需求特定的多样化推荐场景，但传统推荐器通常针对单一静态目标优化，难以按需重新配置行为。大型语言模型通过强化学习后训练展现出强大的指令跟随和推理能力，为对齐复杂推荐目标提供了原则性途径。

Method: 提出FlexRec框架，包含两个关键技术：(1) 基于剩余候选池中反事实交换的因果基础项目级奖励，解决序列级奖励的粗粒度信用分配问题；(2) 评论家引导的不确定性感知缩放，显式建模奖励不确定性并降低低置信度奖励的权重，以在稀疏监督下稳定学习。

Result: 在多样化推荐场景和目标下，FlexRec取得显著提升：在需求特定排序中，NDCG@5提升高达59%，Recall@5提升高达109.4%；在泛化设置下，Recall@5进一步提升24.1%，优于传统推荐器和基于LLM的基线方法。

Conclusion: FlexRec通过解决强化学习在推荐系统中的两个关键障碍——粗粒度奖励分配和稀疏反馈问题，成功将LLM对齐到复杂推荐目标，为动态、需求特定的推荐系统提供了有效的后训练解决方案。
...
### [102] [Causal Representation Learning with Optimal Compression under Complex Treatments](https://arxiv.org/abs/2603.11907)
*Wanting Liang,Haoang Chi,Zhiheng Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种新的多治疗个体治疗效果估计方法，解决了平衡权重超参数选择和计算可扩展性两大挑战，通过理论推导最优平衡权重并提出了三种平衡策略，最终扩展为生成式架构Multi-Treatment CausalEGM。


<details>
  <summary>Details</summary>
Motivation: 多治疗场景下的个体治疗效果估计面临两大关键挑战：平衡权重的超参数选择困境和计算可扩展性的维度诅咒问题。现有方法需要昂贵的启发式调参，且在高维治疗空间中计算复杂度高。

Method: 1. 推导了新的多治疗泛化界，提出了理论上的最优平衡权重α估计器，无需启发式调参；2. 研究了三种平衡策略：Pairwise、One-vs-All和Treatment Aggregation；3. 将框架扩展到生成式架构Multi-Treatment CausalEGM，保持了治疗流形的Wasserstein测地线结构。

Result: 在低维设置下，One-vs-All策略达到最优精度；而提出的Treatment Aggregation策略在治疗空间扩展时既能保持准确性，又能实现O(1)的可扩展性。在半合成和图像数据集上的实验表明，该方法在估计精度和效率上显著优于传统模型，特别是在大规模干预场景中。

Conclusion: 本文提出的多治疗个体治疗效果估计框架通过理论推导最优平衡权重，解决了超参数选择问题，并通过Treatment Aggregation策略实现了计算可扩展性。Multi-Treatment CausalEGM架构进一步提升了性能，为大规模干预场景提供了有效的解决方案。
...
### [103] [Chem4DLLM: 4D Multimodal LLMs for Chemical Dynamics Understanding](https://arxiv.org/abs/2603.11924)
...
<details>
  <summary>Details</summary>
Motivation: 现有化学理解任务主要依赖静态分子表示，无法建模化学反应的动态现象（如键断裂、构象变化），这些动态过程对化学家理解反应机理至关重要。

Method: 提出ChemDU任务框架，构建Chem4DBench数据集（配对4D分子轨迹与专家解释），开发Chem4DLLM模型（整合等变图编码器捕捉分子几何与旋转动力学，结合预训练大语言模型生成解释）。

Result: 建立了首个将4D分子轨迹与自然语言解释配对的数据集，提出了统一的模型架构，能够从动态分子轨迹中生成机理基础的解释性叙述。

Conclusion: ChemDU任务、Chem4DBench数据集和Chem4DLLM模型为动态化学理解和多模态科学推理研究开辟了新方向，有望推动化学动力学理解的发展。
...
### [104] [MobileKernelBench: Can LLMs Write Efficient Kernels for Mobile Devices?](https://arxiv.org/abs/2603.11935)
...
TL;DR: 该研究探索LLM为移动设备生成高效内核的能力，发现现有模型因工程复杂性和数据稀缺而表现不佳，提出了多智能体系统MoKA显著提升编译成功率和性能优化。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在代码生成方面表现出色，但其为移动设备生成内核的潜力尚未充分探索。移动设备内核生成面临独特的工程复杂性和数据稀缺挑战，需要专门的研究框架来评估和提升LLM在这方面的能力。

Method: 1) 引入MobileKernelBench评估框架，包含注重算子多样性和跨框架互操作性的基准测试，以及连接主机-设备差距的自动化流水线；2) 在MNN的CPU后端进行广泛评估；3) 提出Mobile Kernel Agent (MoKA)多智能体系统，具备仓库感知推理和计划-执行范式。

Result: 评估发现当前LLM在移动框架中表现不佳：标准模型和微调变体的编译失败率超过54%，性能提升微乎其微。相比之下，MoKA在MobileKernelBench上达到最先进性能：编译成功率提升至93.7%，27.4%的生成内核能提供可测量的加速效果。

Conclusion: LLM为移动设备生成高效内核面临显著挑战，但通过专门设计的多智能体系统如MoKA，可以显著提升编译成功率和性能优化能力，为移动设备内核自动化生成提供了有前景的解决方案。
...
### [105] [Exhaustive Circuit Mapping of a Single-Cell Foundation Model Reveals Massive Redundancy, Heavy-Tailed Hub Architecture, and Layer-Dependent Differentiation Control](https://arxiv.org/abs/2603.11940)
*Ihor Kendiukhov*

Main category: cs.LG

TL;DR: 本文通过三种实验方法改进了生物基础模型的可解释性研究：穷举电路追踪、高阶组合消融和因果轨迹引导，揭示了选择性特征采样带来的系统性偏差，证实了模型架构的次可加性，并建立了层依赖的细胞状态控制因果证据。


<details>
  <summary>Details</summary>
Motivation: 现有生物基础模型的可解释性研究依赖于选择性特征采样、成对交互测试和观察性轨迹分析，这些方法可能引入系统性偏差。本文旨在通过更全面的方法来克服这些局限性。

Method: 使用三种实验方法：1) 对Geneformer模型第5层所有4065个活跃稀疏自编码器特征进行穷举电路追踪；2) 对8个特征三元组进行三阶组合消融；3) 通过轨迹引导的特征引导建立因果联系。

Result: 1) 穷举追踪发现1393850条显著下游边，比选择性采样扩展27倍，揭示了重尾分布，其中1.8%的特征占主导连接性，40%的top20枢纽缺乏生物注释；2) 三阶组合消融显示冗余度随交互阶数单调加深，三阶比率为0.59，成对比率为0.74，无协同效应；3) 轨迹引导显示第17层晚期特征一致推动细胞状态向成熟发展，而第0层和第11层的早中期特征主要推动远离成熟。

Conclusion: 这些结果从相关性转向因果证据，表明细胞状态控制具有层依赖性，揭示了先前选择性分析中的系统性注释偏差，并确认了模型架构在所有测试阶数上的次可加性。
...
### [106] [Causal Matrix Completion under Multiple Treatments via Mixed Synthetic Nearest Neighbors](https://arxiv.org/abs/2603.11942)
*Minrui Luo,Zhiheng Zhang*

Main category: cs.LG

TL;DR: MSNN方法扩展了SNN，通过整合跨治疗水平的信息来解决因果矩阵补全问题，特别适用于数据稀缺的治疗水平场景。


<details>
  <summary>Details</summary>
Motivation: SNN方法在因果矩阵补全中有效，但需要每个治疗水平都有足够数据，这在多治疗或复杂治疗场景中往往不成立，限制了其应用。

Method: 提出混合合成最近邻（MSNN）方法，这是一种新的逐项因果识别估计器，通过整合跨治疗水平的信息来扩大有效样本量。

Result: MSNN保持了SNN的有限样本误差界和渐近正态性保证，同时显著扩大了可用于估计的有效样本量，在合成和真实数据集上表现出色。

Conclusion: MSNN是SNN的有效扩展，特别适用于治疗水平数据稀缺的场景，通过整合跨治疗信息提高了因果矩阵补全的准确性和鲁棒性。
...
TL;DR: 该论文提出使用Voronoi剖分将几何结构融入概率电路，解决了传统概率电路中混合权重数据独立的问题，并开发了两种解决方案：近似推理框架和精确可处理推理的结构条件。


<details>
  <summary>Details</summary>
Motivation: 传统概率电路使用数据独立的混合权重，限制了其捕捉数据流形局部几何结构的能力。需要一种方法将几何结构直接融入概率电路，同时保持推理的可处理性。

Method: 提出使用Voronoi剖分作为将几何结构融入概率电路和节点的自然方式。开发了两种解决方案：1）提供推理保证上下界的近似推理框架；2）确保精确可处理推理的Voronoi剖分结构条件。还引入了可微分的Voronoi剖分松弛，支持基于梯度的学习。

Result: 提出的方法在标准密度估计任务上进行了实证验证，展示了将几何结构融入概率电路的有效性，同时保持了推理的可处理性。

Conclusion: Voronoi剖分是增强概率电路几何建模能力的有效方法，通过提出的近似推理框架和结构条件，可以在保持推理可处理性的同时捕捉数据流形的局部几何结构。
...
### [108] [Statistical and structural identifiability in representation learning](https://arxiv.org/abs/2603.11970)
*Walter Nelson,Marco Fumero,Theofanis Karaletsos,Francesco Locatello*

Main category: cs.LG

TL;DR: 论文提出将表示稳定性分为统计可识别性和结构可识别性，定义了ε-近可识别性概念，证明了非线性解码器模型的统计近可识别性，并展示了ICA后处理可实现解纠缠


<details>
  <summary>Details</summary>
Motivation: 现有研究将表示稳定性视为单一属性，但作者认为应区分为统计可识别性（跨运行一致性）和结构可识别性（与真实表示的对应关系）。由于现代表示学习模型难以实现完美的点对点可识别性，需要更实用的理论框架

Method: 1. 提出统计和结构ε-近可识别性的模型无关定义；2. 证明非线性解码器模型的统计近可识别性定理；3. 使用独立成分分析（ICA）解决剩余的线性模糊性；4. 通过合成基准和细胞显微镜MAE模型进行实证验证

Result: 1. 理论证明了非线性解码器模型的统计近可识别性；2. 在合成基准上，ICA后处理的普通自编码器达到最先进的解纠缠性能；3. 在细胞显微镜的MAE模型中，成功分离生物变异和技术批次效应，显著提升下游泛化能力

Conclusion: 表示稳定性应区分为统计和结构可识别性，ε-近可识别性为现代表示学习提供了实用理论框架。ICA后处理是实现解纠缠的简单有效方法，在合成数据和真实生物数据中都表现出色，为表示学习提供了理论基础和实用工具
...
### [109] [Cross-Domain Policy Optimization via Bellman Consistency and Hybrid Critics](https://arxiv.org/abs/2603.12087)
*Ming-Hong Chen,Kuan-Chen Pan,You-De Huang,Xi Liu,Ping-Chun Hsieh*

Main category: cs.LG

TL;DR: QAvatar：一种通过跨域Bellman一致性和混合批评器来解决跨域强化学习中状态/动作空间差异和负迁移问题的框架


<details>
  <summary>Details</summary>
Motivation: 跨域强化学习面临两个核心挑战：1）源域和目标域可能具有不同的状态空间或动作空间，导致直接迁移不可行；2）源域模型的迁移性难以预先确定，容易产生负迁移效应

Method: 提出跨域Bellman一致性作为衡量源域模型迁移性的指标，并设计QAvatar框架，通过自适应无超参数权重函数结合源域和目标域的Q函数

Result: QAvatar在各种RL基准任务（包括运动控制和机器人手臂操作）中实现了良好的迁移性能，有效利用源域Q函数进行知识迁移

Conclusion: 通过跨域Bellman一致性和混合批评器的联合设计，QAvatar能够可靠地进行跨域知识迁移，解决了跨域强化学习中的空间差异和负迁移问题
...
### [110] [Taming the Adversary: Stable Minimax Deep Deterministic Policy Gradient via Fractional Objectives](https://arxiv.org/abs/2603.12110)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习在控制决策任务中表现优异，但面对意外外部干扰和模型不确定性时，智能体性能往往不稳定或下降。确保在这些条件下的可靠性能仍然是一个关键挑战。
...
### [111] [Cornserve: A Distributed Serving System for Any-to-Any Multimodal Models](https://arxiv.org/abs/2603.12118)
...
### [112] [IsoCompute Playbook: Optimally Scaling Sampling Compute for LLM RL](https://arxiv.org/abs/2603.12151)
...
TL;DR: 该研究探讨了大型语言模型强化学习后训练的计算最优分配问题，发现并行rollout数量随计算预算增加而增加然后饱和，并揭示了不同难度问题的驱动机制差异。


<details>
  <summary>Details</summary>
Motivation: 虽然缩放定律指导了LLM预训练的计算分配，但针对大型语言模型强化学习后训练的计算最优分配方案仍然缺乏理解，需要研究如何在计算约束下优化RL训练资源分配。

Method: 将RL缩放问题框架化为计算约束下的优化问题，研究三个关键资源：每个问题的并行rollout数量、每批问题数量、更新步骤数。通过实验分析这些参数在不同计算预算下的最优分配。

Result: 发现计算最优的并行rollout数量随计算预算增加而增加然后饱和；简单问题通过解决方案锐化驱动，困难问题通过覆盖扩展驱动；增加并行rollout可以减轻问题间干扰，问题数量主要影响训练稳定性且选择范围较宽。

Conclusion: 研究结果将RL缩放定律重新表述为规定性分配规则，为计算高效的LLM RL后训练提供了实用指导，在不同基础模型和数据分布上得到验证。
...
### [113] [Neural Thickets: Diverse Task Experts Are Dense Around Pretrained Weights](https://arxiv.org/abs/2603.12228)
*Yulu Gan,Phillip Isola*

Main category: cs.LG

TL;DR: 论文提出将预训练结果视为参数向量分布而非单一向量，发现大模型中任务专家密度高，可通过随机采样扰动选择最佳参数实现高效后训练


<details>
  <summary>Details</summary>
Motivation: 传统方法将预训练参数向量作为迭代优化的起点，但作者认为预训练结果应视为参数向量分布，其中已包含任务特定专家，这为更高效的后训练方法提供了理论基础

Method: 提出完全并行的后训练方法：1) 在预训练权重周围随机采样N个参数扰动；2) 选择表现最佳的K个参数；3) 通过多数投票集成预测结果

Result: 该方法在当代大规模模型中与PPO、GRPO、ES等标准后训练方法竞争性相当，尽管方法简单但效果显著

Conclusion: 大模型预训练权重分布中已包含高密度的任务专家，简单的随机采样选择方法可有效利用这一特性，为后训练提供了新视角和实用方法
...
### [114] [Temporal Straightening for Latent Planning](https://arxiv.org/abs/2603.12231)
*Ying Wang,Oumayma Bounou,Gaoyue Zhou,Randall Balestriero,Tim G. J. Rudner,Yann LeCun,Mengye Ren*

Main category: cs.LG

TL;DR: 论文提出"时间拉直"方法，通过曲率正则化改进世界模型中的表示学习，使潜在空间中的欧氏距离更好地反映测地距离，从而提升基于梯度的规划性能。


<details>
  <summary>Details</summary>
Motivation: 虽然预训练的视觉编码器能产生强语义特征，但这些特征并非为规划任务定制，可能包含无关甚至有害信息。受人类视觉处理中感知拉直假说的启发，需要改进表示学习以更好地支持潜在规划。

Method: 引入时间拉直方法，使用曲率正则化器鼓励局部拉直的潜在轨迹，联合学习编码器和预测器。通过减少曲率使潜在空间中的欧氏距离更好地近似测地距离，并改善规划目标的调节。

Result: 实验表明，时间拉直使基于梯度的规划更加稳定，在一系列目标达成任务中显著提高了成功率。

Conclusion: 时间拉直通过改进表示学习，使潜在空间几何更适合规划任务，为世界模型中的潜在规划提供了有效的表示学习方法。
...
### [115] [The Latent Color Subspace: Emergent Order in High-Dimensional Chaos](https://arxiv.org/abs/2603.12261)
*Mateusz Pach,Jessica Bader,Quentin Bouniot,Serge Belongie,Zeynep Akata*

Main category: cs.LG

TL;DR: 论文提出了一种解释FLUX.1变分自编码器潜在空间中颜色表示的方法，揭示了其反映色调、饱和度和亮度的结构，并开发了基于闭式潜在空间操作的免训练颜色控制方法。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成模型发展迅速，但实现生成图像的细粒度控制仍然困难，主要原因是缺乏对语义信息如何编码的理解。作者旨在解释FLUX.1变分自编码器潜在空间中的颜色表示。

Method: 开发了潜在颜色子空间（LCS）解释方法，揭示了潜在空间反映色调、饱和度和亮度的结构。提出了一种完全免训练的方法，仅基于闭式潜在空间操作，在FLUX中实现颜色预测和显式控制。

Result: 验证了LCS解释的有效性，证明其既能预测颜色，又能显式控制颜色。代码已在GitHub上公开。

Conclusion: 通过解释FLUX.1潜在空间中的颜色表示结构，提出了一种免训练的细粒度颜色控制方法，为理解生成模型中的语义编码提供了新视角。
...
### [116] [STAMP: Selective Task-Aware Mechanism for Text Privacy](https://arxiv.org/abs/2603.12237)
...
<details>
  <summary>Details</summary>
Motivation: 现有文本隐私保护方法通常对所有token应用相同的噪声，忽视了不同token对下游任务的重要性差异和隐私敏感度差异，导致隐私保护与任务效用之间的权衡不佳。

Method: STAMP框架：1）选择性分配隐私预算：基于token对下游任务的重要性（通过任务/查询特定表示衡量）和隐私敏感度（如姓名、日期、标识符）；2）极坐标机制：在单位球面上仅扰动嵌入方向，保持幅度不变；3）余弦最近邻解码：使扰动几何与解码几何对齐。

Result: 在SQuAD、Yelp和AG News数据集上的实验表明，STAMP结合归一化极坐标机制在不同每token隐私预算下均能实现优越的隐私-效用权衡。

Conclusion: STAMP通过细粒度、组级别的隐私预算分配和几何对齐的扰动机制，实现了比各向同性噪声机制更好的语义保持和下游任务效用，为任务感知文本隐私保护提供了有效解决方案。
...
### [117] [Matching Features, Not Tokens: Energy-Based Fine-Tuning of Language Models](https://arxiv.org/abs/2603.12248)
*Samy Jelassi,Mujin Kwun,Rosie Zhao,Yuanzhi Li,Nicolo Fusi,Yilun Du,Sham M. Kakade,Carles Domingo-Enrich*

Main category: cs.LG

TL;DR: EBFT是一种基于能量模型的微调方法，通过特征匹配目标优化语言模型的序列级行为，在多个任务上优于监督微调，同时保持较低的验证交叉熵。


<details>
  <summary>Details</summary>
Motivation: 交叉熵训练虽然提供密集监督，但仅优化教师强制下的下一个token预测，而非模型展开下的序列级行为。需要一种能提供密集语义反馈但不需要任务特定验证器或偏好模型的方法。

Method: 提出能量微调（EBFT）：使用跨步块并行采样从嵌套前缀并发生成多个展开，对这些展开进行批量特征提取，并使用得到的嵌入执行基于策略的策略梯度更新。

Result: 在问答编码、非结构化编码和翻译任务上，EBFT与RLVR相当，在下游准确率上优于监督微调，同时验证交叉熵低于两种方法。

Conclusion: EBFT通过特征匹配目标有效优化语言模型的序列级行为，提供了一种不需要任务特定验证器就能获得密集语义反馈的微调方法。
...
### [118] [Enhancing Value Alignment of LLMs with Multi-agent system and Combinatorial Fusion](https://arxiv.org/abs/2603.11126)
*Yuanhong Wu,Djallel Bouneffouf,D. Frank Hsu*

Main category: cs.MA

TL;DR: VAS-CFA框架通过多智能体融合分析实现LLM价值对齐，利用多个代表不同规范视角的道德智能体，结合排名和分数聚合，超越单智能体基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有RLHF等对齐方法通常依赖单一评估者或狭窄的奖励信号，难以捕捉伦理多元主义，限制了LLM与人类价值观的对齐能力。

Method: 提出VAS-CFA框架：实例化多个代表不同规范视角的道德智能体，使用组合融合分析（CFA）结合排名和分数两种聚合方式，融合多智能体输出。

Result: 实证评估显示VAS-CFA在标准指标上优于单智能体基线和先前聚合方法，证明多智能体融合为LLM价值对齐提供了稳健有效的机制。

Conclusion: 多智能体融合分析能够利用认知多样性缓解冲突和冗余，产生更好反映人类价值观的响应，是推进LLM价值对齐的有效框架。
...
### [119] [How Intelligence Emerges: A Minimal Theory of Dynamic Adaptive Coordination](https://arxiv.org/abs/2603.11560)
*Stefano Grassi*
...
<details>
  <summary>Details</summary>
Motivation: 传统方法通过均衡优化或智能体中心学习来分析协调，但本文旨在开发一个更全面的框架，将智能体、激励和环境建模为递归闭合的反馈架构，以理解协调作为耦合动力学的结构特性。

Method: 采用动力学系统理论方法，构建包含持久环境、分布式激励场和自适应智能体的递归闭合反馈架构。持久环境存储累积的协调信号，分布式激励场局部传输这些信号，自适应智能体响应更新。通过谱条件分析雅可比矩阵来研究局部稳定性和振荡机制。

Result: 建立了三个结构结果：1) 在耗散性假设下，闭环系统存在有界前向不变区域，确保可行性而无需全局最优性；2) 当激励信号非平凡地依赖于持久环境记忆时，动力学通常不能简化为仅定义在智能体状态空间上的静态全局目标；3) 持久环境状态诱导历史敏感性，除非系统全局收缩。

Conclusion: 智能协调动力学可以从持久环境中激励介导的自适应交互中涌现，无需预设福利最大化、理性预期或集中式设计。协调作为耦合动力学的结构特性，由持久性、耗散性和耦合共同决定。
...
### [120] [The price of decentralization in managing engineering systems through multi-agent reinforcement learning](https://arxiv.org/abs/2603.11884)
*Prateek Bhustali,Pablo G. Morato,Konstantinos G. Papakonstantinou,Charalampos P. Andriotis*

Main category: cs.MA

TL;DR: 该论文研究了多组件系统检查维护规划中的多智能体深度强化学习，通过设计包含冗余变化的退化系统基准环境，比较了不同MADRL算法在协调挑战和性能表现上的差异。


<details>
  <summary>Details</summary>
Motivation: 检查维护规划涉及不确定性和不完全信息下的序列决策，可建模为部分可观测马尔可夫决策过程。虽然单智能体深度强化学习能为POMDP提供近似解，但在多组件系统中扩展性差。多智能体深度强化学习通过分散决策提高可扩展性，但可能导致合作病理问题影响策略最优性。

Method: 1. 引入一组退化系统基准环境，系统性地变化冗余度设计；2. 保持集中式（近）最优策略计算可处理性，便于直接比较不同方法；3. 实现并基准测试广泛的多智能体深度强化学习算法，涵盖完全集中式和分散式训练范式，包括价值分解和行动者-评论家方法。

Result: 1. 冗余度对协调有明显影响：MADRL算法在串联式设置中达到接近最优性能；2. 增加冗余度会放大协调挑战，可能导致最优性损失；3. 分散式智能体学习到的结构化策略始终优于优化的启发式基线方法。

Conclusion: 多智能体深度强化学习在可扩展维护规划中既有前景也有当前局限性。分散式智能体能够学习到优于启发式基线的结构化策略，但冗余度增加会加剧协调挑战，影响策略最优性。研究为理解MADRL在检查维护规划中的应用提供了基准和分析框架。
...
### [121] [CogSearch: A Cognitive-Aligned Multi-Agent Framework for Proactive Decision Support in E-Commerce Search](https://arxiv.org/abs/2603.11927)
...
<details>
  <summary>Details</summary>
Motivation: 传统电商搜索引擎基于被动检索和排序模型，难以支持复杂决策过程，导致用户面临认知摩擦和决策困难。需要一种新的搜索范式来主动协助用户决策。

Method: 提出CogSearch框架，包含四个专门化智能体：1) 分解复杂用户意图；2) 融合内部和外部异构知识；3) 模拟人类认知工作流程；4) 提供可操作的洞察。系统从被动检索转向主动决策支持。

Result: 离线基准测试显示CogSearch在咨询性和复杂搜索场景中表现优异。在JD.com的在线A/B测试中：决策成本降低5%，整体UCVR提升0.41%，决策密集型查询的转化率显著提升30%。

Conclusion: CogSearch代表了信息检索的根本性转变，从传统的以相关性为中心的范式转向整体协作的决策智能，为电商搜索提供了新的认知导向框架。
...
TL;DR: Co-Diffusion是一个用于药物-靶点亲和力预测的新框架，通过约束潜在去噪过程解决冷启动场景下的表示崩溃问题，在未见分子支架和蛋白质家族上实现卓越的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有深度模型在严格的冷启动场景下容易发生表示崩溃，标签稀缺和领域偏移阻碍了可迁移的药效团和结合基序的学习，需要新的方法来增强泛化能力。
...
TL;DR: 该论文提出诊断传输映射方法，用于改进预测系统的校准性能，特别是在低频或分布外区域，通过调整基础模型的概率分布来匹配真实条件分布。


<details>
  <summary>Details</summary>
Motivation: 当前预测系统虽然提供完整的预测分布，但很少能在所有输入x和输出y上实现良好校准。在低频或分布外区域，估计的密度尤其不可靠，而这些区域恰恰最需要准确的不确定性量化和专家验证来建立模型信任。

Method: 提出诊断传输映射方法：将初始预测分布视为有用但可能错误指定的基础模型，然后引入协变量依赖的概率到概率映射，量化基础模型的概率应如何调整以更好地匹配校准数据的真实条件分布。这些映射在部署时提供实时本地诊断，揭示模型失败的位置和方式（包括偏差、离散度、偏度和尾部误差），同时通过与基础模型的简单组合产生重新校准的预测分布。

Result: 将诊断传输映射应用于短期热带气旋强度预测，发现易于拟合的参数版本能够识别与局部校准错误相关的演化模式，并改善罕见事件（包括24小时快速强度变化）的预测性能，优于美国国家飓风中心的业务预报。

Conclusion: 诊断传输映射提供了一种有效方法来改进预测系统的校准性能，特别是在需要准确不确定性量化的低频和分布外区域。该方法不仅提供重新校准的预测分布，还提供实时诊断信息，帮助专家验证模型结果并建立信任。
...
TL;DR: 本文提出了bfVAE框架，统一了多种解耦VAE方法，并开发了无需真实生成因子的评估工具FVH-LT、DBSR-LS和LSDI，用于评估和解释潜在表示。


<details>
  <summary>Details</summary>
Motivation: 评估和解释潜在表示（如VAE）是一个重大挑战，尤其是在真实生成因子未知的情况下。现有方法需要真实生成因子作为基准，限制了在真实数据中的应用。

Method: 提出了bfVAE框架统一多种解耦VAE方法；开发了FVH-LT（通过潜在遍历的特征方差异质性）和DBSR-LS（潜在空间中的脏块稀疏回归）两种评估程序；提出了LSDI（潜在空间解耦指数）综合评估解耦效果；设计了贪婪对齐策略（GAS）解决标签切换问题。

Result: bfVAE在表格和图像数据实验中超越了现有解耦VAE框架，解耦质量更高、更鲁棒，信息性潜在维度的误报率接近零；FVH-LT和DBSR-LS能可靠地发现语义相关和领域相关的潜在结构；LSDI能有效定量总结解耦效果。

Conclusion: bfVAE框架和配套的评估工具为无需真实生成因子的潜在表示解耦和评估提供了有效解决方案，显著提升了VAE在表格数据等领域的应用价值。
...
TL;DR: 本文提出了一种基于正则化诱导探索的上下文赌博机方法，利用模型训练过程中的随机性作为内在探索源，无需额外探索策略。
...
Method: 采用纯贪婪动作选择策略，利用模型拟合过程中的随机性作为内在探索源。特别关注基于交叉验证的正则化过程中的随机性，这种正则化诱导的探索在理论上等价于Thompson Sampling。
...
TL;DR: wcPCA框架统一处理跨域PCA问题，通过最坏情况优化提升分布偏移下的可靠性，相比标准PCA在异构领域数据中表现更稳定。


<details>
  <summary>Details</summary>
Motivation: 现实世界健康、经济和环境科学数据常从异构领域收集，分布偏移使标准PCA不可靠，主成分在未见域中解释方差显著减少。现有方法如FairPCA关注多域最坏情况性能，但需更统一框架处理异构总方差问题。

Method: 开发统一框架wcPCA，提出新估计器如norm-minPCA和norm-maxregret，适用于异构总方差应用。证明所有目标下估计器不仅在观测源域最优，也在协方差位于源协方差凸包内的所有目标域中最坏情况最优。建立经验估计器的一致性和渐近最坏情况保证。将方法扩展到矩阵补全问题，证明归纳矩阵补全的近似最坏情况最优性。

Result: 模拟和生态系统-大气通量两个真实应用显示，最坏情况性能显著改善，平均性能仅轻微损失。新方法在分布偏移下比标准PCA更可靠。

Conclusion: wcPCA框架为跨域PCA问题提供统一解决方案，通过最坏情况优化确保异构数据环境下的可靠性，可扩展到矩阵补全等低秩近似问题，在现实应用中具有实用价值。
...
TL;DR: 该论文提出了一种针对空间相关性和缺失随机标签数据的双重稳健估计方法，通过交叉拟合和jackknife空间HAC方差校正来解决传统方法在空间依赖场景下置信区间不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 当结果数据收集成本高昂时，研究者常使用机器学习/AI模型预测替代未标记数据，但这会影响统计推断。现有方法在独立采样下有效，但现实应用中存在缺失随机标签和空间依赖性，需要新的推断方法。

Method: 提出双重稳健估计器结合交叉拟合，针对交叉拟合导致的折叠级相关性扭曲空间方差估计的问题，设计了jackknife空间异方差和自相关一致方差校正方法，分离空间依赖和折叠诱导噪声。

Result: 在标准识别和依赖条件下，得到的置信区间具有渐近有效性。模拟和基准数据集显示，在有限样本校准方面有显著改进，特别是在缺失随机标签和聚类抽样场景下。

Conclusion: 该方法为存在空间相关性和缺失随机标签数据的统计推断提供了有效的解决方案，通过创新的方差校正技术解决了交叉拟合在空间依赖场景下的局限性。
...
TL;DR: 该研究为决策树分类器提出了两种观测多重性度量：叶后悔和结构后悔，前者量化固定叶内的预测变异性，后者捕捉学习树结构不稳定引起的变异性，并证明结构后悔是观测多重性的主要驱动因素。


<details>
  <summary>Details</summary>
Motivation: 机器学习任务中存在多个性能相近的模型（预测多重性），其中观测多重性源于标签收集的随机性。虽然逻辑回归的观测多重性理论框架已建立，但对于决策树等非平滑、基于划分的模型，其影响尚未充分探索。

Method: 为决策树分类器引入两种互补的观测多重性概念：叶后悔（量化固定叶内因有限样本噪声引起的预测变异性）和结构后悔（捕捉学习树结构不稳定引起的变异性）。提供形式化分解并建立统计保证。

Result: 在多个信用风险评分数据集上的实验验证了理论分解与经验观测方差近乎完美的一致性。发现结构后悔是观测多重性的主要驱动因素，在某些数据集中其变异性是叶后悔的15倍以上。将这些后悔度量用作选择性预测中的弃权机制，可有效识别任意区域并提高模型安全性。

Conclusion: 该研究为量化观测多重性建立了严格框架，与算法安全性和可解释性的最新进展保持一致。结构后悔的主导作用凸显了树结构稳定性在决策树预测可靠性中的重要性。
...
TL;DR: 本章为四元数在机器学习中的应用奠定基础，涵盖四元数统计、广线性模型、四元数微积分和均方估计等核心内容。


<details>
  <summary>Details</summary>
Motivation: 四元数在三维旋转建模方面具有独特优势，已广泛应用于航空和计算机图形学等领域。随着机器学习的兴起，四元数研究迎来复兴，需要为研究人员提供理论基础来贡献这一新兴领域。

Method: 通过建立四元数随机过程的增广统计模型，发展广线性模型以利用这些高级统计特性，运用四元数微积分和代数进行算法推导，并采用均方估计考虑实际应用。

Result: 提供了系统的四元数理论基础框架，包含多个示例以促进学习、理解和采用这一多维域。

Conclusion: 本章为机器学习中的四元数应用建立了必要的基础设施，使研究人员能够在这一新兴领域做出贡献，推动四元数在工业应用中的进一步发展。
...
TL;DR: 提出了一种基于概率提升的批量贝叶斯最优实验设计新方法，通过将优化问题提升到概率测度空间，优化设计测度上的熵正则化期望效用，得到吉布斯分布形式的设计法则，并开发了可扩展的近似方法和粒子算法。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯最优实验设计(BOED)在实际应用中受到所选效用优化困难的限制，特别是批量设置中期望信息增益(EIG)的高维性和强非凸性挑战。

Method: 通过概率提升将原始优化问题提升到概率测度空间，优化设计测度上的熵正则化期望效用；提出两种可扩展近似：平均场族和i.i.d.乘积族；推导Wasserstein梯度流并开发粒子算法；引入结合交互粒子更新和蒙特卡洛估计的双随机变体。

Result: 在温和条件下，该目标存在唯一最小化器，可显式表征为吉布斯分布；设计法则可直接用作随机批量设计策略或计算松弛；数值实验表明方法能探索多模态优化景观并在挑战性示例中获得高效用批量。

Conclusion: 提出的概率提升方法为批量EIG-based BOED提供了有效框架，通过熵正则化和概率测度空间优化克服了传统优化困难，开发的可扩展算法在实际应用中表现出色。
