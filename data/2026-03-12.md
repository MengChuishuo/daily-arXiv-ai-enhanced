...
TL;DR: LLMGreenRec是一个基于大语言模型的多智能体框架，用于在电子商务中推荐可持续产品，同时减少数字碳足迹。


<details>
  <summary>Details</summary>
Motivation: 电子商务中环境意识增强，需要推荐系统既能引导用户选择可持续产品，又能最小化自身的数字碳足迹。传统的基于会话的系统通常只优化短期转化，无法捕捉用户对环保选择的细微意图，导致绿色意图与实际行动之间存在差距。

Method: 引入LLMGreenRec，这是一个新颖的多智能体框架，利用大语言模型促进可持续消费。通过协作分析用户交互和迭代提示优化，专门设计的智能体推断出绿色导向的用户意图，并优先推荐环保产品。这种意图驱动的方法还能减少不必要的交互和能源消耗。

Result: 在基准数据集上的广泛实验验证了LLMGreenRec在推荐可持续产品方面的有效性，证明这是一个促进负责任数字经济的稳健解决方案。

Conclusion: LLMGreenRec通过大语言模型和多智能体协作，成功解决了传统推荐系统在可持续消费方面的不足，既能准确捕捉用户绿色意图，又能降低系统自身的碳足迹，为构建负责任的数字经济提供了有效方案。
...
### [2] [KernelSkill: A Multi-Agent Framework for GPU Kernel Optimization](https://arxiv.org/abs/2603.10085)
*Qitong Sun,Jun Han,Tianlin Li,Zhe Tang,Sheng Chen,Fei Yang,Aishan Liu,Xianglong Liu,Yang Liu*

Main category: cs.LG

TL;DR: KernelSkill是一个基于多智能体框架的GPU内核优化系统，通过知识驱动的专家技能替代传统LLM的隐式启发式方法，在KernelBench基准测试中取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的GPU内核优化方法依赖模型内部隐式学习的启发式策略，导致试错效率低且优化过程缺乏可解释性。需要更知识驱动、任务轨迹感知的优化方法。

Method: 提出KernelSkill多智能体框架，采用双层记忆架构：长期记忆存储可重用的专家技能，短期记忆防止重复回溯。通过协调多个智能体实现知识驱动的优化。

Result: 在KernelBench Level 1-3测试中，KernelSkill达到100%成功率，相比Torch Eager分别获得5.44x、2.82x和1.92x的平均加速，优于现有基线方法。

Conclusion: KernelSkill通过知识驱动的专家技能替代隐式启发式方法，显著提高了GPU内核优化的效率和可解释性，为AI系统性能提升提供了新思路。
...
### [3] [MoE-SpAc: Efficient MoE Inference Based on Speculative Activation Utility in Heterogeneous Edge Scenarios](https://arxiv.org/abs/2603.09983)
*Shuhuai Li,Jianghao Lin,Dongdong Ge,Yinyu Ye*

Main category: cs.LG

TL;DR: MoE-SpAc：利用推测解码作为内存管理的前瞻传感器，解决边缘设备上MoE模型的内存限制问题，通过动态负载均衡和异步执行实现高效推理。


<details>
  <summary>Details</summary>
Motivation: 混合专家模型在边缘设备上面临严重的内存限制，现有的卸载策略由于自回归专家激活的动态性和低信息特性而难以应对I/O瓶颈。

Method: 提出MoE-SpAc框架：1）推测效用估计器跟踪专家需求；2）异构工作负载均衡器通过在线整数优化动态分区计算；3）异步执行引擎在统一效用空间协调预取和驱逐。

Result: 在七个基准测试中，MoE-SpAc相比最先进的SD基线实现了42%的TPS提升，相比所有标准基线平均加速4.04倍。

Conclusion: 将推测解码重新定位为内存管理的前瞻传感器，结合动态负载均衡和异步执行，能有效解决MoE模型在边缘设备上的内存限制问题。
...
### [4] [Personalized Group Relative Policy Optimization for Heterogenous Preference Alignment](https://arxiv.org/abs/2603.10009)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）通常难以与多样化的个人偏好对齐，因为标准的后训练方法（如RLHF）优化的是单一的全局目标。虽然GRPO被广泛采用，但其基于组的归一化隐含假设所有样本可互换，在个性化设置中继承了这一限制，混淆了不同的用户奖励分布，并系统性地偏向主导偏好而抑制少数信号。

Method: 提出个性化GRPO（P-GRPO）框架，通过将优势估计与即时批次统计解耦来解决这一问题。P-GRPO不是针对并发生成组进行归一化，而是针对偏好组特定的奖励历史进行优势归一化，从而保留学习不同偏好所需的对比信号。

Result: 在多样化任务中评估P-GRPO，发现它比标准GRPO始终实现更快的收敛和更高的奖励，从而增强了恢复和对齐异构偏好信号的能力。
...
### [5] [LWM-Temporal: Sparse Spatio-Temporal Attention for Wireless Channel Representation Learning](https://arxiv.org/abs/2603.10024)
...
<details>
  <summary>Details</summary>
Motivation: 无线信道具有复杂的时空特性，传统方法难以有效捕捉移动性引起的信道演化。需要开发能够学习通用信道表示、可跨任务迁移的基础模型。

Method: 在角度-延迟-时间域操作，引入稀疏时空注意力机制，限制物理可实现的邻域交互，降低注意力复杂度。采用物理信息掩码课程进行自监督预训练，模拟真实遮挡、导频稀疏性和测量损伤。

Result: 在多种移动场景下的信道预测实验中，相比强基线模型表现出一致改进，特别是在长时域预测和有限微调数据情况下优势明显。

Conclusion: 几何感知架构和几何一致预训练对于学习可迁移的时空无线表示至关重要。LWM-Temporal作为任务无关的基础模型，能够学习捕捉移动性演化的通用信道嵌入。
...
TL;DR: XSAM提出了一种新的显式锐度感知最小化方法，通过显式估计最大损失方向来改进SAM的近似效果，解决了单步梯度近似不准确和多步近似质量下降的问题。


<details>
  <summary>Details</summary>
Motivation: SAM在实践中通过梯度上升点处的梯度来更新当前参数，但缺乏对这种做法为何有效的直观理解。同时，单步梯度近似往往不准确，且随着上升步数增加近似质量会下降。

Method: 提出XSAM方法：1）显式估计局部邻域内最大损失的方向；2）设计有效的搜索空间以利用多步上升点的梯度信息；3）提供适用于单步和多步设置的统一公式，计算开销可忽略。

Result: 大量实验表明XSAM在性能上持续优于现有的SAM变体方法。

Conclusion: XSAM通过显式估计最大损失方向和有效利用多步梯度信息，解决了SAM近似不准确的问题，提供了更直接逃离局部最大值的优化方向。
...
<details>
  <summary>Details</summary>
Motivation: 虽然组合融合分析（CFA）提供了结合多个评分系统的方法和实践，包括排名-得分特征函数和认知多样性，但目前缺乏通用的Python工具来整合这些技术。

Method: 开发了InFusionLayer机器学习架构，受CFA在系统融合层面的启发，使用中等规模的基础模型集，支持PyTorch、TensorFlow和Scikit-learn工作流。

Result: 在多个计算机视觉数据集上验证了性能，结果表明结合RSC函数和CD的独特特征具有实际优势。

Conclusion: InFusionLayer为更复杂的集成学习应用铺平了道路，已开源代码以促进持续开发和社区可访问性。
...
TL;DR: CAADRL是一个基于深度强化学习的框架，通过集群感知编码和分层解码显式利用PDP问题的多尺度结构，在保持竞争力的同时显著降低推理时间。


<details>
  <summary>Details</summary>
Motivation: 现有的DRL方法要么将所有节点建模在平面图上，依赖隐式学习来强制约束，要么通过推理时的协作搜索获得强性能但带来显著延迟。需要一种既能利用PDP实例多尺度结构又能保持高效推理的方法。

Method: CAADRL采用集群感知编码和分层解码。编码器基于Transformer，结合全局自注意力和集群内注意力（针对仓库、取货、送货节点）。解码器采用动态双解码器，通过可学习门控平衡集群内路由和集群间转移。使用POMO风格策略梯度方案进行端到端训练。

Result: 在合成的集群化和均匀PDP基准测试中，CAADRL在集群化实例上匹配或超越了现有最先进基线，在均匀实例上保持高度竞争力，尤其随着问题规模增大。关键的是，该方法以显著低于神经协作搜索基线的推理时间实现了这些结果。

Conclusion: 显式建模集群结构为神经PDP求解器提供了有效且高效的归纳偏置，CAADRL在保持性能竞争力的同时大幅降低了推理时间。
...
TL;DR: HTMuon改进Muon训练方法，通过保留参数互依赖性的同时产生更重尾的更新和权重谱，在LLM预训练和图像分类任务上优于现有基线
...
Result: 在LLM预训练和图像分类实验中，HTMuon始终优于最先进的基线方法。在C4数据集上的LLaMA预训练中，HTMuon相比Muon将困惑度降低了0.98

Conclusion: HTMuon在理论上对应Schatten-q范数约束下的最速下降，在平滑非凸设置下提供收敛分析。该方法有效改进了Muon，并在多个任务上表现出更好的性能
...
### [10] [Dissecting Chronos: Sparse Autoencoders Reveal Causal Feature Hierarchies in Time Series Foundation Models](https://arxiv.org/abs/2603.10071)
*Anurag Mishra*

Main category: cs.LG

TL;DR: 该研究首次将稀疏自编码器应用于时间序列基础模型Chronos-T5-Large，通过392个单特征消融实验揭示了模型内部表示的分层结构：早期层编码低频特征，中间层包含关键的变化检测特征，最终层压缩了丰富但因果重要性较低的时间概念。


<details>
  <summary>Details</summary>
Motivation: 时间序列基础模型在关键领域部署日益增多，但其内部表示仍然不透明。研究者希望应用机械可解释性方法来理解Chronos-T5-Large模型的内部工作机制，特别是不同层次在时间序列预测中的作用。

Method: 在Chronos-T5-Large模型的六个层次上训练TopK稀疏自编码器，通过392个单特征消融实验评估每个特征的因果相关性，使用CRPS评分作为评估指标。

Result: 所有消融特征都产生正CRPS退化，证实了因果相关性。发现深度依赖的分层结构：早期编码器层编码低层频率特征，中间编码器集中了因果关键的变化检测特征，最终编码器压缩了丰富但因果重要性较低的时间概念。最关键的特征位于中间编码器层（最大单特征Delta CRPS = 38.61）。

Conclusion: 机械可解释性方法有效适用于时间序列基础模型，Chronos-T5主要依赖突变动态检测而非周期性模式识别。最关键的特征位于中间层而非语义最丰富的最终层，这对模型理解和优化有重要启示。
...
### [11] [ConfHit: Conformal Generative Design with Oracle Free Guarantees](https://arxiv.org/abs/2603.07371)
*Siddhartha Laghuvarapu,Ying Jin,Jimeng Sun*

Main category: cs.LG

TL;DR: ConfHit是一个用于生成式药物发现的分布无关框架，提供在预算限制、无实验验证和分布偏移条件下的有效性保证，通过加权交换性和密度比加权conformal p值来认证和优化候选分子集。


<details>
  <summary>Details</summary>
Motivation: 深度生成模型在科学发现中不仅需要生成新颖候选物，还需要可靠保证这些候选物满足所需特性。现有conformal预测方法在药物发现中应用受限，主要面临预算限制、缺乏实验验证和分布偏移等问题。

Method: ConfHit利用历史样本和生成样本之间的加权交换性消除对实验验证的需求，构建多样本密度比加权conformal p值来量化命中统计置信度，并提出嵌套测试程序来认证和优化多个生成样本的候选集，同时保持统计保证。

Result: 在代表性生成分子设计任务和多种方法中，ConfHit在多个置信水平下始终提供有效的覆盖保证，同时保持紧凑的认证集，为生成建模建立了原则性和可靠的框架。

Conclusion: ConfHit为生成式药物发现提供了一个分布无关框架，能够在现实约束条件下提供统计保证，解决了认证和设计两个核心问题，为科学发现中的生成建模提供了可靠基础。
...
<details>
  <summary>Details</summary>
Motivation: 时间序列预测面临非平稳性挑战，特别是分布漂移问题。传统Adam优化器针对平稳目标设计，其二阶偏差校正限制了在变化损失景观中的响应能力
...
Conclusion: TS_Adam作为简单有效的优化策略，能显著提升非平稳时间序列预测性能，易于集成到现有模型中，具有实用性和通用性
...
### [13] [GGMPs: Generalized Gaussian Mixture Processes](https://arxiv.org/abs/2603.10442)
...
<details>
  <summary>Details</summary>
Motivation: 条件密度估计面临多模态、异方差性和强非高斯性的挑战。传统高斯过程回归虽然提供校准的不确定性，但其单模态高斯预测形式限制了处理复杂输出分布的能力。

Method: GGMP结合了局部高斯混合拟合、跨输入分量对齐和每个分量的异方差高斯过程训练。该方法避免了朴素多模态GP公式中指数级大的潜在分配结构，与标准GP求解器和可扩展方法兼容。

Result: 在具有明显非高斯性和多模态性的合成和真实世界数据集上，GGMP改善了分布近似效果。

Conclusion: GGMP提供了一种可处理、可扩展的高斯过程方法，能够有效处理多模态条件密度估计问题，克服了传统GP回归的局限性。
...
### [14] [Digging Deeper: Learning Multi-Level Concept Hierarchies](https://arxiv.org/abs/2603.10084)
...
<details>
  <summary>Details</summary>
Motivation: 现有概念模型依赖详尽标注且将概念视为扁平独立，而HiCEMs和概念分割方法仅限于浅层层次结构，需要克服这些限制

Method: 提出MLCS（多层次概念分割）从顶层监督中发现多层次概念层次结构，以及Deep-HiCEMs架构来表示这些发现的层次结构并支持多抽象级别的测试时干预

Result: 在多个数据集上的实验表明，MLCS能够发现训练中不存在的人类可解释概念，Deep-HiCEMs在保持高准确率的同时支持测试时概念干预，并能提高任务性能

Conclusion: MLCS和Deep-HiCEMs成功克服了现有概念模型的局限性，能够从有限监督中发现多层次概念层次结构，并支持灵活的概念干预，提高了概念模型的实用性和解释性
...
### [15] [Decoupling Reasoning and Confidence: Resurrecting Calibration in Reinforcement Learning from Verifiable Rewards](https://arxiv.org/abs/2603.09117)
*Zhengzhao Ma,Xueru Wen,Boxi Cao,Yaojie Lu,Hongyu Lin,Jinglin Yang,Min He,Xianpei Han,Le Sun*

Main category: cs.LG

TL;DR: DCPO框架通过解耦推理和校准目标，解决RLVR中LLM过度自信问题，在保持准确性的同时显著改善校准性能。


<details>
  <summary>Details</summary>
Motivation: RLVR虽然能增强大语言模型的推理能力，但会导致严重的校准退化问题，使模型对错误答案过度自信。现有研究试图直接将校准目标纳入优化目标，但存在根本性的梯度冲突。

Method: 提出DCPO框架，通过系统性地解耦推理和校准目标来解决梯度冲突问题。该方法简单而有效。

Result: 实验表明DCPO在保持与GRPO相当的准确性的同时，实现了最佳的校准性能，显著缓解了过度自信问题。

Conclusion: 该研究为更可靠的大语言模型部署提供了有价值的见解和实用解决方案，证明了通过解耦目标可以有效解决RLVR中的校准退化问题。
...
### [16] [ES-dLLM: Efficient Inference for Diffusion Large Language Models by Early-Skipping](https://arxiv.org/abs/2603.10088)
*Zijian Zhu,Fei Ren,Zhanhong Tan,Kaisheng Ma*

Main category: cs.LG

TL;DR: ES-dLLM是一种无需训练的扩散大语言模型推理加速框架，通过基于重要性估计跳过早期层中的token，实现5.6-16.8倍加速，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(dLLMs)相比自回归模型具有捕获双向上下文和并行生成的潜力，但其推理过程计算成本高昂，因为每次迭代都需要处理完整的输入上下文。作者发现dLLMs的中间表示在连续迭代中变化细微，这为加速提供了机会。

Method: 提出ES-dLLM框架，通过基于重要性估计跳过早期层中的token来减少计算。重要性计算基于中间张量变化和先前迭代的置信度分数。该方法无需额外训练，直接应用于现有dLLM模型。

Result: 在LLaDA-8B和Dream-7B模型上的实验表明，ES-dLLM在NVIDIA H200 GPU上分别达到226.57和308.51 tokens/秒的吞吐量，相比原始实现获得5.6-16.8倍加速，相比最先进的缓存方法提升1.85倍，同时保持生成质量。

Conclusion: ES-dLLM通过利用dLLMs中间表示的稳定性，提供了一种有效的推理加速解决方案，显著提升吞吐量而不损害生成质量，为dLLMs的实际应用提供了重要支持。
...
TL;DR: EAD是一种新型扩散模型，通过异步去噪调度更好地捕捉分子层次结构，同时保持分子级视野，在3D分子生成中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D分子生成方法存在局限性：自回归模型受限于短视野和训练推理差异，同步扩散模型无法捕捉分子层次结构的因果关系。需要结合两者优势的新方法。

Method: 提出Equivariant Asynchronous Diffusion (EAD)，采用异步去噪调度捕捉分子层次结构，同时保持分子级视野。引入动态调度机制自适应确定去噪时间步。

Result: 实验结果显示EAD在3D分子生成任务中达到了最先进的性能。

Conclusion: EAD成功结合了自回归和扩散模型的优势，通过异步去噪和动态调度机制，在捕捉分子层次结构的同时保持分子级视野，实现了更好的3D分子生成效果。
...
TL;DR: BlueDown是一种新的后处理方法，相比TopDown能产生更准确、一致的估计，同时满足相同的隐私保证和结构约束，在县级和区域级聚合数据上获得显著精度提升。


<details>
  <summary>Details</summary>
Motivation: 美国人口普查局的披露避免系统(DAS)需要在保护隐私和数据实用性之间取得平衡，TopDown方法虽然被用于2020年人口普查，但存在改进空间。需要开发更准确、更高效的后处理方法来处理大规模人口普查数据。

Method: 开发了BlueDown方法，包含：1) 新的分层广义最小二乘回归算法，利用测量值的层次结构；2) 结合优化程序以支持相关测量；3) 使用简洁的线性代数操作利用测量和约束结构中的对称性提高效率。

Result: BlueDown在县级和区域级聚合数据上获得了特别大的精度提升，计算复杂度从矩阵乘法时间降低到线性时间，能够处理人口普查规模的数据。

Conclusion: BlueDown在保持相同隐私保证和结构约束的前提下，相比TopDown提供了更准确、更高效的后处理方法，其分层回归算法和简洁操作具有独立的研究价值。
...
TL;DR: CLIPO在RLVR基础上引入对比学习机制，通过优化成功轨迹间的对比损失，使LLM捕捉正确推理路径的共享不变结构，缓解步骤级推理不一致和幻觉问题。
...
Method: 在策略优化中引入对比学习机制，通过优化成功轨迹间的对比损失，引导LLM捕捉正确推理路径的共享不变结构，提供比原始单路径监督更鲁棒的跨轨迹正则化。

Result: CLIPO在多个推理基准测试中持续改进多个RLVR基线，在LLM策略优化的泛化性和鲁棒性方面展现出统一改进。

Conclusion: CLIPO通过对比学习机制增强了RLVR过程，有效缓解步骤级推理不一致和抑制幻觉伪影，为LLM策略优化提供了更鲁棒的跨轨迹正则化方法。
...
### [20] [Lost in the Middle at Birth: An Exact Theory of Transformer Position Bias](https://arxiv.org/abs/2603.10123)
*Borun D Chowdhury*

Main category: cs.LG

TL;DR: 研究发现"迷失在中间"现象（U形性能曲线）在模型初始化时就已存在，是因果解码器架构的固有几何特性，而非训练或位置编码所致。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为"迷失在中间"现象是由学习到的Softmax伪影或RoPE等位置编码的距离衰减引起的。本文旨在证明这一U形曲线在模型初始化时就已存在，是架构本身的固有属性。

Method: 将多层因果注意力建模为Cesàro矩阵的迭代幂次，推导出连续极限下的精确闭式影响密度。通过理论分析因果掩码和残差连接的作用机制，并实证验证未训练的Qwen2和GPT-2架构在初始化时的表现。

Result: 理论分析显示：因果掩码导致提示开始处的梯度影响对数发散（首要尾部），残差连接在最后一个token处创建孤立的O(1)锚点（最近性增量），中间区域形成阶乘死区O(1/(H-1)!)。实证验证了未训练模型在初始化时即表现出U形曲线，且无论是否使用RoPE都相同。

Conclusion: "迷失在中间"现象是因果解码器架构的固有几何特性，在初始化时即已存在。标准训练无法克服这一拓扑谷底，该U形曲线成为架构基线。研究为未来针对性改进提供了理论基础。
...
### [21] [Mashup Learning: Faster Finetuning by Remixing Past Checkpoints](https://arxiv.org/abs/2603.10156)
*Sofia Maria Lo Cicero Vaina,Artem Chumachenko,Max Ryabinin*

Main category: cs.LG

TL;DR: Mashup Learning是一种利用历史训练检查点来提升模型适应新任务性能的方法，通过选择相关检查点、模型融合，并作为改进的初始化进行训练，显著提升下游任务准确率并加速收敛。


<details>
  <summary>Details</summary>
Motivation: 领域特定数据的微调虽然能提升LLM在下游任务的表现，但每次训练产生的模型权重检查点很少被重复利用，尽管这些检查点可能包含对类似任务有用的改进能力。现有方法未能充分利用这些训练产物。

Method: Mashup Learning方法包括三个步骤：1）为目标数据集识别最相关的历史检查点；2）通过模型融合技术聚合这些检查点；3）将融合结果作为改进的初始化进行训练。该方法能够利用先前训练运行的结果来增强模型对新任务的适应能力。

Result: 在8个标准LLM基准测试、4个模型和两个源检查点集合上，Mashup Learning相比从头训练平均下游准确率提升0.5-5个百分点。同时加速收敛，需要减少41-46%的训练步数，总实际时间最多减少37%（包括所有选择和融合开销）。

Conclusion: Mashup Learning是一种简单有效的利用历史训练产物的方法，能够显著提升模型性能并加速训练收敛，为模型复用和高效适应提供了新思路。
...
### [22] [ReMix: Reinforcement routing for mixtures of LoRAs in LLM finetuning](https://arxiv.org/abs/2603.10160)
...
TL;DR: ReMix提出了一种新的Mixture-of-LoRAs路由器设计，使用非可学习的路由权重避免权重不平衡问题，通过强化学习技术训练路由器，显著提升了参数高效微调方法的性能。
...
Method: 提出ReMix方法：1) 使用非可学习路由权重确保所有活跃LoRA同等有效；2) 采用强化学习技术，将监督损失作为奖励，路由器作为策略；3) 使用reinforce leave-one-out(RLOO)技术提供无偏梯度估计器；4) 支持扩展训练计算以提升预测性能。

Result: 大量实验表明，ReMix在激活参数数量相当的情况下，显著优于最先进的参数高效微调方法。

Conclusion: 通过重新思考路由器的基础设计，采用非可学习路由权重和强化学习训练策略，ReMix有效解决了现有Mixture-of-LoRAs模型中路由权重不平衡的问题，提升了模型的表达能力和性能。
...
### [23] [DT-BEHRT: Disease Trajectory-aware Transformer for Interpretable Patient Representation Learning](https://arxiv.org/abs/2603.10180)
*Deyi Li,Zijun Yao,Qi Xu,Muxuan Liang,Lingyao Li,Zijian Xu,Mei Liu*

Main category: cs.LG

TL;DR: DT-BEHRT：一种图增强的序列架构，通过显式建模器官系统内的诊断中心交互和捕获异步进展模式来解耦疾病轨迹，结合轨迹级代码掩码和本体信息祖先预测的预训练方法，在多个基准数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有基于序列、图或图增强序列的EHR预测方法往往忽视了医疗代码因不同临床特征和上下文而产生的异质性角色，无法充分捕捉疾病轨迹的复杂模式。

Method: 提出DT-BEHRT（疾病轨迹感知Transformer），采用图增强序列架构，显式建模器官系统内的诊断中心交互，捕获异步进展模式；设计结合轨迹级代码掩码和本体信息祖先预测的预训练方法，增强表示鲁棒性。

Result: 在多个基准数据集上的广泛实验表明，DT-BEHRT实现了强大的预测性能，并提供与临床医生疾病中心推理一致的可解释患者表示。

Conclusion: DT-BEHRT通过解耦疾病轨迹和增强表示鲁棒性，有效提升了EHR预测模型的性能，同时提供了临床可解释的患者表示，源代码已公开。
...
### [24] [SiMPO: Measure Matching for Online Diffusion Reinforcement Learning](https://arxiv.org/abs/2603.10250)
*Haitong Ma,Chenxiao Gao,Tianyi Chen,Na Li,Bo Dai*

Main category: cs.LG

TL;DR: SiMPO提出一个统一的扩散强化学习框架，通过有符号测度匹配和单调函数重加权，支持负样本反馈，提升策略性能。


<details>
  <summary>Details</summary>
Motivation: 现有扩散策略RL算法通常使用softmax重加权，这会导致策略过于贪婪且无法有效利用负样本反馈，需要更灵活的加权方案。

Method: 提出有符号测度策略优化(SiMPO)：1) 通过f-散度正则化策略优化构建虚拟目标策略，允许有符号目标测度；2) 使用该有符号测度通过重加权匹配指导扩散或流模型。

Result: SiMPO通过灵活的加权方案实现了优越性能，提供了负重加权的理论依据和几何解释，并给出了根据奖励景观选择重加权方法的实用指南。

Conclusion: SiMPO为扩散RL提供了一个统一框架，支持任意单调递增加权函数，能够有效利用负样本反馈，显著提升策略性能。
...
### [25] [Improving TabPFN's Synthetic Data Generation by Integrating Causal Structure](https://arxiv.org/abs/2603.10254)
*Davide Tugnoli,Andrea De Lorenzo,Marco Virgolin,Giovanni Cinà*

Main category: cs.LG

TL;DR: TabPFN生成合成表格数据时，由于自回归特性对特征顺序敏感，当特征顺序与因果结构冲突时会产生虚假相关性。研究提出两种因果结构注入方法：DAG感知条件化和CPDAG策略，以提升合成数据质量和因果效应保持能力。


<details>
  <summary>Details</summary>
Motivation: TabPFN作为表格数据的基础模型，在生成合成数据时存在自回归特性对特征顺序敏感的问题。当特征生成顺序与真实因果结构不一致时，模型会产生虚假相关性，损害合成数据质量和因果效应的保持能力。

Method: 提出两种因果结构注入方法：1) DAG感知条件化：基于因果图，每个变量在其因果父节点条件下采样；2) CPDAG策略：针对部分因果知识场景，使用完成的部分有向无环图指导生成过程。

Result: 在控制基准测试和六个CSuite数据集上的评估显示，DAG感知条件化在大多数设置下相比原始TabPFN显著提升了合成数据的质量和稳定性。CPDAG策略显示出中等程度的改进，其效果取决于定向边的数量。

Conclusion: 将因果结构注入自回归生成过程可以增强合成表格数据的可靠性。DAG感知条件化是有效的改进方法，而CPDAG策略在部分因果知识场景下也有一定效果。
...
### [26] [Discovery of a Hematopoietic Manifold in scGPT Yields a Method for Extracting Performant Algorithms from Biological Foundation Model Internals](https://arxiv.org/abs/2603.10261)
*Ihor Kendiukhov*

Main category: cs.LG

TL;DR: 从scGPT基础模型中提取出首个紧凑的造血算法，通过机制可解释性方法获得，在造血细胞分型任务上表现优于现有方法，且计算效率显著提升。


<details>
  <summary>Details</summary>
Motivation: 探索基础模型内部是否编码了有意义的生物学算法，特别是造血发育的几何结构，并开发从冻结基础模型中提取有用算法的方法。

Method: 提出三阶段提取方法：1) 从冻结注意力权重直接导出操作符；2) 轻量级学习适配器；3) 任务特定读出层。该方法无需目标数据集重新训练，可生成独立算法。

Result: 提取的算法在88个供体保留基准测试中表现最佳，在伪时间排序和关键亚型分类（CD4/CD8 AUROC 0.867，单核/巨噬AUROC 0.951）上领先。相比标准探测方法，在6/8分类端点上显著更好，计算速度快34.5倍，参数减少约1000倍。

Conclusion: 成功从scGPT基础模型中提取出首个具有生物学意义且性能优越的紧凑造血算法，证明基础模型内部确实编码了有用的生物学知识，且提取方法具有通用性。
...
### [27] [Robust Post-Training for Generative Recommenders: Why Exponential Reward-Weighted SFT Outperforms RLHF](https://arxiv.org/abs/2603.10279)
*Keertana Chidambaram,Sanath Kumar Krishnamurthy,Qiuling Xu,Ko-Jen Hsiao,Moumita Bhattacharya*

Main category: cs.LG

TL;DR: 该论文提出了一种基于指数奖励加权监督微调的方法，用于对齐生成式推荐系统与用户偏好，解决了现有方法在规模化生产系统中的问题。


<details>
  <summary>Details</summary>
Motivation: 现有后训练方法不适合生产级推荐系统：RLHF方法因噪声用户反馈和不可靠奖励模型导致奖励攻击问题；离线RL方法需要倾向性分数但实际不可得；在线交互不可行。需要一种简单、可扩展且完全离线的对齐方法。

Method: 提出指数奖励加权监督微调方法，权重w = exp(r/λ)，其中r为观察到的奖励，λ为温度参数。该方法直接在观察到的奖励上优化，无需查询学习到的奖励模型，完全离线且不需要倾向性分数。

Result: 理论证明该方法在噪声奖励下具有策略改进保证，改进差距仅随目录大小对数增长。实验在三个开源数据集和一个专有数据集上验证，该方法简单、可扩展，且一致优于基于RLHF的基线方法。

Conclusion: 指数奖励加权监督微调是生成式推荐系统对齐的有效方法，具有理论保证、实践简单性，并能通过单一可解释的温度参数λ控制鲁棒性与改进的权衡。
...
### [28] [Taming Score-Based Denoisers in ADMM: A Convergent Plug-and-Play Framework](https://arxiv.org/abs/2603.10281)
...
<details>
  <summary>Details</summary>
Motivation: 基于分数的生成模型作为先验在解决逆问题中表现出色，但直接集成到ADMM等优化算法中存在两个核心挑战：1) 训练分数函数使用的噪声数据流形与ADMM迭代几何不匹配（特别是对偶变量的影响）；2) 配备基于分数的去噪器的ADMM缺乏收敛性理解。
...
Result: 建立了两个收敛性结果：1) 在适当的去噪器参数下，每个ADMM迭代是弱非扩张算子，使用恒定步长确保高概率固定点球收敛；2) 在更宽松条件下，AC-DC去噪器是有界去噪器，在自适应步长调度下实现收敛。实验表明该方法在多种逆问题上一致优于基线。

Conclusion: 提出的ADMM-PnP with AC-DC denoiser框架有效解决了基于分数的生成模型在ADMM优化中的流形不匹配问题，并提供了理论收敛保证，在多种逆问题上展现出优越性能。
...
### [29] [GSVD for Geometry-Grounded Dataset Comparison: An Alignment Angle Is All You Need](https://arxiv.org/abs/2603.10283)
*Eduarda de Souza Marques,Arthur Sobrinho Ferreira da Rocha,Joao Paixao,Heudson Mirandola,Daniel Sadoc Menasche*

Main category: cs.LG

TL;DR: 该论文提出了一种基于几何的线性关系比较方法，使用广义奇异值分解(GSVD)分析两个数据集，并推导出可解释的角度评分θ(z)作为样本级几何诊断工具。


<details>
  <summary>Details</summary>
Motivation: 几何基础学习要求模型尊重问题域的结构，而不是将观测视为任意向量。受此启发，作者重新审视了比较数据集的经典但未被充分利用的原语：两个数据矩阵之间的线性关系，通过共享环境空间中的共跨度约束Ax=By=z来表达。

Method: 使用广义奇异值分解(GSVD)作为两个子空间的联合坐标系，利用GSVD形式A=HCU、B=HSV，其中CᵀC+SᵀS=I，通过(C,S)的对角结构分离共享方向和数据集特定方向。从这些因子推导出可解释的角度评分θ(z)∈[0,π/2]，量化样本z是否更多由A解释、更多由B解释或由两者相当地解释。

Result: 通过角度分布和代表性GSVD方向在MNIST数据集上展示了角度评分的行为。提出了基于θ(z)的二元分类器作为评分作为可解释诊断工具的示例应用。

Conclusion: 角度评分θ(z)主要作为样本级几何诊断工具，提供了一种可解释的方法来比较两个数据集，识别样本是由哪个数据集相对更好地解释，或由两者共同解释。
...
TL;DR: FairFAL是一个自适应的联邦主动学习框架，通过预测差异推断全局不平衡和本地-全局差异，自适应选择查询模型，使用原型引导的伪标注促进类别感知查询，并应用两阶段不确定性-多样性平衡采样策略，在长尾和非独立同分布设置下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 联邦主动学习在现实场景中面临严重全局类别不平衡和高度异构客户端的问题，导致性能下降。现有方法缺乏对这些挑战的系统研究，特别是在查询模型选择方面。
...
Conclusion: FairFAL通过自适应查询模型选择和类别公平的采样策略，有效解决了联邦主动学习中的全局类别不平衡和客户端异构性问题，为现实场景中的联邦主动学习提供了有效的解决方案。
...
TL;DR: 提出因果概念图（CCG）方法，通过有向无环图表示稀疏可解释特征间的因果依赖关系，用于分析语言模型中的多步推理过程。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器能定位语言模型中的概念位置，但无法捕捉多步推理过程中概念间的交互关系，需要开发能建模概念间因果依赖的方法。

Method: 结合任务条件化稀疏自编码器进行概念发现，使用DAGMA风格的可微分结构学习进行图恢复，并引入因果保真度分数（CFS）评估图引导干预的效果。

Result: 在ARC-Challenge、StrategyQA和LogiQA数据集上使用GPT-2 Medium模型，CCG获得CFS=5.654±0.625，显著优于ROME风格追踪（3.382±0.233）、仅SAE排序（2.479±0.196）和随机基线（1.032±0.034）。

Conclusion: CCG能有效建模语言模型中概念间的因果依赖关系，学习到的图结构稀疏（5-6%边密度）、领域特定且在不同随机种子下稳定。
...
### [32] [Optimal Expert-Attention Allocation in Mixture-of-Experts: A Scalable Law for Dynamic Model Design](https://arxiv.org/abs/2603.10379)
*Junzhuo Li,Peijie Jiang,Changxin Tian,Jia Liu,Zhiqiang Zhang,Xuming Hu*

Main category: cs.LG

TL;DR: 该论文将神经缩放定律扩展到MoE模型，研究了专家层与注意力层之间的计算分配最优比例，并建立了该比例与总计算量之间的幂律关系。


<details>
  <summary>Details</summary>
Motivation: 随着MoE架构成为扩展模型容量而不成比例增加计算的有效方法，确定专家层与注意力层之间的最优计算分配比例变得至关重要。现有缩放定律主要关注模型大小和数据，而忽略了MoE架构中专家与注意力子层之间的计算分配问题。

Method: 通过GPT风格的MoE Transformer进行大量实验，定义专家层计算占比r（专家层FLOPs占总FLOPs的比例），研究该比例与总计算预算和模型稀疏度的相互作用关系。

Result: 经验发现最优比例r*与总计算量遵循幂律关系，且随稀疏度变化。推导出r*的显式公式，能够精确控制专家-注意力计算分配。将Chinchilla缩放定律推广到包含这一架构参数。

Conclusion: 为设计高效MoE模型提供实用指导，在固定计算预算下优化性能。建立了超越模型大小和数据的MoE模型调优新框架。
...
### [33] [Regime-aware financial volatility forecasting via in-context learning](https://arxiv.org/abs/2603.10299)
*Saba Asaad,Shayan Mohajer Hamidi,Ali Bereyhi*

Main category: cs.LG

TL;DR: 提出一种基于大语言模型的机制感知上下文学习框架，用于非平稳市场条件下的金融波动率预测，通过机制感知演示样本实现无参数微调的适应性预测


<details>
  <summary>Details</summary>
Motivation: 传统波动率预测方法在非平稳市场条件下表现不佳，需要能够适应不同市场机制的预测框架。大语言模型具有强大的上下文推理能力，但需要机制感知的引导才能有效应用于金融时间序列预测

Method: 1) 使用预训练大语言模型作为上下文学习器；2) 开发基于oracle指导的机制感知演示构建方法；3) 根据估计的市场机制标签条件采样演示样本；4) 模型仅通过上下文推理调整预测，无需参数微调

Result: 在多个金融数据集上的实验表明，该机制感知上下文学习框架优于传统波动率预测方法和直接单样本学习，特别是在高波动率期间表现更佳

Conclusion: 机制感知上下文学习框架能够有效利用大语言模型的推理能力适应非平稳市场条件，为金融波动率预测提供了新的无参数微调解决方案
...
### [34] [How to make the most of your masked language model for protein engineering](https://arxiv.org/abs/2603.10302)
*Calvin McCarter,Nick Bhattacharya,Sebastian W. Ober,Hunter Elliott*

Main category: cs.LG

TL;DR: 提出一种用于掩码语言模型的灵活采样方法，通过系统评估在抗体工程中验证其有效性


<details>
  <summary>Details</summary>
Motivation: 尽管近年来涌现了大量蛋白质语言模型，但如何从这些模型中采样以优化特定生物学特性的研究相对较少。本文旨在填补这一空白，特别是在抗体治疗的实际应用中。

Method: 提出随机束搜索采样方法，利用MLMs高效评估序列1-编辑邻域的伪困惑度，将生成问题重构为全序列评估，实现多目标优化的灵活指导。

Result: 通过体外头对头评估发现，采样方法的选择对结果的影响至少与模型选择同等重要，这为这一未充分探索的领域提供了研究动机。

Conclusion: 采样方法在蛋白质语言模型应用中具有关键作用，随机束搜索采样为优化生物学特性提供了有效工具，未来需要更多研究关注采样策略的改进。
...
TL;DR: STemDist：首个专门针对时空时间序列预测的数据集蒸馏方法，通过平衡压缩时空维度，在保持预测性能的同时大幅降低训练时间和内存消耗。


<details>
  <summary>Details</summary>
Motivation: 随着时空数据集规模和模型复杂度的增长，深度学习模型训练变得时间和资源密集。现有的数据集蒸馏方法只压缩单一维度，不适合时空数据集，因为时空维度共同贡献了大量数据。

Method: 提出STemDist方法：1）平衡压缩时空维度；2）在聚类级别而非单个位置级别进行蒸馏以降低成本；3）结合基于子集的细粒度蒸馏技术提升预测性能。

Result: 在五个真实世界数据集上的实验表明，STemDist蒸馏的数据集相比通用和时间序列数据集蒸馏方法：1）训练速度提升高达6倍；2）内存效率提升高达8倍；3）预测误差降低高达12%。

Conclusion: STemDist是首个专门针对时空时间序列预测的数据集蒸馏方法，能有效平衡压缩时空维度，显著降低训练成本同时保持甚至提升预测性能。
...
### [36] [Data-Driven Integration Kernels for Interpretable Nonlocal Operator Learning](https://arxiv.org/abs/2603.10305)
*Savannah L. Ferretti,Jerry Lin,Sara Shamekh,Jane W. Baldwin,Michael S. Pritchard,Tom Beucler*

Main category: cs.LG

TL;DR: 论文提出数据驱动的积分核框架，通过将非局部信息聚合与局部非线性预测分离，为机器学习模型添加结构约束，提高可解释性并减少过拟合。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型能够表示气候过程中在水平空间、高度和时间上的非局部关系，但高度非线性的组合方式使得学习到的关系难以解释，且随着非局部信息范围的扩大容易过拟合。
...
Conclusion: 数据驱动的积分核框架通过分离非局部信息聚合和局部非线性预测，为机器学习模型提供了结构约束，既保持了预测性能又提高了可解释性，同时减少了过拟合风险。
...
TL;DR: 论文提出LLM表示中存在显著的各向异性问题，在低比特训练中导致数值不稳定，主要原因是秩一的均值偏差，通过简单的均值减除操作可有效恢复稳定性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言训练中表现出明显的各向异性：少数方向集中了不成比例的能量，而其余维度形成广泛的语义尾部。在低比特训练中，这种几何结构变得数值不稳定，因为块量化尺度由极端元素幅度决定，主导方向拉伸动态范围，将长尾语义变化压缩到狭窄的数值区间。

Method: 研究发现不稳定性主要由一个相干的秩一均值偏差驱动，这构成了LLM表示中光谱各向异性的主导成分。这种均值成分在层和训练阶段中系统性地出现，并占据了极端激活幅度的大部分。由于主导不稳定性是秩一的，可以通过简单的源级均值减除操作消除。这种基于偏差的调节恢复了SVD光谱方法的大部分稳定性优势，同时只需要归约操作和标准量化内核。

Result: 在FP4（W4A4G4）训练中的实证结果显示，均值移除显著缩小了与BF16的损失差距，并恢复了下游性能，为稳定的低比特LLM训练提供了硬件高效的路径。

Conclusion: LLM表示中的各向异性问题在低比特训练中导致数值不稳定，主要原因是秩一的均值偏差。通过简单的均值减除操作可以有效解决这一问题，提供了一种硬件高效的方法来实现稳定的低比特LLM训练。
...
### [38] [UAV-MARL: Multi-Agent Reinforcement Learning for Time-Critical and Dynamic Medical Supply Delivery](https://arxiv.org/abs/2603.10528)
*Islam Guven,Mehmet Parlak*

Main category: cs.LG

TL;DR: 本文提出了一种基于多智能体强化学习（MARL）的无人机医疗配送协调框架，用于在不确定条件下优化医疗物资配送的优先级和资源分配。


<details>
  <summary>Details</summary>
Motivation: 无人机在紧急医疗物资配送中应用日益广泛，但需要有效的协调机制来处理不同紧急程度、位置和截止时间的医疗请求，并在不确定的操作条件下优化有限的空中资源分配。

Method: 将问题建模为部分可观测马尔可夫决策过程（POMDP），采用近端策略优化（PPO）作为主要学习算法，评估了异步扩展、经典actor-critic方法和架构修改等多种变体，并使用OpenStreetMap的真实地理数据进行评估。

Result: 实验结果表明，经典PPO相比异步和顺序学习策略实现了更优的协调性能，验证了强化学习在自适应、可扩展的无人机辅助医疗物流中的潜力。

Conclusion: 该框架为医疗任务优先级排序、无人机资源实时重分配提供了决策支持层，展示了多智能体强化学习在优化紧急医疗物流协调方面的有效性和可扩展性。
...
TL;DR: 该论文提出了一种基于损失方差感知的自适应加权策略，用于解决扩散模型在不同噪声水平上训练动态不平衡的问题，从而改善优化效率和训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成建模方面取得了显著成功，但不同噪声水平下的训练动态存在高度不平衡，这可能导致优化效率低下和学习行为不稳定。作者从log-SNR水平上损失方差的角度研究这种不平衡问题。

Method: 提出了一种方差感知的自适应加权策略，该方法基于观察到的方差分布动态调整训练权重，鼓励在噪声水平之间实现更平衡的优化过程。

Result: 在CIFAR-10和CIFAR-100上的大量实验表明，该方法相比标准训练方案持续改善了生成性能，实现了更低的Fréchet Inception Distance（FID），同时减少了随机种子间的性能方差。损失-log-SNR可视化、方差热图和消融研究进一步表明自适应加权有效稳定了训练动态。

Conclusion: 方差感知训练策略在改善扩散模型优化方面具有潜力，能够解决训练动态不平衡问题，提高优化效率和训练稳定性。
...
TL;DR: 本文提出了梯度流漂移的数学框架，证明了漂移模型与前向KL散度在核密度估计近似下的Wasserstein梯度流的等价性，并扩展了混合散度策略到黎曼流形上。


<details>
  <summary>Details</summary>
Motivation: 建立漂移模型与Wasserstein梯度流之间的精确数学联系，为生成模型提供统一的理论框架，同时解决模式塌缩和模式模糊问题。

Method: 提出梯度流漂移框架，证明漂移场等于KDE对数密度梯度差，将MMD生成器纳入该框架，提出混合散度策略（反向KL和χ²散度），并扩展到黎曼流形。

Result: 证明了漂移模型与Wasserstein-2梯度流的等价性，建立了统一的理论框架，混合散度策略能同时避免模式塌缩和模式模糊，黎曼流形扩展放宽了核函数约束。

Conclusion: 梯度流漂移框架为生成模型提供了统一的理论基础，混合散度策略有效解决了生成模型的常见问题，黎曼流形扩展使方法更适用于语义空间，合成基准实验验证了框架有效性。
...
### [41] [Graph-GRPO: Training Graph Flow Models with Reinforcement Learning](https://arxiv.org/abs/2603.10395)
*Baoheng Zhu,Deyu Bo,Delvin Ce Zhang,Xiao Wang*

Main category: cs.LG

TL;DR: 提出Graph-GRPO框架，使用在线强化学习训练图流模型，通过解析推导转移概率和局部扰动策略，实现对复杂人类偏好和任务特定目标的优化。


<details>
  <summary>Details</summary>
Motivation: 图生成在药物发现等领域有广泛应用，但现有的图流模型难以有效对齐复杂的人类偏好或任务特定目标，需要开发能够处理可验证奖励的训练框架。

Method: 提出Graph-GRPO框架：1) 推导图流模型转移概率的解析表达式，替代蒙特卡洛采样，实现完全可微的RL训练；2) 提出细化策略，随机扰动图中的特定节点和边并重新生成，实现局部探索和生成质量自我提升。

Result: 在合成和真实数据集上的实验表明，仅用50个去噪步骤，在平面图和树数据集上分别达到95.0%和97.5%的有效-唯一-新颖性分数；在分子优化任务上达到最先进性能，优于基于图的RL方法、基于片段的RL方法以及经典遗传算法。

Conclusion: Graph-GRPO是一个有效的在线强化学习框架，能够成功训练图流模型以满足可验证的奖励要求，在多个图生成任务上表现出优越性能。
...
### [42] [Reinforcement Learning with Conditional Expectation Reward](https://arxiv.org/abs/2603.10624)
*Changyi Xiao,Caijun Xu,Yixin Cao*

Main category: cs.LG

TL;DR: 本文提出了一种名为条件期望奖励（CER）的新方法，利用大语言模型本身作为隐式验证器，解决了传统RLVR方法在自由形式答案推理任务中依赖手工规则验证器的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统的强化学习与可验证奖励（RLVR）方法在数学等领域有效，但在自由形式答案的一般推理领域中，由于答案变化多样，难以建立完整准确的手工验证规则，限制了其应用范围。

Method: 提出条件期望奖励（CER）方法，将大语言模型本身作为隐式验证器，通过计算在生成答案条件下生成参考答案的期望似然来提供软性的、分级的奖励信号，无需外部验证器或辅助模型。

Result: 实验结果表明CER在广泛的推理任务中有效，涵盖数学和一般领域，表明CER作为一种灵活通用的验证机制具有良好性能。

Conclusion: CER通过利用大语言模型自身作为验证器，解决了传统RLVR方法在一般推理领域的局限性，提供了一种适用于多种任务且无需外部验证器的通用验证机制。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型能准确预测蛋白质性质，但缺乏对蛋白质工程的机制性见解和可操作指导。当模型标记抗体不稳定时，蛋白质工程师无法知道哪些突变能在保持功能的同时恢复稳定性。
...
### [44] [$V_{0.5}$: Generalist Value Model as a Prior for Sparse RL Rollouts](https://arxiv.org/abs/2603.10848)
*Yi-Kai Zhang,Yueqing Sun,Hongyan Hao,Qi Gu,Xunliang Cai,De-Chuan Zhan,Han-Jia Ye*

Main category: cs.LG

TL;DR: 提出V_{0.5}方法，通过自适应融合预训练价值模型的基线预测（作为先验）与稀疏rollouts的经验均值，构建低方差、计算高效的鲁棒基线，使用实时统计测试和动态预算分配平衡方差与偏差。


<details>
  <summary>Details</summary>
Motivation: 在可验证奖励的强化学习中，构建鲁棒的优势基线对策略梯度至关重要。现有通用价值模型（如V_0）虽然能通过上下文编码实现预训练价值估计，但存在系统性偏差或幻觉问题，而稀疏采样则会导致高方差。

Method: 提出V_{0.5}方法：1）自适应融合预训练价值模型的基线预测（先验）与稀疏rollouts的经验均值；2）引入实时统计测试评估先验可靠性；3）动态分配额外rollout预算以最小化基线估计器的均方误差。

Result: 在六个数学推理基准测试中，V_{0.5}显著优于GRPO和DAPO，实现更快收敛和超过10%的性能提升，即使在组大小为4的极端稀疏条件下也能保证稳定的策略梯度。

Conclusion: V_{0.5}通过平衡预训练价值模型的先验知识与稀疏采样的经验数据，构建了计算高效且方差极低的鲁棒基线，为可验证奖励强化学习中的策略梯度优化提供了有效解决方案。
...
### [45] [Dynamics-Predictive Sampling for Active RL Finetuning of Large Reasoning Models](https://arxiv.org/abs/2603.10887)
*Yixiu Mao,Yun Qu,Qi Wang,Heming Zou,Xiangyang Ji*

Main category: cs.LG

TL;DR: DPS是一种新的在线提示选择方法，通过预测学习动态来减少RL微调中的冗余rollout计算开销，在保持性能的同时显著加速训练过程。


<details>
  <summary>Details</summary>
Motivation: 现有的在线提示选择方法虽然能加速RL微调，但需要大量LLM rollout来计算候选样本的信息量，这种计算开销可能超过微调过程本身，因此需要更高效的提示选择策略。

Method: 提出Dynamics-Predictive Sampling (DPS)，将每个提示在RL微调中的解决进度建模为动态系统，其中解决程度表示为状态，转移由隐马尔可夫模型表征。利用历史rollout奖励信号进行在线贝叶斯推断来估计演化状态分布，推断结果为高效提示选择提供预测先验，无需rollout密集型筛选。

Result: 在数学、规划和视觉几何等多种推理任务上的实证结果表明，DPS显著减少了冗余rollout，加速了训练过程，并实现了优越的推理性能。

Conclusion: DPS通过预测学习动态来高效选择信息丰富的提示，解决了现有在线提示选择方法计算开销过大的问题，为RL微调提供了一种更高效的数据选择策略。
...
### [46] [Muscle Synergy Priors Enhance Biomechanical Fidelity in Predictive Musculoskeletal Locomotion Simulation](https://arxiv.org/abs/2603.10474)
*Ilseung Park,Eunsik Choi,Jangwhan Ahn,Jooeun Ahn*

Main category: cs.LG

TL;DR: 提出一种基于生理学约束的强化学习框架，通过肌肉协同作用控制三维人体行走模型，在多种速度和地形条件下生成稳定步态，提高了生物力学保真度和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 人体运动源于高维神经肌肉控制，使得预测性肌肉骨骼模拟具有挑战性。传统方法难以在有限实验数据下生成具有生理学合理性的运动控制。

Method: 从少量地面行走试验的逆向肌肉骨骼分析中提取低维协同基，将其作为三维肌肉驱动模型的动作空间，通过强化学习训练控制器适应不同速度、坡度和不平坦地形。
...
### [47] [LookaheadKV: Fast and Accurate KV Cache Eviction by Glimpsing into the Future without Generation](https://arxiv.org/abs/2603.10899)
*Jinwoo Ahn,Ingyu Seong,Akhil Kedia,Junhan Kim,Hyemi Jang,Kangwook Lee,Yongkweon Jeon*

Main category: cs.LG

TL;DR: LookaheadKV是一个轻量级的KV缓存淘汰框架，无需显式生成草稿就能利用未来响应信息，显著降低长上下文推理的计算开销。


<details>
  <summary>Details</summary>
Motivation: Transformer大语言模型使用KV缓存提高自回归推理效率，但缓存大小随输入序列长度线性增长，成为长上下文任务的瓶颈。现有解决方案通过淘汰不重要的提示KV来缓解问题，但依赖计算昂贵的草稿生成来"窥视未来"，导致显著的预填充开销。

Method: 提出LookaheadKV框架，通过向Transformer层添加参数高效的模块，训练这些模块来准确预测真实重要性分数，无需显式生成草稿响应。设计确保运行时开销可忽略，与现有廉价启发式方法相当。

Result: 在多种模型的长上下文理解基准测试中，该方法不仅在各种长上下文理解任务中优于最近的竞争基线，还将淘汰成本降低高达14.5倍，显著缩短首个令牌生成时间。

Conclusion: LookaheadKV成功解决了KV缓存淘汰中草稿生成开销大的问题，实现了轻量级、高效的长上下文推理优化，具有实际部署价值。
...
### [48] [A Universal Nearest-Neighbor Estimator for Intrinsic Dimensionality](https://arxiv.org/abs/2603.10493)
*Eng-Jon Ong,Omer Bobrowski,Gesine Reinert,Primoz Skraba*

Main category: cs.LG

TL;DR: 提出一种基于最近邻距离比率的本征维度估计器，计算简单且性能达到SOTA，理论证明该估计器具有普适性，不依赖于数据分布。


<details>
  <summary>Details</summary>
Motivation: 本征维度估计是机器学习和计算机视觉中的基础问题，现有方法通常依赖几何或分布假设，当这些假设不成立时性能会显著下降。需要开发一种不依赖特定假设的普适性估计器。

Method: 基于最近邻距离比率的ID估计器，通过简单的距离比率计算来估计本征维度，避免了复杂的几何或分布假设。

Result: 在基准流形和真实世界数据集上的实验表明，该估计器性能达到最先进水平。理论分析证明该估计器具有普适性，能够独立于数据生成分布收敛到真实ID。

Conclusion: 提出的基于最近邻距离比率的ID估计器计算简单、性能优越，且理论保证其普适性，为不依赖特定假设的本征维度估计提供了有效解决方案。
...
### [49] [Historical Consensus: Preventing Posterior Collapse via Iterative Selection of Gaussian Mixture Priors](https://arxiv.org/abs/2603.10935)
*Zegu Zhang,Jian Zhang*

Main category: cs.LG

TL;DR: 提出历史共识训练方法，通过利用高斯混合模型聚类的多样性来完全消除VAE后验坍塌问题，无需架构约束或超参数调优。


<details>
  <summary>Details</summary>
Motivation: 变分自编码器经常遭受后验坍塌问题，导致潜在变量变得无信息且近似后验退化为先验。现有方法通过架构约束或超参数调优来避免坍塌，但作者希望从根本上消除坍塌的可能性。

Method: 提出历史共识训练，这是一种迭代选择过程，通过交替优化和选择逐步细化一组候选GMM先验。关键洞察是：满足多个不同聚类约束训练的模型会形成历史屏障——参数空间中即使后续使用单一目标训练也能保持稳定的区域。

Result: 证明了该屏障排除了坍塌解，并在合成和真实数据集上进行了广泛实验，表明该方法无论解码器方差或正则化强度如何都能实现非坍塌表示。该方法不需要显式稳定性条件，适用于任意神经架构。

Conclusion: 通过利用GMM聚类的多样性，历史共识训练能够完全消除VAE后验坍塌问题，提供了一种无需架构约束或超参数调优的稳健解决方案。
...
### [50] [World Model for Battery Degradation Prediction Under Non-Stationary Aging](https://arxiv.org/abs/2603.10527)
*Kai Chin Lim,Khay Wai See*

Main category: cs.LG

TL;DR: 将电池退化预测建模为世界模型问题，通过编码循环数据到潜在状态并学习动态转移来预测未来80个循环的SOH轨迹，相比直接回归方法误差减半。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动方法虽然能通过直接回归产生轨迹输出，但缺乏随时间推移传播退化动态的机制。需要一种能够编码电池循环数据并学习动态转移的方法来改进退化预测。

Method: 将电池退化预测构建为世界模型问题：1）将每个循环的电压、电流和温度时间序列编码为潜在状态；2）通过学习动态转移将潜在状态向前传播；3）引入单粒子模型（SPM）约束到训练损失中，研究电化学知识是否改善学习动态；4）在138个电池的Severson LiFePO4数据集上评估三种配置。

Result: 迭代展开相比相同编码器的直接回归，轨迹预测误差减半。SPM约束在退化拐点处（电阻与SOH关系最适用时）改善了预测，但不改变总体精度。

Conclusion: 将电池退化预测构建为世界模型问题能有效学习动态转移，显著改善轨迹预测精度。电化学知识约束在特定退化阶段（如拐点）能提供额外改进，为电池健康管理提供了更准确的预测框架。
...
### [51] [Safe RLHF Beyond Expectation: Stochastic Dominance for Universal Spectral Risk Control](https://arxiv.org/abs/2603.10938)
*Yaswanth Chittepu,Ativ Joshi,Rajarshi Bhattacharjee,Scott Niekum*

Main category: cs.LG

TL;DR: 本文提出RAD框架，用一阶随机占优约束替代期望成本约束，通过最优传输框架实现风险敏感的强化学习对齐，能更好地控制尾部风险和分布外失败。


<details>
  <summary>Details</summary>
Motivation: 传统的基于期望成本约束的RLHF方法只关注成本分布的单统计量，无法处理分布不确定性，特别是在重尾分布或罕见灾难事件情况下。这在对鲁棒性和风险敏感性要求高的场景中存在问题。
...
Conclusion: RAD框架通过随机占优约束提供了更原则性的风险敏感对齐方法，能够更好地控制尾部风险和分布外失败，为调整模型风险偏好提供了理论机制。
...
### [52] [Tackling Length Inflation Without Trade-offs: Group Relative Reward Rescaling for Reinforcement Learning](https://arxiv.org/abs/2603.10535)
*Zichao Li,Jie Lou,Fangchen Dong,Zhiyuan Fan,Mengjie Ren,Hongyu Lin,Xianpei Han,Debing Zhang,Le Sun,Yaojie Lu,Xing Yu*

Main category: cs.LG

TL;DR: GR³通过乘法重缩放范式解决强化学习中LLM的长度膨胀问题，相比现有方法更通用且无损


<details>
  <summary>Details</summary>
Motivation: 强化学习虽然能增强LLM能力，但存在长度膨胀问题：模型倾向于使用冗长或低效推理来最大化奖励。现有方法要么引入补偿效应导致优化捷径，要么缺乏超越二元反馈的通用性。

Method: 提出Group Relative Reward Rescaling (GR³)，将长度控制重构为乘法重缩放范式，建立通用、连续、奖励依赖的门控机制。同时结合组相对正则化和优势感知校准，动态调整长度预算以适应实例难度，并保留高质量轨迹的优势信号。

Result: 在RLHF和RLVR设置下，GR³在保持训练动态和下游性能与标准GRPO相当的同时，显著缓解了长度膨胀问题，优于现有的长度正则化基线方法。

Conclusion: GR³提供了一种通用且无损的方法来解决强化学习中LLM的长度膨胀问题，通过乘法重缩放范式和自适应校准机制，在保持性能的同时有效控制输出长度。
...
TL;DR: 该论文提出了一种基于强化学习的方法，用于优化集群调度器中评分函数的权重分配，以提高作业端到端性能。


<details>
  <summary>Details</summary>
Motivation: 传统集群调度器通常对所有评分函数的结果进行等权重处理，这种一刀切的方法没有考虑不同工作负载的特性，可能导致次优部署。手动调整权重需要专业知识且计算成本高。

Method: 采用强化学习方法学习调度器评分算法中的权重，主要创新包括：1) 使用百分比改进奖励来处理多步参数调整目标；2) 引入帧堆叠技术以在优化实验中传递信息；3) 限制领域信息以防止过拟合并提高在未见集群和工作负载上的性能。

Result: 在实验室无服务器场景中，该方法相比固定权重平均提高了33%的性能，相比最佳基线方法提高了12%的性能。

Conclusion: 提出的强化学习方法能够有效优化集群调度器评分函数的权重分配，显著提升作业性能，且具有良好的泛化能力。
...
TL;DR: 使用机器学习框架研究中美股市的跨市场收益预测性，发现美国市场对A股有显著预测能力，但反向预测效果有限。


<details>
  <summary>Details</summary>
Motivation: 研究跨市场收益预测性，利用中美股市交易时间不重叠的特点，构建经济结构保持的机器学习框架来发现跨市场依赖关系。

Method: 构建有向二分图捕捉跨市场股票的时间顺序预测联系，通过滚动窗口假设检验选择边，作为稀疏可解释的特征选择层，应用正则化和集成方法预测开盘至收盘收益。

Result: 发现明显的方向不对称性：美国市场前收盘至收盘收益对中国市场日内收益有显著预测信息，但反向效应有限；这种信息不对称转化为经济上有意义的性能差异。

Conclusion: 结构化机器学习框架能够在保持可解释性的同时揭示跨市场依赖关系，美国市场对中国市场的信息传导更强，体现了市场间的信息不对称性。
...
TL;DR: 提出了一种基于黎曼几何保持的变分自编码器（RGP-VAE），用于生成运动想象脑机接口应用中的合成EEG协方差矩阵，保持矩阵的对称正定性，并学习主体不变的潜在空间。
...
Method: 提出了黎曼几何保持变分自编码器（RGP-VAE），整合了几何映射和复合损失函数，结合黎曼距离、切空间重建精度和生成多样性。

Result: 模型能够生成有效且具有代表性的EEG协方差矩阵，同时学习主体不变的潜在空间。合成数据对MI-BCI具有实际应用价值，其效果取决于配对的分类器。

Conclusion: 该工作引入并验证了RGP-VAE作为EEG协方差矩阵的几何保持生成模型，突出了其在信号隐私、可扩展性和数据增强方面的潜力。
...
TL;DR: 本文研究了Transformer的上下文学习机制，通过二元假设检验的统计决策理论框架，发现模型能够近似贝叶斯最优充分统计量，而非简单的核平滑相似性匹配。


<details>
  <summary>Details</summary>
Motivation: 尽管上下文学习允许Transformer无需权重更新就能适应新任务，但其底层算法机制仍不明确。作者希望通过统计决策理论视角，在已知算法真实情况的数学严谨框架下，深入理解上下文学习的工作原理。

Method: 采用统计决策理论框架，研究简单的二元假设检验问题（最优策略由似然比检验决定）。训练Transformer处理两种不同几何结构的任务：线性均值偏移任务和非线性方差估计任务。利用logit lens和电路对齐等方法进行机制分析。

Result: Transformer能够从上下文中近似贝叶斯最优充分统计量（达到单调变换的程度），在非线性区域匹配理想oracle估计器的性能。机制分析表明，模型不依赖固定的核平滑启发式方法，而是自适应地调整决策点：线性任务表现出投票式集成模式，非线性任务则使用更深层的顺序计算。
...
TL;DR: HAPEns是一种后处理集成方法，在表格数据上平衡预测性能与硬件效率，通过多目标优化构建多样化的集成模型集合


<details>
  <summary>Details</summary>
Motivation: 集成学习在表格数据上能提升预测性能和鲁棒性，但更大的集成模型会增加硬件需求，需要平衡准确性与硬件效率

Method: 受多目标和质量多样性优化启发，HAPEns沿着预测性能和资源使用的帕累托前沿构建多样化集成集合，采用静态多目标加权方案改进贪婪集成算法

Result: 在83个表格分类数据集上的实验表明，HAPEns显著优于基线方法，在集成性能和部署成本之间找到更优权衡，内存使用是特别有效的目标指标

Conclusion: HAPEns成功平衡了预测性能与硬件效率，为硬件感知的后处理集成提供了有效解决方案，即使简单的贪婪算法也能通过多目标加权得到显著改进
...
TL;DR: Riemannian MeanFlow (RMF) 是一种在黎曼流形上进行单步采样的生成模型，通过并行传输定义平均速度场，避免数值积分，显著降低采样成本。


<details>
  <summary>Details</summary>
Motivation: 虽然流匹配可以在黎曼流形上进行免模拟训练，但采样通常仍需要数值积分概率流ODE，计算成本高。需要一种更高效的采样方法。

Method: RMF通过并行传输定义平均速度场，推导黎曼流形上的MeanFlow恒等式，使用对数映射切线表示避免轨迹模拟和复杂几何计算，采用冲突感知多任务学习稳定优化。

Result: 在球面、环面和SO(3)上的实验表明，RMF实现了竞争性的单步采样，改善了质量-效率权衡，并大幅降低了采样成本。

Conclusion: RMF为黎曼流形上的生成建模提供了一种高效的单步采样方法，通过平均速度场和冲突感知优化实现了更好的质量-效率平衡。
...
<details>
  <summary>Details</summary>
Motivation: 数据泄露问题影响了294篇已发表的论文，覆盖17个科学领域。当前的主要应对措施（检查清单、代码检查工具、最佳实践指南等文档化方法）无法有效防止这些失败。需要一种结构化的解决方案来从根本上解决数据泄露问题。

Method: 提出了一种语法，将监督学习生命周期分解为7个核心原语，通过类型化有向无环图连接。设置了4个硬约束，在调用时拒绝两类最有害的泄露：1）终端评估约束：运行时强制的评估/测试边界，通过名义上不同的Evidence类型保护来拒绝重复测试集评估；2）其他三个约束来防止其他类型的泄露。

Result: 伴随研究在2,047个实验实例中量化了泄露的影响：选择泄露使性能膨胀d_z=0.93，记忆泄露使性能膨胀d_z=0.53-1.11。三个独立的实现（Python、R和Julia）验证了该方法的有效性。附录规范允许任何人构建符合要求的版本。

Conclusion: 该论文提出的结构化语法方法能够有效防止机器学习中的数据泄露问题，相比传统的文档化方法提供了更可靠的解决方案。通过运行时约束和类型系统保护，可以从根本上拒绝最有害的泄露类型。
...
TL;DR: SP-FL提出了一种新的无线联邦学习框架，通过优先传输重要梯度信息（梯度符号）并采用分层资源分配策略，在资源受限的无线环境中提高联邦学习的可靠性和性能。


<details>
  <summary>Details</summary>
Motivation: 无线联邦学习面临无线资源有限导致的通信不可靠问题，这严重影响了模型训练效果。需要设计一种能够优先传输重要梯度信息的方法来提高无线联邦学习的可靠性。

Method: 提出Sign-Prioritized FL框架：1) 将梯度符号和模数分开传输，优先传输梯度符号；2) 如果模数无法正确恢复，允许重用梯度符号进行梯度下降；3) 设计分层资源分配问题，在设备和数据包两个层面优化带宽和功率分配；4) 分析SP-FL的一步收敛行为，提出基于牛顿-拉弗森方法和连续凸近似的交替优化算法。

Result: 在CIFAR-10数据集上的仿真结果表明，SP-FL在资源受限场景下表现优异，相比现有方法测试准确率最高可提升9.96%。
...
TL;DR: 提出基于瞬态不稳定性的可解释机制感知前兆，用于高维混沌系统中极端事件的长期预测，相比传统方法显著延长预测时间


<details>
  <summary>Details</summary>
Motivation: 高维混沌系统中的极端事件预测面临根本性挑战，因为这类事件罕见、间歇性发生，且源于难以从有限观测中推断的瞬态动力学机制。实时预测需要编码驱动极端事件机制的前兆，而不仅仅是依赖统计关联。

Method: 提出完全数据驱动的框架，通过显式跟踪事件发生前的瞬态不稳定性来构建可解释的机制感知前兆。采用降阶公式直接从状态快照计算类似有限时间Lyapunov指数(FTLE)的前兆，无需已知控制方程。为避免经典FTLE计算的过高计算成本，在自适应演化的低维子空间中评估不稳定性增长，该子空间由最优时间依赖(OTD)模态张成，从而高效识别瞬态放大方向。然后将这些前兆作为输入提供给基于Transformer的模型，实现极端事件可观测量的预测。

Result: 在Kolmogorov流（间歇性湍流的典型模型）上验证了该框架。结果表明，显式编码瞬态不稳定性机制相比基于可观测量的基线方法，显著延长了实际预测时间范围。

Conclusion: 该研究提出了一种创新的数据驱动框架，通过机制感知的前兆构建实现了高维混沌系统中极端事件的长期预测，为理解复杂系统中的极端事件动力学提供了新工具。
...
TL;DR: 本文提出了一种基于随机平滑的轨迹预测模型防御机制，能有效提升模型对抗攻击的鲁棒性而不影响正常场景下的预测精度。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中准确鲁棒的轨迹预测至关重要，但现有研究表明即使最先进的预测模型也极易受到对抗攻击的轻微扰动。尽管模型对此类攻击的脆弱性已有研究，但有效防御措施的研究仍然有限。

Method: 开发并评估基于随机平滑的轨迹预测模型防御机制，该方法已在其他领域成功应用。通过一系列实验测试不同的随机平滑策略，评估其提升模型鲁棒性的能力。

Result: 该方法能够一致地提升多个基础轨迹预测模型在不同数据集上的预测鲁棒性，同时在非对抗场景下不损害预测精度。随机平滑提供了一种简单且计算成本低的技术来缓解轨迹预测中的对抗攻击。

Conclusion: 随机平滑是一种有效且实用的轨迹预测模型防御方法，能够在保持正常预测性能的同时显著提升对抗攻击下的鲁棒性。
...
TL;DR: 开发了6ABOS开源Python框架，用于EnMAP高光谱影像的大气校正，特别针对水体反射率反演，通过6S辐射传输模型和Google Earth Engine实现自动化处理。


<details>
  <summary>Details</summary>
Motivation: EnMAP任务为光学复杂环境监测开辟了新途径，但水体表面反射率的准确反演仍面临重大挑战，因为水体离水信号通常只占总辐射的很小部分，容易被大气散射和表面反射效应掩盖。

Method: 开发了6ABOS框架，基于6S辐射传输模型实现物理反演方案，考虑瑞利散射、气溶胶相互作用和气体吸收，集成EnMAP元数据自动解析和通过Google Earth Engine API的动态大气参数检索。

Result: 在两个地中海内陆水库（贫营养的Benagéber和超富营养的Bellús）进行验证，现场测量与EnMAP反演的水体离水反射率显示出高度光谱相似性，光谱角制图值始终较低（SAM < 10°）。
...
Method: SNPgen采用两阶段条件潜在扩散框架：1) GWAS引导的变异选择（1,024-2,048个与性状相关的SNPs）；2) 变分自编码器进行基因型压缩；3) 通过无分类器指导的条件潜在扩散模型，以二元疾病标签为条件。

Result: 在458,724个UK Biobank个体和四种复杂疾病（冠状动脉疾病、乳腺癌、1型和2型糖尿病）上评估，使用合成数据训练的模型在"合成训练-真实测试"协议中匹配了真实数据的预测性能，接近使用2-6倍更多变异的全基因组PRS方法。隐私分析显示零相同匹配、接近随机的成员推断（AUC≈0.50）、保留的连锁不平衡结构以及与源数据的高等位基因频率相关性（r≥0.95）。

Conclusion: SNPgen提供了一个隐私保护且实用的合成基因型生成框架，能够在保持统计保真度的同时支持下游基因组分析任务，为基因组数据共享提供了可行的替代方案。
...
TL;DR: LAtte框架通过整合洛伦兹注意力模块和InceptionTime编码器，专注于跨被试EEG分类，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: EEG分类面临信噪比低和个体间差异大的挑战，现有方法主要关注单被试性能，缺乏有效的跨被试泛化能力。

Method: 提出LAtte框架：1) 通过预训练任务学习所有被试的共享基线信号；2) 使用洛伦兹低秩适配器学习个体特异性嵌入；3) 结合洛伦兹注意力模块和InceptionTime编码器。

Result: 在三个成熟的EEG数据集上评估，相比当前最先进方法取得了显著性能提升。

Conclusion: LAtte框架能够学习跨被试的共享模型，既能微调用于个体被试，也能泛化到未见被试，为EEG分类提供了鲁棒且可泛化的解决方案。
...
TL;DR: 本文探讨了强化学习中非遍历奖励过程对智能体的影响，指出当奖励过程非遍历时，期望值（无限多轨迹的平均）无法反映单个无限长轨迹的平均表现，因此不是好的优化目标。


<details>
  <summary>Details</summary>
Motivation: 在强化学习中，通常优化智能体在轨迹上收集奖励的期望值。但如果生成这些奖励的过程是非遍历的，那么期望值（给定策略下无限多轨迹的平均）对于单个但无限长轨迹的平均是无意义的。如果我们关心个体智能体在部署期间的表现，期望值就不是一个好的优化目标。

Method: 通过指导性示例讨论非遍历奖励过程对强化学习智能体的影响，将遍历奖励过程的概念与更广泛使用的遍历马尔可夫链概念联系起来，并介绍在非遍历奖励动态下优化个体轨迹长期表现的现有解决方案。

Result: 分析了非遍历奖励过程对强化学习的影响，阐明了期望值作为优化目标的局限性，并提供了在非遍历环境中优化个体轨迹性能的解决方案。

Conclusion: 当奖励过程非遍历时，传统的期望值优化目标不适用于关心个体智能体表现的情况，需要采用针对非遍历奖励动态的优化方法来确保个体轨迹的长期性能。
...
TL;DR: 该论文提出使用组合融合分析(CFA)结合多个评分系统来生成体育队伍排名，相比传统排名方法提高了预测准确率。


<details>
  <summary>Details</summary>
Motivation: 尽管机器学习模型在体育预测方面取得了显著成功，但通常将其视为分类任务。本文旨在通过新的视角分析体育数据，利用排名信息更准确地预测比赛结果。

Method: 采用组合融合分析(CFA)这一新范式，通过秩-分数特征(RSC)函数和认知多样性(CD)来结合多个评分系统，为2024年数据集生成队伍排名。

Result: 基于队伍排名的秩组合方法取得了74.60%的准确率，高于十个流行公共排名系统中最好的73.02%，证明了CFA在提高体育预测精度方面的有效性。

Conclusion: 组合融合分析(CFA)通过不同视角有效提升了体育预测的精确度，为体育数据分析提供了新的有效方法。
...
TL;DR: ECoLAD是一个面向部署的异常检测评估协议，通过计算资源阶梯缩减和CPU线程限制来评估时间序列异常检测器在车载环境下的实际可行性，发现轻量级传统方法在资源受限时仍能保持性能，而某些深度学习方法在失去可行性前就已失去准确性。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列异常检测器通常在无约束的工作站硬件上进行比较，但车载监控需要可预测的延迟和有限CPU并行性下的稳定行为。仅基于准确性的排行榜可能会误导哪些方法在实际部署约束下仍然可行。

Method: 提出ECoLAD评估协议：1) 使用单调计算缩减阶梯，通过机械确定的整数缩放规则减少计算资源；2) 应用显式CPU线程限制；3) 记录所有配置更改；4) 在吞吐量约束下通过扫描目标评分率来表征行为，报告覆盖率和满足目标的最佳AUC-PR。

Result: 在受限的汽车遥测数据上，轻量级传统检测器在整个吞吐量扫描范围内都能维持覆盖率和检测提升（高于随机基线）。而几种深度方法在失去准确性之前就失去了可行性。

Conclusion: ECoLAD协议揭示了仅基于准确性的评估可能高估某些方法在实际部署环境中的实用性，特别是在资源受限的车载场景下。轻量级传统方法在资源效率方面表现更好，而深度学习方法需要更仔细地考虑其计算可行性。
...
TL;DR: 提出基于核密度估计的实用方法，量化表格型合成数据中的成员推断风险，无需计算昂贵的影子模型


<details>
  <summary>Details</summary>
Motivation: 合成数据作为隐私保护替代方案日益流行，但其隐私保证并非绝对，仍易受成员推断攻击。现有方法需要计算昂贵的影子模型，需要更实用有效的风险评估方法。

Method: 使用核密度估计器建模合成数据与训练记录之间最近邻距离的分布，通过概率推断成员身份，支持ROC曲线评估。提出两种攻击模型：假设拥有训练数据特权的"真实分布攻击"和使用无真实标签辅助数据的"现实攻击"。
...
<details>
  <summary>Details</summary>
Motivation: 测试时扩展通过每个提示采样多个输出来评估推理大语言模型，但在此机制下的模型排名方法研究不足，需要系统化的统计方法来准确比较模型性能。

Method: 提出了Scorio库，实现了多种统计排名方法：配对比较模型、项目反应理论（IRT）模型、投票规则、图方法和谱方法，并在20个推理模型和4个数学竞赛基准上进行了评估。

Result: 在完整试验（N=80次）下，大多数排名方法与贝叶斯黄金标准高度一致（Kendall's τ_b = 0.93-0.95），19-34种方法能恢复完全相同的排序；在单次试验下，最佳方法达到τ_b ≈ 0.86；使用贪婪解码作为经验先验可将N=1时的方差降低16-52%。

Conclusion: 研究确定了在高低预算测试时扩展场景下可靠的排名方法，Scorio库为推理大语言模型的准确比较提供了实用工具，并开源供社区使用。
...
TL;DR: 基于运动控制子运动理论，提出一种新的令牌化策略，将手腕加速度信号分解为子运动序列组成的运动段作为令牌，通过掩码运动段重建预训练Transformer编码器，在HAR任务中优于现有自监督学习方法


<details>
  <summary>Details</summary>
Motivation: 可穿戴加速度计虽然实现了大规模健康监测，但标记数据稀缺限制了鲁棒的人体活动表征学习。现有自监督学习方法将传感器流视为非结构化时间序列，忽略了人体运动的生物结构，而我们认为这对有效的HAR至关重要

Method: 基于运动控制子运动理论，提出新颖的令牌化策略：将连续手腕运动视为叠加的基本基函数（子运动），定义运动段为有限子运动序列组成的运动单元。将这些运动段作为令牌，通过掩码运动段重建预训练Transformer编码器，建模运动段的时间依赖关系

Result: 在NHANES语料库（约28k小时，11k参与者，10M窗口）上预训练，在六个主体不相交的HAR基准测试中优于现有可穿戴自监督学习方法，在数据稀缺场景下表现出更强的数据效率

Conclusion: 基于生物运动结构的令牌化策略能有效学习人体活动表征，通过子运动理论将连续运动分解为结构化令牌，使Transformer能够建模运动段的时间依赖关系，在HAR任务中取得优异性能
...
TL;DR: TOSSS是一个评估大语言模型在安全代码选择方面能力的基准测试，基于CVE数据库构建，可随时间扩展集成新漏洞，为模型提供0-1的安全评分。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在软件开发中的广泛应用，需要评估它们是否具备良好的软件安全能力。LLMs集成到开发流程中可能引入新漏洞并削弱现有安全措施，因此需要专门的评估工具。

Method: 提出TOSSS（双选项安全代码片段选择）基准测试，基于CVE数据库构建可扩展框架，让LLMs在安全代码片段和易受攻击代码片段之间进行选择，根据选择行为给出0-1的安全评分。

Result: 评估了14个广泛使用的开源和闭源模型在C/C++和Java代码上的表现，观察到安全评分范围在0.48到0.89之间。

Conclusion: TOSSS可作为LLM提供商现有基准测试报告的补充安全评分，帮助评估模型在软件安全方面的能力，促进更安全的LLM集成到开发流程中。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于物理方程的方法在处理高维状态空间和非线性动态系统时计算成本高，不适合实时分析和控制。需要一种能够学习参数化动态系统演化、支持长期预测、泛化到未见参数并能进行谱分析的方法。

Method: 提出物理编码神经场参数化的Koopman算子谱分解方法。与物理约束神经场（拟合单个解曲面）和神经算子（直接近似固定时间范围的解算子）不同，该方法学习因子化的流算子，将空间模式和时间演化解耦，从而揭示底层特征值、模式和稳定性。

Result: 在多种动态系统问题上展示了方法的有效性，能够准确预测复杂的时空现象，同时提供对系统动态行为的深入理解。

Conclusion: 该方法通过物理编码神经场参数化Koopman算子的谱分解，实现了对动态系统的稳定长期预测、参数空间插值和谱分析，为复杂物理系统的建模提供了有效的数据驱动解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 单细胞电生理记录是研究神经元功能多样性的重要手段，能够将内在生理特性与转录组身份联系起来。本研究旨在复制并扩展Gouwens等人建立的电生理-转录组学框架，利用公开的Allen Institute Patch-seq数据集，探索跨物种（小鼠和人类）的神经元亚类预测方法。

Method: 研究分析了3,699个小鼠视觉皮层神经元和506个人类新皮层神经元，聚焦于GABA能抑制性中间神经元（Lamp5、Pvalb、Sst、Vip亚类）。使用标准化电生理特征和稀疏PCA重现了原始研究中的主要类别分离。开发了基于注意力的双向LSTM模型，直接在结构化IPFX特征族表示上操作，避免了稀疏PCA，并通过学习到的注意力权重提供特征族级别的可解释性。最后评估了跨物种迁移学习设置，在小鼠数据上预训练模型，然后在人类数据上进行微调。

Result: 成功复制了小鼠数据中Gouwens流程的可重复性，证实了主要类别水平的分离。基于注意力的BiLSTM模型能够匹配特征工程基线的性能。跨物种迁移学习显著提升了人类神经元亚类预测的宏F1分数，相对于仅使用人类数据训练的基线有明显改善。

Conclusion: 该研究证实了Gouwens流程在小鼠数据中的可重复性，证明了序列模型能够达到特征工程基线的性能水平，并展示了从小鼠到人类的迁移学习能够为人类神经元亚类预测提供可测量的性能提升，为跨物种神经科学研究提供了有价值的工具和方法。
...
TL;DR: LLVQ利用Leech格子的最优球堆积特性实现大语言模型向量量化，无需显式码本存储，在24维空间中获得最佳压缩性能


<details>
  <summary>Details</summary>
Motivation: 标量量化受信息论限制，向量量化虽能突破但需要昂贵的查找机制或显式码本存储。格子方法通过结构化密集堆积解决此问题，Leech格子在24维具有最优球堆积特性，适合LLM量化

Method: 扩展基于扩展Golay码的搜索算法：1)支持索引，实现比特串与码字转换而无需物化码本；2)允许在Leech格子壳层联合上进行角度搜索；3)提出完全并行化的反量化核。这些构成了实用的Leech格子向量量化算法

Result: LLVQ在大语言模型量化方面达到最先进性能，优于Quip#、QTIP和PVQ等近期方法

Conclusion: 高维格子对于可扩展、理论基础的模型压缩具有重要意义，Leech格子的最优特性使其成为LLM量化的有效工具
...
### [76] [HEAL: Hindsight Entropy-Assisted Learning for Reasoning Distillation](https://arxiv.org/abs/2603.10359)
...
<details>
  <summary>Details</summary>
Motivation: 传统蒸馏方法将教师模型视为静态过滤器，丢弃教师无法独立解决复杂"角落案例"问题，导致学生模型存在"教师天花板"限制，无法超越教师能力

Method: 提出HEAL框架，包含三个核心模块：GEAR通过熵动态检测关键推理断点并注入事后提示修复轨迹；PURE分离真实认知突破与虚假捷径；PACE组织三阶段蒸馏策略

Result: 在多个基准测试上的广泛实验表明，HEAL显著优于传统SFT蒸馏和其他基线方法

Conclusion: HEAL通过主动干预和严格过滤机制，有效克服传统蒸馏的局限性，显著提升小模型的推理能力
...
<details>
  <summary>Details</summary>
Motivation: 传统基于标量概率评估LLM可靠性的方法无法捕捉推理的结构动态，需要一种能够揭示推理内部动态的理论基础框架。

Method: 引入TRACED框架，通过几何运动学分析推理轨迹，将推理分解为进展（位移）和稳定性（曲率）两个维度，建立概率框架来评估推理质量。
...
TL;DR: 该论文针对大语言模型不确定性评估不准确的问题，提出了基于不精确概率理论的新提示方法，能够同时评估一阶和二阶不确定性，提高LLM不确定性报告的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型不确定性评估技术基于经典概率框架，但实证表明这些技术与LLM实际行为不匹配，导致在模糊问答、上下文学习和自我反思等场景中出现系统性失败。需要更准确的不确定性评估方法来提高LLM的可信度和支持下游决策。

Method: 提出基于不精确概率理论的新提示方法，该理论能够表示和评估高阶不确定性。开发了通用的提示和后处理程序，直接评估和量化一阶不确定性（对可能回答的不确定性）和二阶不确定性（对底层概率模型本身的不确定性）。

Result: 该方法在不同设置下都表现出有效性，能够更准确地评估大语言模型的不确定性，改善不确定性报告的可信度，并支持下游决策制定。

Conclusion: 基于不精确概率理论的提示方法能够更准确地评估大语言模型的不确定性，解决了现有技术不匹配的问题，提高了LLM不确定性报告的可信度，为下游决策提供了更好的支持。
...
Method: 提出轻量级混合框架，结合图注意力自编码器为多步蒙特卡洛树搜索提供信息，使用随机图遗传算法优化评估信号，利用GPT-4o-mini生成合成训练数据。框架从噪声和不完美的监督中学习，图注意力机制作为结构过滤器对LLM输出进行去噪。

Result: 在10×10亚马逊棋盘上，混合方法比基线提升15%-56%的决策准确性，显著超越其教师模型GPT-4o-mini。在N=30节点时达到45.0%的竞争性胜率，在仅N=50节点时达到66.5%的决定性胜率。

Conclusion: 验证了在严格计算约束下，从通用基础模型演化出专业化高性能游戏AI的可行性。图注意力机制能有效过滤LLM噪声输出，实现弱到强泛化。
...
TL;DR: 研究发现：在道德推理对齐任务中，分布匹配方法并未显著优于奖励最大化方法，这与预期相反。道德推理的高奖励响应分布比数学推理更集中，因此模式寻求优化同样有效。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索LLM对齐任务是否需要与逻辑推理任务不同的方法。鉴于道德推理中允许多种有效回答，假设对齐任务需要多样性寻求的分布匹配算法而非奖励最大化的策略方法。

Method: 在MoReBench上进行首个全面的实证研究，比较两种范式。通过训练Qwen3-1.7B评判模型构建基于评分标准的奖励管道，实现稳定的RLVR训练。通过语义可视化将高奖励响应映射到语义空间进行分析。

Result: 与假设相反，分布匹配方法在道德推理对齐任务中并未表现出预期的显著优势。语义可视化显示道德推理的高奖励分布比数学推理更集中，数学推理中多样化的解决策略能产生相似的高奖励。

Conclusion: 对齐任务并不固有地需要多样性保持算法，标准的奖励最大化RLVR方法可以有效地迁移到道德推理中，无需显式的多样性机制。
...
### [81] [Trajectory-Informed Memory Generation for Self-Improving Agent Systems](https://arxiv.org/abs/2603.10600)
...
<details>
  <summary>Details</summary>
Motivation: LLM驱动的智能体面临持续挑战：从执行经验中学习以改进未来性能。虽然智能体能成功完成许多任务，但经常重复低效模式，无法从类似错误中恢复，并错过应用过去成功策略的机会。

Method: 框架包含四个组件：(1)轨迹智能提取器，执行智能体推理模式的语义分析；(2)决策归因分析器，识别导致失败、恢复或低效的决策和推理步骤；(3)上下文学习生成器，生成三种指导：成功模式的策略提示、失败处理的恢复提示、低效但成功执行的优化提示；(4)自适应记忆检索系统，基于多维相似性将相关学习经验注入智能体提示。
...
TL;DR: PharmGraph-Auditor：基于混合知识库和验证链的处方审核系统，解决大语言模型在医疗领域不可靠问题


<details>
  <summary>Details</summary>
Motivation: 用药错误严重威胁患者安全，药师审核是关键但负担重的最后防线。大语言模型直接应用于零容忍医疗领域不可行，因为存在事实不可靠、缺乏可追溯性和复杂推理能力弱等问题。

Method: 1. 构建可信混合药学知识库（HPKB），采用虚拟知识图谱范式，统一关系型组件（集合约束满足）和图组件（拓扑推理）；2. 提出迭代模式精炼算法（ISR），从医学文本中协同演化图模式和关系模式；3. 引入知识库驱动的验证链（CoV），将大语言模型从不可靠的生成器转变为透明推理引擎，将审核任务分解为可验证的知识库查询序列。

Result: 实验结果表明系统具有强大的知识提取能力，有望帮助药师实现更安全、更快速的处方验证。

Conclusion: PharmGraph-Auditor通过混合知识库架构和验证链推理范式，为大语言模型在医疗安全关键领域的安全应用提供了可行方案，能够支持药师进行更可靠的处方审核。
...
### [83] [Stability and Robustness via Regularization: Bandit Inference via Regularized Stochastic Mirror Descent](https://arxiv.org/abs/2603.10184)
*Budhaditya Halder,Ishan Sengupta,Koustav Chowdhury,Koulik Khamaru*

Main category: stat.ML

TL;DR: 本文为基于随机镜像下降的赌博机算法建立了系统稳定性理论，提出了稳定性判据，设计了正则化EXP3算法，证明其同时满足稳定性（支持统计推断）和最优遗憾保证，并展示了在对抗性污染下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 赌博机数据中的自适应采样违反了经典渐近理论的独立性假设，给统计推断带来根本性挑战。稳定性已被确定为适应条件下有效推断的充分条件，但缺乏对赌博机算法（特别是基于随机镜像下降的广泛算法框架）稳定性的系统理论。

Method: 1. 建立基于随机镜像下降算法的通用稳定性判据：如果平均迭代以比率收敛到非随机概率向量，则诱导的赌博机算法是稳定的。2. 设计使用对数障碍正则化器的正则化-EXP3算法族，并证明其满足稳定性判据。3. 建立对抗性污染下的鲁棒性：改进的正则化-EXP3变体在存在o(T^{1/2})对抗性污染时仍保持经验臂均值的渐近正态性。

Result: 1. 稳定性判据为分析不同算法实例的稳定性提供了统一视角。2. 正则化-EXP3算法满足稳定性判据，使得线性泛函均值参数的Wald型置信区间达到名义覆盖率。3. 同一算法在达到对数因子内的极小极大最优遗憾保证，表明推断所需的稳定性和学习效率在镜像下降框架内是兼容的。4. 改进的正则化-EXP3在对抗性污染下保持渐近正态性，而其他稳定算法（如UCB）在少量污染下就会遭受线性遗憾。

Conclusion: 本文为基于随机镜像下降的赌博机算法建立了系统稳定性理论，证明了稳定性与学习效率的兼容性，并展示了算法在对抗性环境下的鲁棒性。正则化-EXP3算法同时实现了最优遗憾保证和有效统计推断，为自适应采样环境下的可靠推断提供了实用解决方案。
...
### [84] [A Diffusion Analysis of Policy Gradient for Stochastic Bandits](https://arxiv.org/abs/2603.10219)
*Tor Lattimore*

Main category: stat.ML

TL;DR: 本文研究了k臂随机赌博机的策略梯度连续时间扩散近似，证明了在特定学习率下可实现对数遗憾，并构造了需要更小学习率才能避免线性遗憾的实例。


<details>
  <summary>Details</summary>
Motivation: 研究策略梯度方法在随机赌博机问题中的理论性能，特别是通过连续时间扩散近似来分析其遗憾界，为理解策略梯度在强化学习中的收敛性质提供理论保证。

Method: 采用连续时间扩散近似来分析策略梯度方法，通过理论推导证明遗憾界，并构造特殊实例来验证学习率要求的紧性。

Result: 证明了当学习率η=O(Δ²/log(n))时，遗憾为O(k log(k) log(n)/η)，其中n为时间范围，Δ为最小奖励差距。同时构造了一个只有对数数量臂的实例，表明除非η=O(Δ²)，否则遗憾将是线性的。

Conclusion: 策略梯度方法在k臂赌博机问题中可以实现对数遗憾，但需要适当设置学习率，且学习率要求与最小奖励差距的平方成正比，这一条件是必要的。
...
### [85] [Bayesian Hierarchical Models and the Maximum Entropy Principle](https://arxiv.org/abs/2603.10252)
*Brendon J. Brewer*

Main category: stat.ML

TL;DR: 贝叶斯层次模型中，当给定超参数时先验是典型分布（具有矩约束的最大熵分布），则依赖的边际先验也具有最大熵特性，但具有不同的约束条件。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯层次模型在实际数据分析中广泛应用，通过引入超参数间接为未知参数分配先验。这种模型产生的边际先验（对超参数积分后）通常是依赖的，使得学习一个参数能为其他参数提供信息。本文旨在阐明当给定超参数时的先验是典型分布时，依赖的边际先验也具有最大熵特性。

Method: 通过理论分析证明，当层次模型中给定超参数时的先验是典型分布（即具有矩约束的最大熵分布）时，对超参数积分后得到的依赖边际先验也具有最大熵特性，但约束条件不同。这种约束是关于未知量某些函数的边际分布的约束。

Result: 证明了在贝叶斯层次模型中，如果给定超参数时的先验是典型分布，那么依赖的边际先验也具有最大熵特性，但约束条件变为对未知量某些函数的边际分布的约束。这一结果阐明了当我们分配层次模型时实际假设的信息类型。

Conclusion: 该研究揭示了贝叶斯层次模型中依赖边际先验的最大熵特性，为理解层次模型所隐含的信息假设提供了理论洞见。结果表明，层次模型通过超参数引入的依赖关系实际上对应于对未知量函数边际分布的约束，这有助于更清晰地理解层次模型的信息内涵。
...
### [86] [MultiwayPAM: Multiway Partitioning Around Medoids for LLM-as-a-Judge Score Analysis](https://arxiv.org/abs/2603.10287)
*Chihiro Watanabe,Jingyu Sun*

Main category: stat.ML

TL;DR: 该论文提出MultiwayPAM方法，通过张量聚类分析LLM-as-a-Judge评分张量，解决LLM评估的计算成本和偏见问题。


<details>
  <summary>Details</summary>
Motivation: LLM-as-a-Judge框架虽然灵活，但面临两大挑战：评估大量文本时LLM推理的计算成本高昂，以及LLM评估器固有的偏见问题。需要揭示LLM评估器造成的评分偏见结构。

Method: 提出MultiwayPAM张量聚类方法，应用于LLM-as-a-Judge评分张量（包含问题、回答者、评估者组合的评分）。该方法能同时估计每个维度的聚类成员关系和中心点，通过观察中心点了解各问题/回答者/评估者聚类的成员关系。

Result: 通过将MultiwayPAM应用于两个实际数据集的评分张量，实验证明了该方法的有效性。

Conclusion: MultiwayPAM方法能够有效分析LLM-as-a-Judge评分张量，揭示LLM评估器的偏见结构，为解决LLM评估的计算成本和偏见问题提供了有效工具。
...
### [87] [On The Complexity of Best-Arm Identification in Non-Stationary Linear Bandits](https://arxiv.org/abs/2603.10346)
*Leo Maynard-Zhang,Zhihan Xiong,Kevin Jamieson,Maryam Fazel*

Main category: stat.ML

TL;DR: 论文研究了非平稳线性赌博机中的固定预算最佳臂识别问题，提出了基于相邻最优设计的算法，并建立了与臂集相关的复杂度下界。


<details>
  <summary>Details</summary>
Motivation: 传统G-最优设计的复杂度下界过于悲观，因为它只考虑了标准基向量臂集，忽略了臂集几何结构可能带来的优势。需要建立适用于任意臂集的臂集依赖下界。

Method: 提出了Adjacent-optimal设计（XY-最优设计的特化），并开发了Adjacent-BAI算法。通过相邻最优设计来匹配臂集依赖的下界。

Result: 证明了Adjacent-BAI算法的错误概率与下界在常数因子内匹配，验证了下界的紧致性，并建立了该设置的臂集依赖复杂度。

Conclusion: 该工作为非平稳线性赌博机中的固定预算BAI问题提供了臂集依赖的复杂度分析，通过相邻最优设计实现了与下界匹配的最优算法。
...
### [88] [Adaptive Active Learning for Regression via Reinforcement Learning](https://arxiv.org/abs/2603.10435)
*Simon D. Nguyen,Troy Russo,Kentaro Hoffman,Tyler H. McCormick*

Main category: stat.ML

TL;DR: WiGS通过强化学习动态调整探索与利用平衡，在回归主动学习中比iGS等方法表现更好


<details>
  <summary>Details</summary>
Motivation: 现有改进贪婪采样(iGS)方法使用静态乘法规则平衡特征空间多样性和输出空间不确定性，但在数据密度不均匀时，这种方法会忽略密集区域的高误差样本，限制了主动学习的效果

Method: 提出加权改进贪婪采样(WiGS)，将静态乘法规则替换为动态加法准则，将权重选择建模为强化学习问题，使智能体能够在学习过程中自适应调整探索与利用的平衡

Result: 在18个基准数据集和一个合成环境上的实验表明，WiGS在准确性和标注效率方面均优于iGS和其他基线方法，特别是在数据密度不均匀的领域中表现更佳

Conclusion: WiGS通过动态权重调整机制有效解决了传统方法在数据密度不均匀时的局限性，提高了回归主动学习的性能
...
### [89] [Brenier Isotonic Regression](https://arxiv.org/abs/2603.10452)
*Han Bao,Amirreza Eshraghi,Yutong Wang*

Main category: stat.ML

TL;DR: 论文提出了一种新的多输出回归方法——Brenier等渗回归，将经典等渗回归扩展到多输出场景，通过利用最优传输中的循环单调性概念来解决多输出回归中的单调性约束问题。


<details>
  <summary>Details</summary>
Motivation: 经典等渗回归只能处理单变量情况，在多输出回归中无法直接应用，因为单调性概念难以扩展到多变量场景。需要一种新的方法来处理多输出回归中的形状约束问题。

Method: 提出Brenier等渗回归，利用最优传输理论中的循环单调性概念。将回归函数解释为广义线性模型中的链接函数，将凸势函数解释为最优传输中的Brenier势函数。通过Kantorovich最优传输总是产生循环单调耦合作为最优解的特性来实施循环单调性约束。

Result: 在概率校准和广义线性模型实验中，Brenier等渗回归表现出色。特别是在概率校准任务中，该方法稳健地优于许多著名基线模型。

Conclusion: Brenier等渗回归成功地将等渗回归扩展到多输出回归问题，通过最优传输理论中的循环单调性概念提供了一种有效的形状约束回归方法，在概率校准等应用中展现出优越性能。
...
### [90] [Dual Space Preconditioning for Gradient Descent in the Overparameterized Regime](https://arxiv.org/abs/2603.10485)
*Reza Ghane,Danil Akhtiamov,Babak Hassibi*

Main category: stat.ML

TL;DR: 本文研究了双空间预条件梯度下降的收敛性，包括归一化梯度下降、梯度裁剪和Adam等优化器。证明了在过参数化线性模型中，预条件梯度下降总是收敛到满足数据拟合的解，并分析了其隐式偏差特性。


<details>
  <summary>Details</summary>
Motivation: 研究双空间预条件梯度下降（包括归一化梯度下降、梯度裁剪和Adam等常用优化器）的收敛性质，特别是在过参数化线性模型中的行为，以理解这些优化算法的理论基础和隐式偏差。

Method: 使用形式为∇K的预条件器，其中K是凸函数，应用于训练过参数化线性模型ℓ(XW-Y)。引入新的Bregman散度版本及其恒等式来建立收敛性证明。对一般预条件器和各向同性预条件器分别分析隐式偏差。

Result: 证明了预条件梯度下降的迭代总是收敛到满足XW∞=Y的点。对于各向同性预条件器，W∞最小化‖W∞-W0‖²_F，与标准梯度下降的收敛点相同。对于一般预条件器，收敛点与梯度下降收敛点之间的距离存在常数倍关系。

Conclusion: 双空间预条件梯度下降在过参数化线性模型中保证收敛到数据拟合解，但隐式偏差取决于预条件器类型：各向同性预条件器与标准梯度下降有相同的隐式偏差，而一般预条件器的隐式偏差受学习率影响且只能保证与梯度下降解有常数倍距离关系。
...
### [91] [ReTabSyn: Realistic Tabular Data Synthesis via Reinforcement Learning](https://arxiv.org/abs/2603.10823)
*Xiaofeng Lin,Seungbae Kim,Zhuoya Li,Zachary DeSoto,Charles Fleming,Guang Cheng*

Main category: stat.ML

TL;DR: ReTabSyn是一个强化表格合成管道，通过直接反馈特征相关性来优先学习条件分布P(y|X)，在小样本、类别不平衡和分布偏移场景中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 深度生成模型在低数据、不平衡的表格设置中难以完全学习复杂的数据分布。追求完整的联合分布可能过度，应该优先学习条件分布P(y|X)以提高数据效率。

Method: 提出ReTabSyn（强化表格合成管道），在合成器训练期间提供特征相关性保存的直接反馈，鼓励生成器在训练数据有限时优先学习最有用的预测信号。

Result: 在小样本、类别不平衡和分布偏移的基准测试中，ReTabSyn持续优于最先进的基线方法。该方法还可以扩展到控制合成表格数据的各个方面。

Conclusion: 通过优先学习条件分布而非完整联合分布，ReTabSyn在低数据表格场景中实现了更好的数据效率和下游模型效用，并能灵活应用专家指定的约束。
...
### [92] [Kernel Tests of Equivalence](https://arxiv.org/abs/2603.10886)
*Xing Liu,Axel Gandy*

Main category: stat.ML

TL;DR: 提出基于核的等价性检验方法，用于评估两个分布是否等价，解决了传统拟合优度检验无法证明分布无差异的问题。


<details>
  <summary>Details</summary>
Motivation: 传统拟合优度检验无法证明分布无差异，因为不拒绝原假设可能是由于检验功效不足（第二类错误）。现有等价性检验要么限于参数分布，要么只关注特定矩而非完整分布。

Method: 使用两种基于核的统计差异度量：核斯坦因差异和最大均值差异。原假设假设候选分布与名义分布差异至少超过预定义边界。提出两种计算临界值的方法：渐近正态近似和自助法。

Result: 通过数值实验评估了这些检验的性能，验证了所提方法的有效性。

Conclusion: 提出的核基等价性检验能够有效评估分布等价性，解决了传统方法的局限性，为分布等价性检验提供了更全面的框架。
