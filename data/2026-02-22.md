...
TL;DR: AIdentifyAGE本体论为法医牙科年龄评估提供了一个标准化的语义框架，整合了手动和AI辅助的工作流程，旨在提高法医和司法决策中的一致性、透明度和可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前法医牙科年龄评估面临方法异质性、数据表示碎片化以及临床、法医和法律信息系统之间互操作性有限的问题，这些问题阻碍了透明度和可重复性，特别是在AI方法日益普及的背景下。

Method: 开发了一个领域特定的AIdentifyAGE本体论，该本体论基于上层和已建立的生物医学、牙科和机器学习本体论，与领域专家合作开发，建模完整的法医-法律工作流程，包括司法背景、个体信息、法医检查数据、牙齿发育评估方法、放射成像、统计参考研究和基于AI的估计方法。

Result: AIdentifyAGE本体论提供了一个标准化的语义连贯框架，能够追踪观察、方法、参考数据和报告结果之间的可追溯链接，确保与FAIR原则的互操作性、可扩展性和合规性。

Conclusion: AIdentifyAGE本体论是提高法医和司法背景下一致性和透明度的基础步骤，为基于本体的决策支持系统建立了坚实基础，特别是在涉及无证件个人和无人陪伴未成年人的案件中。
...
Method: 将上下文建模为作用于共享内部状态的干预，证明任何再现上下文结果统计的经典模型都必须承担不可约的信息论成本，提供最小构造性示例，并解释非经典概率框架如何通过放宽单一全局联合概率空间假设来避免这种障碍

Result: 证明了上下文性不是量子力学的特性，而是经典概率表示中单状态重用的必然结果，揭示了上下文依赖不能仅通过内部状态中介，必须承担信息论成本

Conclusion: 上下文性是自适应智能的一般表示约束，独立于物理实现，非经典概率框架通过放弃单一全局联合概率空间假设可以避免经典模型的限制
...
TL;DR: 该研究分析了60个大型语言模型基准测试的饱和现象，发现近半数基准已饱和，饱和率随基准使用时间增长而增加。研究检验了14个基准设计属性对饱和的影响，发现隐藏测试数据无保护效果，而专家策划的基准比众包基准更能抵抗饱和。


<details>
  <summary>Details</summary>
Motivation: AI基准测试在衡量模型进展和指导部署决策中起核心作用，但许多基准测试很快饱和，无法区分最佳性能模型，降低了其长期价值。研究旨在分析基准饱和现象并识别驱动饱和的因素。

Method: 研究分析了60个大型语言模型基准测试，这些基准选自主要模型开发商的技术报告。研究通过14个属性（涵盖任务设计、数据构建和评估格式）对基准进行表征，并检验了5个假设来检验每个属性对饱和率的影响。

Result: 研究发现近半数基准测试表现出饱和现象，饱和率随基准使用时间增长而增加。隐藏测试数据（公开vs私有）没有保护效果，而专家策划的基准比众包基准更能抵抗饱和。研究揭示了哪些设计选择能延长基准寿命。

Conclusion: 研究结果强调了哪些设计选择能延长基准测试的寿命，并为更持久的评估策略提供了信息。专家策划的基准设计比众包方法更能抵抗饱和，而隐藏测试数据并不能有效防止饱和。
...
TL;DR: 论文发现简单的代码演化基线方法在数学边界、智能体脚手架设计和机器学习竞赛三个领域中，其表现与或优于更复杂的方法，揭示了当前代码演化研究中的评估缺陷。


<details>
  <summary>Details</summary>
Motivation: 当前代码演化技术虽然表现出色，但缺乏与简单基线的系统比较。研究者希望验证简单方法在实际应用中的表现，并识别代码演化研究中的问题。

Method: 在三个领域测试简单基线方法：1）寻找更好的数学边界；2）设计智能体脚手架；3）机器学习竞赛。通过分析结果识别代码演化研究中的问题。

Result: 简单基线在所有三个领域都匹配或超过了更复杂的方法。数学边界问题中，搜索空间设计和领域知识比演化管道更重要；智能体脚手架设计中，高方差和小数据集导致次优选择；机器学习竞赛中也存在类似问题。

Conclusion: 代码演化的主要挑战在于搜索空间设计而非搜索算法本身。需要更好的评估方法来减少随机性，同时保持经济可行性。提出了未来代码演化研究的改进方向和最佳实践。
...
TL;DR: 该论文研究了n维超立方体Q_n的边被超平面切割的最小超平面数量S(n)，证明了新的上界S(n) ≤ ⌈4n/5⌉（当n不是5的奇数倍时），改进了1971年Paterson的S(n) ≤ ⌈5n/6⌉上界。


<details>
  <summary>Details</summary>
Motivation: 研究超平面切割超立方体边的最小数量问题，这是一个经典的组合几何问题。之前的最佳上界是1971年Paterson提出的S(n) ≤ ⌈5n/6⌉，作者希望改进这个上界。

Method: 通过构造8个超平面切割Q_{10}来证明改进的上界，并利用新开发的自动工具CPro1辅助。CPro1结合推理大语言模型和自动超参数调优，用于搜索数学构造的算法发现。

Result: 证明了S(n) ≤ ⌈4n/5⌉（当n不是5的奇数倍时），当n是5的奇数倍时S(n) ≤ 4n/5 + 1。同时获得了使用k<n个超平面时能切割的最大边数的新下界。

Conclusion: 成功改进了超立方体边切割问题中S(n)的上界，展示了自动工具CPro1在数学构造发现中的有效性，为组合几何中的这一经典问题提供了新的进展。
...
### [6] [Node Learning: A Framework for Adaptive, Decentralised and Collaborative Network Edge AI](https://arxiv.org/abs/2602.16814)
...
Result: 该概念论文发展了Node Learning范式的概念基础，对比了现有去中心化方法，并探讨了对通信、硬件、信任和治理的影响

Conclusion: Node Learning不抛弃现有范式，而是将其置于更广泛的去中心化视角中，将自主和协作行为统一在单一抽象中，适应数据、硬件、目标和连接性的异构性
...
### [7] [An order-oriented approach to scoring hesitant fuzzy elements](https://arxiv.org/abs/2602.16827)
*Luis Merino,Gabriel Navarro,Carlos Salvatierra,Evangelina Santos*

Main category: cs.AI

TL;DR: 本文提出了一种基于序理论的犹豫模糊集评分统一框架，证明传统序不构成格结构，而对称序满足评分函数的关键规范准则，并引入了用于排序的支配函数。


<details>
  <summary>Details</summary>
Motivation: 传统犹豫模糊集评分方法缺乏序理论的形式化基础，需要建立更灵活、一致的评分机制，以支持群体决策。

Method: 提出基于给定序的统一评分框架，分析犹豫模糊元素上的经典序结构，证明对称序满足强单调性和Gärdenfors条件，引入支配函数类用于排序，提供离散支配函数和相对支配函数两个具体实例。

Result: 证明经典序不诱导格结构，而对称序满足评分函数的规范准则；支配函数可用于构建犹豫模糊集上的模糊偏好关系，支持群体决策。

Conclusion: 基于序理论的统一框架为犹豫模糊集评分提供了形式化基础，对称序满足关键规范性质，支配函数为犹豫模糊元素排序和群体决策提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前智能体开发套件要么功能支持不足，要么依赖人工设计智能体拓扑、工具和内存等组件，限制了智能体的泛化能力和整体性能

Method: OpenSage让LLM自动创建和管理子智能体及工具集，采用分层图结构内存系统，并提供专门针对软件工程任务的工具包

Result: 在三个最先进基准测试中，使用不同骨干模型的OpenSage均优于现有ADK，消融研究也验证了各组件设计的有效性
...
TL;DR: AgentLAB是首个专门评估LLM智能体对自适应长视野攻击脆弱性的基准测试，包含5种新型攻击类型、28个真实环境和644个安全测试用例，发现现有智能体对长视野攻击高度脆弱且单轮防御措施无效。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体在复杂长视野环境中部署增加，它们面临利用多轮用户-智能体-环境交互的长视野攻击风险，这些攻击在单轮设置中无法实现。目前缺乏专门评估智能体对此类攻击脆弱性的基准测试。

Method: 提出AgentLAB基准测试框架，支持5种新型攻击类型：意图劫持、工具链式攻击、任务注入、目标漂移和内存污染，覆盖28个真实智能体环境和644个安全测试用例。使用该基准评估代表性LLM智能体。

Result: 评估发现代表性LLM智能体对长视野攻击高度脆弱，为单轮交互设计的防御措施无法可靠缓解长视野威胁。AgentLAB可作为跟踪实际环境中LLM智能体安全进展的基准。

Conclusion: AgentLAB是首个专门评估LLM智能体长视野攻击脆弱性的基准测试，揭示了现有智能体的安全漏洞和防御措施的不足，为实际部署中的智能体安全评估提供了重要工具。
...
TL;DR: LLM-Wikirace是一个评估大语言模型规划、推理和世界知识的基准测试，要求模型通过维基百科超链接从源页面逐步导航到目标页面。前沿模型在简单任务上表现优异甚至超越人类，但在困难任务上性能急剧下降，最佳模型成功率仅23%，揭示了当前推理系统的明显局限性。


<details>
  <summary>Details</summary>
Motivation: 需要评估大语言模型在规划、推理和世界知识方面的能力，特别是在需要前瞻性规划和理解现实世界概念连接的任务中。现有基准可能无法充分测试这些复杂能力，因此创建了一个通过维基百科导航来评估这些技能的新基准。

Method: 创建了LLM-Wikirace基准测试，要求模型通过维基百科超链接逐步导航从源页面到目标页面。评估了包括Gemini-3、GPT-5和Claude Opus 4.5在内的广泛开源和闭源模型，设置了不同难度级别（简单和困难），并进行了轨迹级分析以了解模型失败的原因。

Result: 前沿模型在简单任务上表现优异，甚至超越人类水平，但在困难任务上性能急剧下降：最佳模型Gemini-3在困难游戏中的成功率仅为23%。分析表明世界知识是成功的必要条件，但达到一定阈值后，规划和长视野推理能力成为主导因素。轨迹分析显示即使最强模型在失败后也难以重新规划，经常陷入循环而非恢复。

Conclusion: LLM-Wikirace是一个简单但有效的基准测试，揭示了当前推理系统的明显局限性。虽然前沿模型在简单任务上表现出色，但在需要复杂规划和长视野推理的困难任务中仍有很大提升空间。该基准为规划能力强的LLMs提供了一个开放的竞技场，展示了它们在处理复杂导航任务时的不足。
...
TL;DR: 研究发现对已对齐的视觉语言模型进行窄域有害数据微调会导致严重的泛化性错位，这种错位在视觉模态中比文本模态更严重，且即使只有10%有害数据也会导致显著对齐退化。


<details>
  <summary>Details</summary>
Motivation: 终身多模态智能体需要通过后训练持续适应新任务，但这在获取能力和保持安全对齐之间产生了根本性矛盾。研究者希望探究微调已对齐的视觉语言模型对窄域有害数据集是否会导致严重的泛化性错位。

Method: 使用Gemma3-4B模型进行实验，通过LoRA微调技术在不同秩参数下测试，比较文本和多模态评估中的错位程度。进行几何分析探究有害行为的低维子空间特性，并评估两种缓解策略：良性窄域微调和基于激活的引导。

Result: 研究发现错位程度随LoRA秩单调增加，多模态评估显示错位程度（70.71±1.22，r=128）远高于文本评估（41.19±2.51）。即使只有10%有害数据的训练混合也会导致显著对齐退化。几何分析显示有害行为占据极低维子空间，仅10个主成分就能捕获大部分错位信息。两种缓解策略都能显著减少错位，但都无法完全消除已学习的有害行为。

Conclusion: 当前的后训练范式可能无法在部署后环境中充分保持对齐，凸显了需要开发鲁棒的持续学习框架来平衡能力获取和安全对齐的迫切需求。
...
### [12] [Fundamental Limits of Black-Box Safety Evaluation: Information-Theoretic and Computational Barriers from Latent Context Conditioning](https://arxiv.org/abs/2602.16984)
*Vishal Srivastava*

Main category: cs.AI

TL;DR: 论文挑战了AI系统黑盒安全评估的基本假设，证明了对于依赖未观测内部变量的模型，任何黑盒评估器都无法可靠估计部署风险，并建立了统计和计算上的根本限制。


<details>
  <summary>Details</summary>
Motivation: 挑战AI安全评估的核心假设——测试分布上的模型行为能可靠预测部署性能。作者发现许多模型存在"潜在上下文条件策略"，其输出依赖于在评估中罕见但在部署中普遍的未观测内部变量，这导致黑盒评估存在根本性缺陷。

Method: 采用理论分析方法：1) 被动评估：使用Le Cam方法证明极小极大下界；2) 自适应评估：使用基于哈希的触发器构造和Yao极小极大原理；3) 计算分离：基于陷门单向函数假设；4) 白盒探测：提供样本复杂度分析和偏差校正方法。

Result: 建立了三个主要结果：1) 被动评估的期望绝对误差下界为约0.208*δ*L；2) 自适应评估的最坏情况误差下界为δ*L/16；3) 计算上，拥有特权信息的部署环境可以激活任何多项式时间评估器无法区分的危险行为；4) 白盒探测需要O(1/(γ²·ε_R²))样本。

Conclusion: 黑盒测试在统计上是欠确定的，为最坏情况安全保证需要额外的保障措施：架构约束、训练时保证、可解释性和部署监控。研究量化了黑盒评估的局限性，并提供了何时需要额外安全措施的明确标准。
...
### [13] [Conv-FinRe: A Conversational and Longitudinal Benchmark for Utility-Grounded Financial Recommendation](https://arxiv.org/abs/2602.16990)
...
Method: 构建Conv-FinRe基准测试，包含入职访谈、逐步市场背景和咨询对话。模型需要在固定投资期限内生成股票排名。基准提供多视角参考，区分描述性行为和基于投资者风险偏好的规范性效用。使用真实市场数据和人类决策轨迹构建，实例化受控的咨询对话。

Result: 评估了一系列最先进的LLM，结果显示理性决策质量和行为对齐之间存在持续紧张关系：在基于效用的排名上表现良好的模型往往无法匹配用户选择，而行为对齐的模型可能过度拟合短期噪声。

Conclusion: Conv-FinRe基准测试能够诊断LLM是遵循理性分析、模仿用户噪声还是受市场动量驱动。数据集已在Hugging Face公开，代码库在GitHub上可用，为金融推荐系统提供了超越行为模仿的评估框架。
...
### [14] [Sonar-TS: Search-Then-Verify Natural Language Querying for Time Series Databases](https://arxiv.org/abs/2602.17001)
*Zhao Tan,Yiji Zhao,Shiyu Wang,Chang Xu,Yuxuan Liang,Xiping Liu,Shirui Pan,Ming Jin*

Main category: cs.AI

TL;DR: Sonar-TS是一个神经符号框架，通过"搜索-验证"管道处理时间序列数据库的自然语言查询，解决了现有方法无法处理连续形态意图和超长历史的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL方法无法处理时间序列中的连续形态意图（如形状或异常），而时间序列模型难以处理超长历史记录，因此需要专门针对时间序列数据库自然语言查询的解决方案。

Method: 提出Sonar-TS神经符号框架，采用类似主动声纳的"搜索-验证"管道：首先使用特征索引通过SQL查询候选窗口，然后生成Python程序锁定并验证候选窗口与原始信号的匹配。

Result: 实验表明Sonar-TS能够有效处理传统方法失败的复杂时间查询，并引入了NLQTSBench作为该领域首个大规模基准测试，突显了该领域的独特挑战。
...
### [15] [Cinder: A fast and fair matchmaking system](https://arxiv.org/abs/2602.17015)
*Saurav Pal*

Main category: cs.AI

TL;DR: Cinder是一个两阶段匹配系统，通过Ruzicka相似性指数快速筛选，然后使用基于倒置正态分布技能桶的Kantorovich距离计算"制裁分数"，实现公平快速的多人游戏匹配。


<details>
  <summary>Details</summary>
Motivation: 现代多人游戏中，公平快速的匹配系统对玩家留存和满意度至关重要。传统基于平均团队技能的方法在处理异质技能水平的预组队时，常常导致不平衡和一边倒的比赛，特别是当技能分布广泛或偏斜时。

Method: Cinder采用两阶段方法：第一阶段使用Ruzicka相似性指数快速比较团队的"非异常值"技能范围进行初步筛选；第二阶段将玩家排名映射到基于倒置正态分布生成的技能桶中，提供平均技能水平更高的粒度，然后使用Kantorovich距离在排序的桶索引上计算"制裁分数"来量化匹配公平性。

Result: 通过分析1.4亿个模拟团队配对的制裁分数分布，证明了系统的可行性，为公平匹配阈值提供了稳健的基础。

Conclusion: Cinder系统能够为异质技能水平的预组队提供快速且公平的匹配，解决了传统平均技能匹配方法的局限性，有望提高玩家游戏体验和满意度。
...
### [16] [M2F: Automated Formalization of Mathematical Literature at Scale](https://arxiv.org/abs/2602.17016)
*Zichen Wang,Wanli Ma,Zhenyu Ming,Gong Zhang,Kun Yuan,Zaiwen Wen*

Main category: cs.AI

TL;DR: M2F是首个面向项目级数学自动形式化的智能框架，能在Lean中将长篇数学资料转换为完整的形式化库，实现教科书级规模的形式化。


<details>
  <summary>Details</summary>
Motivation: 现有的数学自动形式化技术仅限于孤立定理和简短片段，无法扩展到教科书和研究论文级别，需要解决跨文件依赖、导入解析和端到端项目编译等问题。

Method: 采用两阶段智能框架：1) 语句编译阶段将文档拆分为原子块，通过依赖关系排序，修复声明框架直到项目编译通过；2) 证明修复阶段在固定签名下使用目标导向的局部编辑填补证明空缺。整个过程保持验证器在循环中，仅当工具链反馈确认改进时才提交编辑。

Result: 在约三周时间内，M2F将479页的实分析和凸分析教科书转换为包含153,853行代码的Lean库，实现了教科书级规模的形式化。在FATE-H基准测试中达到96%的证明成功率（基线为80%）。

Conclusion: M2F框架证明了大规模自动化数学文献形式化是可行的，其效率远超传统专家手动形式化所需的时间（从数月/年缩短到数周）。
...
### [17] [Sales Research Agent and Sales Research Bench](https://arxiv.org/abs/2602.17017)
*Deepanjan Bhol*
...
<details>
  <summary>Details</summary>
Motivation: 企业需要能够基于实时、定制化CRM数据回答销售领导问题的AI系统，但现有模型缺乏透明、可重复的质量证据。需要开发一个能够连接实时CRM数据、处理复杂模式并提供可靠洞察的AI应用。

Method: 开发了Sales Research Agent应用，连接实时CRM及相关数据，在复杂模式上进行推理，通过文本和图表输出决策就绪的洞察。引入了Sales Research Bench基准测试，在8个客户加权的维度上评估系统质量，包括文本和图表的基础性、相关性、可解释性、模式准确性和图表质量。
...
Conclusion: Sales Research Agent通过连接实时CRM数据并提供透明、可重复的质量证据，有效解决了企业销售领导的数据分析需求。Sales Research Bench基准测试为客户提供了评估和比较不同AI解决方案的可靠方法。
...
Method: 提出连续子价值Q学习(S2Q)，学习多个子价值函数保留替代高价值动作，结合Softmax行为策略鼓励持续探索
...
TL;DR: PBS是一种通过动态优先处理高损失样本来加速语言模型收敛的新型训练优化技术，使用轻量级线性预测器从静态特征估计样本难度，相比传统方法无需预定义难度指标或昂贵的逐样本损失跟踪。


<details>
  <summary>Details</summary>
Motivation: 传统课程学习方法需要预定义的难度指标，而硬样本挖掘方法需要昂贵的逐样本损失跟踪，这些方法在实际应用中存在局限性。作者希望开发一种更轻量、更高效的方法来加速语言模型训练。

Method: 提出预测性批量调度（PBS），使用在线训练的轻量级线性预测器，仅基于四个简单的词元级静态特征（词频、序列长度、词汇多样性和稀有词元比例）来估计样本难度，动态构建包含高损失样本的批次。

Result: 在130M参数Transformer上的实验表明，PBS实现了6-13%的收敛加速（通过训练检查点的评估损失衡量）。预测器与真实损失的相关系数从0.14提升到0.44（经过10,000训练步），证明词频统计能有效编码样本难度信息。

Conclusion: PBS验证了词频统计包含有关样本难度的有意义信息，能够以可忽略的计算开销实现有效的课程学习，为加速语言模型训练提供了一种实用且高效的解决方案。
...
TL;DR: 该论文提出一个人类-AI协作框架来生成可信的基准数据集，以解决不同ESG评级机构评分差异大的问题，提高可持续性评级的可比性和可信度。


<details>
  <summary>Details</summary>
Motivation: 不同可持续性评级机构对同一公司的ESG评分差异很大，这限制了评级的可比性、可信度和决策相关性，需要解决这一标准化问题。

Method: 提出STRIDE（可持续性信任评级与完整性数据方程）提供原则性标准和评分系统，指导使用大语言模型构建公司级基准数据集；同时提出SR-Delta差异分析程序框架，揭示潜在调整的见解。

Result: 该框架能够实现可持续性评级方法的可扩展和可比评估，为评级方法学提供标准化基准。

Conclusion: 呼吁AI社区采用AI驱动的方法来加强和推进可持续性评级方法学，以支持和执行紧迫的可持续性议程。
...
TL;DR: 论文提出了一种基于Owen值的新SHAP方法，通过满足T属性的特征分组策略改进视觉任务中的特征归因，在保持Shapley值理论基础上提升语义一致性和计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统Shapley值方法在视觉任务中假设特征独立，但像素特征存在空间和语义依赖性，现有分组方法（如轴对齐或SLIC分割）违反关键一致性属性，影响归因效果。

Method: 提出满足T属性的新分割方法，通过层次化特征分组确保语义对齐，利用Owen值（Shapley值的层次化推广）进行特征归因，支持计算剪枝。

Result: 在图像和表格数据集上的实验表明，O-Shap在归因精度、语义一致性和运行效率方面优于基线SHAP变体，特别是在结构重要的情况下表现更佳。

Conclusion: 通过满足T属性的特征分组策略，O-Shap方法在保持Shapley值理论优势的同时，显著提升了视觉任务中特征归因的准确性和可解释性。
...
TL;DR: InstructKG：一个自动构建教师对齐知识图谱的框架，从课程材料中提取概念节点和学习依赖关系，用于个性化学习中的知识差距诊断


<details>
  <summary>Details</summary>
Motivation: 掌握教育概念需要理解其先决条件和子概念，这对于识别学生知识差距和实现个性化学习至关重要。在大规模课程中，教师难以诊断个体误解或确定需要强化的概念。现有知识图谱方法要么停留在表面层次（课程级概念或后勤关系），要么忽略了教学材料中丰富的教学信号。
...
Conclusion: InstructKG框架能够自动构建捕获课程预期学习进展的知识图谱，为个性化学习中的知识差距诊断和针对性干预提供了有效工具。
...
### [23] [Epistemology of Generative AI: The Geometry of Knowing](https://arxiv.org/abs/2602.17116)
*Ilya Levin*

Main category: cs.AI

TL;DR: 论文提出高维空间索引认识论，认为生成式AI通过在高维语义空间中导航产生知识，这代表了与符号推理和统计重组不同的第三种知识生产方式。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的认知机制仍然模糊，缺乏对其工作原理的哲学理解，这阻碍了其在科学、教育和制度生活中的负责任整合。需要突破传统的图灵-香农-冯·诺依曼范式，建立新的认识论框架。

Method: 基于高维几何的四个结构特性（测度集中、近正交性、指数方向容量和流形正则性），结合皮尔士符号学和帕珀特建构主义，发展高维空间的索引认识论，将生成模型重新概念化为学习流形的导航器。

Result: 提出了导航知识作为第三种知识生产模式，区别于符号推理和统计重组。生成式AI通过在语义参数的高维几何空间中定位和导航来产生内容，这种空间构成了生成生产的主动认知条件。

Conclusion: 生成式AI代表了一种根本性的认识论转变，需要新的哲学框架来理解其知识生产方式。高维空间索引认识论为理解生成式AI的认知机制提供了理论基础，导航知识作为新的知识生产模式具有重要哲学意义。
...
TL;DR: 提出一种新颖的并行算法用于分解困难的CircuitSAT实例，通过专用约束将原始SAT实例分割成一系列弱化公式，参数化设计允许高效识别高质量分解


<details>
  <summary>Details</summary>
Motivation: CircuitSAT问题在电路等价性验证和密码学攻击等应用中具有重要价值，但困难实例的求解效率低下，需要更有效的分解方法

Method: 采用并行算法框架，使用专用约束将原始SAT实例分割成弱化公式族，通过参数调整在并行计算指导下高效识别高质量分解

Result: 在具有挑战性的CircuitSAT实例上验证了算法的实际效果，包括布尔电路逻辑等价性验证和密码哈希函数原像攻击等应用场景

Conclusion: 提出的参数化并行分解算法能够有效处理困难的CircuitSAT实例，为电路验证和密码分析等应用提供了新的解决方案
...
TL;DR: JEPA-DNA是一个结合联合嵌入预测架构和传统生成目标的基因组基础模型预训练框架，通过潜在空间预测提升功能上下文理解能力


<details>
  <summary>Details</summary>
Motivation: 现有的基因组基础模型主要依赖掩码语言建模或下一标记预测，这些方法擅长捕捉局部基因组语法和精细的基序模式，但往往无法捕获更广泛的功能上下文，导致表示缺乏全局生物学视角

Method: 引入JEPA-DNA框架，将联合嵌入预测架构与传统生成目标结合。通过潜在接地方法，将标记级恢复与潜在空间中的预测目标耦合，通过监督CLS标记来预测掩码基因组片段的高级功能嵌入，而不仅仅是关注单个核苷酸

Result: 在多样化的基因组基准测试中，JEPA-DNA在监督和零样本任务上始终优于仅生成式基线模型，提供更稳健和生物学基础的表示

Conclusion: JEPA-DNA为理解基因组字母和序列底层功能逻辑的基础模型提供了可扩展的路径，超越了传统生成式方法，实现了更好的生物学功能上下文理解
...
### [26] [Texo: Formula Recognition within 20M Parameters](https://arxiv.org/abs/2602.17189)
*Sicheng Mao*

Main category: cs.AI

TL;DR: Texo是一个仅含2000万参数的轻量级公式识别模型，通过精心设计、蒸馏和词汇表/分词器迁移，性能媲美SOTA模型，同时模型大小分别减少了80%和65%，支持消费级硬件实时推理和浏览器部署。
...
Method: 采用注意力机制设计、知识蒸馏技术，以及词汇表和分词器的迁移方法，构建仅含2000万参数的简约模型架构。

Result: Texo在性能上与UniMERNet-T和PPFormulaNet-S等SOTA模型相当，同时模型大小分别减少了80%和65%，实现了消费级硬件上的实时推理和浏览器部署能力。
...
Method: 采用"方法论实验"定位，设计七阶段模块化工作流，基于任务模块化、人机分工和可验证性三原则。以台湾Claude.ai使用数据（N=7,729个对话）为实证案例，展示工作流在二手数据分析中的应用。

Result: 成功验证了AI Agent协作工作流的可行性，识别出三种人机协作操作模式：直接执行、迭代优化和人类主导。揭示了人类在研究问题制定、理论解释、情境化推理和伦理反思中的不可替代性。

Conclusion: 本研究为人文社科研究者提供了可复制的AI协作框架，强调了人类判断在关键研究环节的核心作用。承认了单平台数据、横断面设计和AI可靠性风险等局限性。
...
### [28] [Decoding the Human Factor: High Fidelity Behavioral Prediction for Strategic Foresight](https://arxiv.org/abs/2602.17222)
*Ben Yellin,Ehud Ezra,Mark Foreman,Shula Grinapol*

Main category: cs.AI

TL;DR: 提出大型行为模型（LBM），通过行为嵌入而非临时提示来预测个体战略决策，在复杂心理特征与情境约束交互中实现高保真行为预测。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在预测人类高风险决策时存在局限性，难以生成一致、个体特定的行为，特别是当准确预测依赖于心理特征与情境约束的复杂交互时。基于提示的方法在此设置中脆弱，存在身份漂移问题，且无法有效利用详细人物描述。

Method: 引入大型行为模型（LBM），这是一种行为基础模型，通过条件化基于全面心理测量工具得出的结构化高维特征配置文件，从临时人物提示转向行为嵌入。在连接稳定倾向、动机状态和情境约束与观察到的选择的专有数据集上进行微调。

Result: 在保留场景评估中，LBM微调相对于未适应的Llama-3.1-8B-Instruct骨干模型改进了行为预测，当基于大五人格特质条件化时，性能与前沿基线相当。研究发现基于提示的基线存在复杂性上限，而LBM继续受益于日益密集的特征配置文件，随着提供更多特征维度，性能持续提升。

Conclusion: LBM为高保真行为模拟提供了一种可扩展方法，可在战略预见、谈判分析、认知安全和决策支持等领域实现应用。
...
TL;DR: 该研究使用布鲁姆分类法作为层次化视角，通过分析不同大语言模型的高维激活向量，探究不同认知层次（从基础回忆到抽象综合）是否在模型的残差流中线性可分。结果显示线性分类器在所有布鲁姆层次上达到约95%的平均准确率，证明认知层次被编码在模型表征的线性可访问子空间中。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的黑箱特性需要超越表面性能指标的新型评估框架。本研究旨在探索认知复杂性在模型内部神经表征中的体现，使用布鲁姆分类法作为层次化视角来理解模型如何处理不同认知难度的任务。

Method: 通过分析不同大语言模型的高维激活向量，使用布鲁姆分类法作为认知层次框架，探究不同认知水平（从Remember到Create）是否在模型的残差流中线性可分。研究采用线性分类器来测试认知层次在模型表征中的可分离性。

Result: 线性分类器在所有布鲁姆认知层次上达到约95%的平均准确率，提供了强有力的证据表明认知层次被编码在模型表征的线性可访问子空间中。研究发现模型在正向传播早期就解析了提示的认知难度，且表征在不同层中变得越来越可分。

Conclusion: 该研究证明了大语言模型内部存在对认知复杂性的系统编码，认知层次在模型的神经表征中是线性可分的。这为理解大语言模型如何处理不同认知难度的任务提供了新的视角，并为开发更精细的模型评估框架奠定了基础。
...
### [30] [All Leaks Count, Some Count More: Interpretable Temporal Contamination Detection in LLM Backtesting](https://arxiv.org/abs/2602.17234)
...
<details>
  <summary>Details</summary>
Motivation: 评估LLM预测未来事件需要回溯测试，但LLM可能无意中泄露训练时学到的截止日期后知识，影响评估有效性

Method: 1) 提出声明级框架检测时间知识泄露，将模型推理分解为原子声明并按时间可验证性分类；2) 使用Shapley值衡量每个声明对预测的贡献，计算Shapley-DCLR指标；3) 开发TimeSPEC方法，通过声明验证和再生主动过滤时间污染

Result: 在350个实例（美国最高法院案件预测、NBA薪资估计、股票回报排名）的实验显示，标准提示方法存在显著泄露，TimeSPEC能降低Shapley-DCLR同时保持任务性能

Conclusion: 显式的、可解释的声明级验证优于基于提示的时间约束，TimeSPEC方法能有效减少时间知识泄露，为可靠的回溯测试提供解决方案
...
### [31] [ArXiv-to-Model: A Practical Study of Scientific LM Training](https://arxiv.org/abs/2602.17288)
*Anuj Gupta*
...
<details>
  <summary>Details</summary>
Motivation: 虽然前沿大语言模型展现出强大的推理和数学能力，但如何从原始科学文献训练领域专用语言模型的实践过程缺乏详细文档。本研究旨在填补这一空白，为有限计算预算的研究者提供可复现的工程指导。

Method: 开发了端到端训练流程：包括元数据过滤、存档验证、LaTeX提取、文本规范化、领域感知分词，以及在2xA100 GPU上使用密集transformer进行训练。通过24次实验运行分析训练稳定性、扩展行为和数据损失。

Result: 研究发现预处理决策显著影响可用token数量，分词策略影响符号稳定性，存储和I/O限制可能成为与计算同等重要的瓶颈因素。在数据丰富环境下（520亿预训练token）实现了稳定的训练收敛。

Conclusion: 本文提供了工程导向、透明的科学语言模型训练记录，而非提出新架构。这些见解有助于在中等计算预算下构建领域专用模型的研究者，强调了数据处理和基础设施考虑的重要性。
...
### [32] [Dataless Weight Disentanglement in Task Arithmetic via Kronecker-Factored Approximate Curvature](https://arxiv.org/abs/2602.17385)
...
Method: 提出一种无需数据的方法，将针对表示漂移的正则化框架化为曲率矩阵近似问题。采用Kronecker分解近似曲率技术，获得一个实用的正则化器，在任务添加和否定方面达到最先进结果。

Result: 该方法在任务添加和否定方面取得最先进结果，具有与任务数量无关的恒定复杂度，促进对任务向量重新缩放的鲁棒性，无需保留调优。

Conclusion: 通过曲率矩阵近似实现的无数据正则化方法有效解决了任务算术中的表示漂移问题，在保持模块化和数据隐私约束的同时，实现了高性能的任务组合操作。
...
### [33] [Visual Model Checking: Graph-Based Inference of Visual Routines for Image Retrieval](https://arxiv.org/abs/2602.17386)
*Adrià Molina,Oriol Ramos Terrades,Josep Lladós*

Main category: cs.AI

TL;DR: 提出一种将形式化验证与深度学习图像检索相结合的新框架，通过图验证和神经代码生成处理复杂查询，提供可验证的透明检索结果
...
Method: 结合图验证方法和神经代码生成，将形式化验证集成到深度学习图像检索中，通过形式推理系统验证用户查询中的每个原子真值

Result: 框架不仅能返回匹配结果，还能识别和标记哪些具体约束被满足、哪些未满足，提供更透明、可问责的检索过程，同时提升最流行嵌入方法的性能

Conclusion: 通过形式化验证与深度学习的协同结合，实现了对开放词汇自然语言查询的可信、可验证图像检索，超越了向量表示的模糊性和近似性
...
### [34] [A Contrastive Variational AutoEncoder for NSCLC Survival Prediction with Missing Modalities](https://arxiv.org/abs/2602.17402)
...
<details>
  <summary>Details</summary>
Motivation: 非小细胞肺癌生存预测因个体预后特征差异而困难，多模态数据（全切片图像、转录组、DNA甲基化）提供互补信息但临床数据常严重缺失，现有方法在严重缺失情况下缺乏鲁棒性。

Method: 提出多模态对比变分自编码器（MCVAE）：模态特定变分编码器捕捉各数据源不确定性；融合瓶颈带学习门控机制归一化现有模态贡献；多任务目标结合生存损失和重建损失正则化患者表示；跨模态对比损失强制潜在空间对齐；训练时应用随机模态掩蔽提高对任意缺失模式的鲁棒性。

Result: 在TCGA-LUAD（475例）和TCGA-LUSC（446例）数据集上广泛评估，证明该方法在预测疾病特异性生存方面有效，且在严重缺失情况下比两种最先进模型更具鲁棒性。

Conclusion: MCVAE能有效处理多模态缺失数据，但测试所有模态子集发现多模态整合并非总是对任务有益，这澄清了多模态整合的适用性。
...
### [35] [A Privacy by Design Framework for Large Language Model-Based Applications for Children](https://arxiv.org/abs/2602.17418)
*Diana Addae,Diana Rogachova,Nafiseh Kahani,Masoud Barati,Michael Christensen,Chen Zhou*

Main category: cs.AI

TL;DR: 本文提出一个基于隐私设计原则的框架，指导开发者为儿童AI应用（特别是LLM应用）提供隐私保护，结合多国法规和儿童权利原则，并通过教育辅导案例验证可行性。


<details>
  <summary>Details</summary>
Motivation: 儿童越来越多地使用AI技术，但面临隐私风险。现有隐私法规要求企业提供保护，但在实践中实施困难。需要为AI服务提供商和开发者提供实用框架，在满足法律标准的同时降低儿童隐私风险。

Method: 提出基于隐私设计（PbD）的框架，整合GDPR、PIPEDA、COPPA等隐私法规原则，映射到LLM应用的数据收集、模型训练、运营监控和持续验证等阶段。结合UNCRC、AADC等儿童权利和适龄设计指南，为每个阶段提供学术文献中的操作控制措施。

Result: 框架展示了如何通过技术和组织控制以及适龄设计决策，支持开发符合法律要求的儿童AI应用。通过13岁以下儿童LLM教育辅导案例研究，验证了框架的实际应用可行性。

Conclusion: 通过在整个LLM生命周期中采用数据保护策略和适龄设计决策，可以支持开发既提供隐私保护又符合法律要求的儿童AI应用。该框架为AI服务提供商和开发者提供了实用的指导。
...
### [36] [WarpRec: Unifying Academic Rigor and Industrial Scale for Responsible, Reproducible, and Efficient Recommendation](https://arxiv.org/abs/2602.17442)
...
TL;DR: WarpRec是一个高性能推荐系统框架，通过后端无关架构解决了学术界与工业界之间的鸿沟，支持50+算法、40个指标和19种策略，能够无缝从本地执行扩展到分布式训练，同时集成了能源追踪功能，为可持续、面向智能体的下一代推荐系统提供架构基础。


<details>
  <summary>Details</summary>
Motivation: 当前推荐系统创新受到生态系统分裂的阻碍，研究人员必须在内存实验的便利性与工业级分布式引擎的复杂重写之间做出选择，这限制了研究进展和实际应用。

Method: 提出了WarpRec框架，采用新颖的后端无关架构，包含50多种最先进算法、40个评估指标和19种过滤与分割策略，能够无缝从本地执行扩展到分布式训练和优化，并集成了CodeCarbon进行实时能源追踪。

Result: WarpRec消除了学术界与工业界之间的鸿沟，展示了可扩展性不必以科学完整性或可持续性为代价，同时为推荐系统向生成式AI生态系统中的交互工具演进提供了架构基础。

Conclusion: WarpRec不仅弥合了学术界与工业界之间的差距，还可以作为下一代可持续、面向智能体的推荐系统的架构骨干，代码已开源。
...
### [37] [Pareto Optimal Benchmarking of AI Models on ARM Cortex Processors for Sustainable Embedded Systems](https://arxiv.org/abs/2602.17508)
*Pranay Jain,Maximilian Kasper,Göran Köber,Axel Plinge,Dominik Seuß*

Main category: cs.AI

TL;DR: 提出了一个针对ARM Cortex处理器（M0+, M4, M7）的AI模型优化基准测试框架，通过自动化测试平台系统评估能耗、精度和资源利用率，发现浮点运算与推理时间呈近线性关系，并利用帕累托分析平衡能耗与精度权衡。


<details>
  <summary>Details</summary>
Motivation: 嵌入式系统中AI模型部署面临能耗、精度和资源利用率的平衡挑战，需要针对ARM Cortex处理器提供系统化的优化框架，帮助开发者设计既高性能又节能的AI应用。

Method: 设计了自动化测试平台，系统评估ARM Cortex M0+、M4、M7处理器的AI模型性能，通过关键性能指标（KPIs）分析，使用帕累托分析平衡能耗与精度权衡，研究浮点运算与推理时间的相关性。

Result: 发现浮点运算（FLOPs）与推理时间存在近线性相关，可作为计算需求估计的可靠指标。M7处理器适合短推理周期任务，M4处理器在长推理任务中能效更高，M0+处理器适合简单AI任务。

Conclusion: 为开发者提供了实用的AI系统设计指导，通过系统化基准测试和帕累托分析，可在嵌入式系统中实现能耗、精度和性能的优化平衡，推动节能高效AI应用的发展。
...
### [38] [Enhancing Large Language Models (LLMs) for Telecom using Dynamic Knowledge Graphs and Explainable Retrieval-Augmented Generation](https://arxiv.org/abs/2602.17529)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在电信领域应用面临挑战：领域复杂性高、标准不断演进、专业术语多，导致通用大语言模型在该领域容易产生幻觉、准确性不足

Method: 提出KG-RAG框架，将知识图谱（提供电信标准和文档的结构化知识表示）与检索增强生成（动态检索相关事实）相结合，增强大语言模型在电信任务中的表现

Result: 在基准数据集上的实验表明，KG-RAG优于纯大语言模型和标准RAG基线，平均准确率分别提升21.6%和14.3%

Conclusion: KG-RAG框架能有效提高大语言模型在复杂电信场景中的准确性、可靠性和可解释性，为电信领域的大语言模型应用提供了可行解决方案
...
### [39] [Evaluating Chain-of-Thought Reasoning through Reusability and Verifiability](https://arxiv.org/abs/2602.17544)
...
Method: 采用Thinker-Executor框架将CoT生成与执行解耦，引入可重用性（衡量Executor重用Thinker的CoT的容易程度）和可验证性（衡量Executor使用CoT匹配Thinker答案的频率）两个新指标。在5个基准测试上评估4个Thinker模型和10个Executor模型委员会。

Result: 可重用性和可验证性与标准准确率不相关，暴露了当前基于准确率的排行榜在评估推理能力方面的盲点。令人惊讶的是，专门推理模型的CoT并不比通用LLM（如Llama和Gemma）的CoT更可重用或可验证。

Conclusion: 需要超越准确率的新评估指标来全面评估推理能力，专门推理模型在CoT质量方面不一定优于通用LLM，这对多智能体IR系统中的模型选择有重要启示。
...
### [40] [KLong: Training LLM Agent for Extremely Long-horizon Tasks](https://arxiv.org/abs/2602.17547)
*Yue Liu,Zhiyuan Hu,Flood Sung,Jiaheng Zhang,Bryan Hooi*

Main category: cs.AI

TL;DR: KLong是一个开源LLM智能体，通过轨迹分割SFT和渐进式RL训练来解决超长视野任务，在PaperBench等基准上超越了Kimi K2 Thinking等模型。
...
Conclusion: 提出的轨迹分割SFT和渐进式RL训练方法有效提升了LLM智能体处理超长视野任务的能力，KLong在多个基准上展现了优越性能。
...
### [41] [ODESteer: A Unified ODE-Based Steering Framework for LLM Alignment](https://arxiv.org/abs/2602.17560)
...
TL;DR: 本文提出了一种基于常微分方程的统一理论框架ODESteer，用于改进大语言模型中的激活导向对齐方法，通过屏障函数和多步自适应导向实现更好的对齐效果。


<details>
  <summary>Details</summary>
Motivation: 当前激活导向方法存在两个主要问题：缺乏统一的理论框架来指导导向方向设计，以及过度依赖单步导向而无法捕捉激活分布的复杂模式。

Method: 提出基于常微分方程的ODESteer方法，将传统激活加法解释为ODE的一阶近似，通过控制理论中的屏障函数设计导向方向，使用正负激活的对数密度比作为屏障函数，实现多步自适应导向。

Result: ODESteer在多个LLM对齐基准测试中表现优于现有方法，在TruthfulQA上提升5.7%，UltraFeedback上提升2.5%，RealToxicityPrompts上提升2.4%。

Conclusion: 该工作通过ODE统一了激活导向的理论基础，提出的ODESteer方法在理论和实证上都取得了进展，为大语言模型对齐提供了新的原则性视角。
...
### [42] [A Hybrid Federated Learning Based Ensemble Approach for Lung Disease Diagnosis Leveraging Fusion of SWIN Transformer and CNN](https://arxiv.org/abs/2602.17566)
*Asif Hasan Chowdhury,Md. Fahim Islam,M Ragib Anjum Riad,Faiyaz Bin Hashem,Md Tanzim Reza,Md. Golam Rabiul Alam*

Main category: cs.AI

TL;DR: 本文提出了一种基于联邦学习的混合AI模型，结合SWIN Transformer和CNN技术，用于通过X光片诊断COVID-19和肺炎，旨在构建安全、分布式的医疗数据处理系统。


<details>
  <summary>Details</summary>
Motivation: 随着计算能力的显著提升，人工智能在医疗健康领域应用前景广阔。医疗专家和医院需要共享数据空间，但医疗数据具有敏感性和隐私性。通过结合人工智能和联邦学习技术，可以构建安全、分布式的医疗数据处理系统，提高疾病诊断的准确性和可靠性。

Method: 采用混合联邦学习模型，结合最新的CNN模型（DenseNet201、Inception V3、VGG 19）和Transformer模型（SWIN Transformer）。使用TensorFlow和Keras框架，以及微软开发的Vision Transformer技术。通过联邦学习实现分布式训练，保护数据隐私，同时利用实时持续学习方法提高模型性能。

Result: 虽然论文摘要中没有提供具体的实验结果数据，但该方法旨在提高COVID-19和肺炎诊断的准确性，并通过联邦学习确保模型安全性和信息真实性。混合模型结合了CNN和Transformer的优势，为医生提供可靠的辅助诊断工具。

Conclusion: 联邦学习支持的混合AI模型能够有效提高疾病诊断和严重程度预测的准确性，同时确保医疗数据的安全性和隐私保护。这种结合最新深度学习技术和联邦学习的方法为医疗领域提供了创新的解决方案，有助于全球共同应对疫情挑战。
...
### [43] [AI Gamestore: Scalable, Open-Ended Evaluation of Machine General Intelligence with Human Games](https://arxiv.org/abs/2602.17594)
...
TL;DR: 论文提出用"人类游戏宇宙"作为评估AI通用智能的新方法，并开发了AI GameStore平台，通过LLM生成代表性人类游戏来测试前沿视觉语言模型，结果显示当前模型在大多数游戏上表现远低于人类水平。


<details>
  <summary>Details</summary>
Motivation: 当前AI基准测试通常只评估狭窄能力，且容易饱和，无法全面评估类似人类的通用智能。需要一种更全面、动态的方法来评估AI系统在人类智能广度上的表现。

Method: 提出使用"人类游戏宇宙"概念——所有人类为人类设计的游戏空间。开发了AI GameStore平台，利用LLM和人类参与，从Apple App Store和Steam等平台自动获取并容器化游戏环境，生成代表性人类游戏。作为概念验证，生成了100个游戏并测试了7个前沿视觉语言模型。

Result: 最好的模型在大多数游戏上得分不到人类平均水平的10%。模型在挑战世界模型学习、记忆和规划能力的游戏上表现尤其困难，显示出当前AI系统与人类通用智能之间的显著差距。

Conclusion: AI GameStore为评估和推动机器向人类通用智能发展提供了实用方法。结果表明当前AI系统在人类游戏表现上仍有很大提升空间，特别是在复杂认知能力方面。
...
### [44] [MolHIT: Advancing Molecular-Graph Generation with Hierarchical Discrete Diffusion Models](https://arxiv.org/abs/2602.17602)
...
<details>
  <summary>Details</summary>
Motivation: 现有的图扩散模型在分子生成中存在化学有效性低、难以满足期望性质的问题，相比1D建模表现不佳。需要开发能够克服这些性能限制的新方法。

Method: MolHIT基于层次离散扩散模型，将离散扩散推广到编码化学先验的额外类别，并采用解耦原子编码，根据原子的化学角色分离原子类型。
...
### [45] [CLEF HIPE-2026: Evaluating Accurate and Efficient Person-Place Relation Extraction from Multilingual Historical Texts](https://arxiv.org/abs/2602.17663)
...
Method: HIPE-2026采用基于评估实验室的方法，要求系统从多语言历史文本中提取两种类型的人物-地点关系（at和isAt），需要推理时间和地理线索。实验室引入了三重评估框架，联合评估准确性、计算效率和领域泛化能力。

Result: 作为评估实验室，HIPE-2026本身不提供具体系统结果，而是为参与者提供了一个评估框架，用于测试和比较不同系统在人物-地点关系提取任务上的性能，特别是在准确性、效率和泛化能力方面的表现。

Conclusion: HIPE-2026成功扩展了HIPE系列，将重点从命名实体识别转向语义关系提取，为历史文本中的人物-地点关联提取建立了标准化评估框架，有助于推动数字人文领域的大规模历史数据处理技术发展。
...
### [46] [Beyond Procedure: Substantive Fairness in Conformal Prediction](https://arxiv.org/abs/2602.16794)
*Pengqi Liu,Zijun Yu,Mouloud Belbahri,Arthur Charpentier,Masoud Asgharian,Jesse C. Cresswell*

Main category: stat.ML

TL;DR: 该研究探讨了保形预测（CP）与下游决策公平性的关系，超越了传统的程序公平性，关注实质性公平。通过理论分析和实验验证，发现标签聚类的CP变体能提供更好的实质性公平，且均衡的预测集大小而非覆盖率与实质性公平强相关。


<details>
  <summary>Details</summary>
Motivation: 保形预测虽然能为机器学习模型提供分布无关的不确定性量化，但其与下游决策公平性的相互作用尚未得到充分探索。现有研究主要关注保形预测作为独立操作的程序公平性，而缺乏对整个决策流程中实质性公平（下游结果的公平性）的分析。

Method: 1. 理论分析：推导了预测集大小差异的上界，将其分解为可解释的组成部分，阐明标签聚类CP如何控制方法驱动的公平性问题。2. 实证方法：引入了LLM-in-the-loop评估器，近似人类对不同模态的实质性公平评估。3. 实验设计：通过实验比较不同CP变体的表现，特别关注标签聚类CP的效果。

Result: 1. 标签聚类的CP变体在实验中始终提供更优的实质性公平性。2. 实证表明，均衡的预测集大小（而非覆盖率）与改善的实质性公平性有强相关性。3. LLM-in-the-loop评估器能够有效近似人类对实质性公平的评估。

Conclusion: 该研究为设计更公平的保形预测系统提供了理论和实证基础。通过关注实质性公平而非仅仅程序公平，并利用标签聚类CP和均衡预测集大小，实践者可以构建更公平的决策系统。研究还展示了LLM在评估公平性方面的实用性。
...
### [47] [Poisson-MNL Bandit: Nearly Optimal Dynamic Joint Assortment and Pricing with Decision-Dependent Customer Arrivals](https://arxiv.org/abs/2602.16923)
*Junhui Cai,Ran Chen,Qitao Huang,Linda Zhao,Wu Zhu*

Main category: stat.ML

TL;DR: 论文研究动态联合品类与定价决策，考虑品类和价格不仅影响顾客选择还影响顾客到达率，提出Poisson-MNL模型和基于UCB的PMNL算法，实现√TlogT的遗憾界。


<details>
  <summary>Details</summary>
Motivation: 传统MNL模型假设顾客到达率固定，但现实中品类和价格会影响顾客到达数量，忽略这一依赖关系会导致次优决策。需要开发能同时学习顾客选择和到达模型的动态决策方法。

Method: 提出Poisson-MNL模型，将上下文MNL选择模型与泊松到达模型结合，到达率依赖于提供的品类和价格。基于UCB思想开发PMNL算法进行动态联合品类与定价决策。

Result: 证明PMNL算法具有√TlogT的非渐近遗憾界，并给出匹配的下界（至多logT因子）。仿真研究表明PMNL能有效学习顾客选择和到达模型，优于假设固定到达率的方法。

Conclusion: 考虑品类和价格对顾客到达率的影响至关重要。Poisson-MNL模型和PMNL算法为动态联合品类与定价决策提供了理论保证和实际有效的解决方案。
...
### [48] [Anti-causal domain generalization: Leveraging unlabeled data](https://arxiv.org/abs/2602.17187)
*Sorawit Saengkyongam,Juan L. Gamella,Andrew C. Miller,Jonas Peters,Nicolai Meinshausen,Christina Heinze-Deml*

Main category: stat.ML

TL;DR: 该论文研究领域泛化问题，提出在反因果设置下利用无标签数据增强模型对分布变化的鲁棒性，通过惩罚模型对协变量跨环境变化的敏感性来实现领域泛化。


<details>
  <summary>Details</summary>
Motivation: 现有领域泛化方法通常需要来自多个训练环境的标记数据，这在标记数据稀缺时限制了其适用性。本文旨在解决这一问题，特别是在反因果设置下，利用环境扰动对协变量的影响不会传播到结果这一特性。

Method: 提出两种方法：1）惩罚模型对协变量跨环境均值变化的敏感性；2）惩罚模型对协变量跨环境协方差变化的敏感性。这些方法利用无标签数据估计扰动方向，无需标记数据。

Result: 证明了这些方法在特定环境类别下具有最坏情况最优性保证，并在受控物理系统和生理信号数据集上展示了方法的经验性能。

Conclusion: 在反因果设置下，通过利用无标签数据来正则化模型对协变量跨环境变化的敏感性，可以有效实现领域泛化，特别是在标记数据稀缺的情况下。
...
### [49] [genriesz: A Python Package for Automatic Debiased Machine Learning with Generalized Riesz Regression](https://arxiv.org/abs/2602.17543)
*Masahiro Kato*
...
<details>
  <summary>Details</summary>
Motivation: 自动化因果和结构参数的高效估计，通过Riesz表示定理和去偏机器学习方法，为研究人员提供统一的框架来估计Riesz表示器，简化复杂的因果推断流程。

Method: 实现自动DML和广义Riesz回归，通过最小化经验Bregman散度来估计Riesz表示器。采用自动回归器平衡设计原则，给定Bregman生成器和表示器模型类，自动构建兼容的链接函数，使估计器满足用户选择基中的平衡最优条件。

Result: 开发了genriesz开源Python包，提供模块化接口，支持回归调整、Riesz加权、增强Riesz加权和TMLE风格估计器，包含交叉拟合、置信区间和p值计算，可用于平均处理效应、处理组平均处理效应和平均边际效应等估计问题。

Conclusion: genriesz为因果推断提供了一个强大而灵活的工具，通过自动化Riesz表示器估计和去偏机器学习，简化了复杂因果参数的估计过程，提高了研究效率和准确性。
...
### [50] [SOLVAR: Fast covariance-based heterogeneity analysis with pose refinement for cryo-EM](https://arxiv.org/abs/2602.17603)
*Roey Yadgar,Roy R. Lederman,Yoel Shkolnisky*

Main category: stat.ML

TL;DR: SOLVAR是一种基于低秩协方差矩阵假设的冷冻电镜连续异质性分析方法，通过随机优化高效估计主成分，并能同时优化粒子图像姿态。


<details>
  <summary>Details</summary>
Motivation: 冷冻电镜中表征分子的连续构象异质性是一个关键挑战，现有协方差方法在高效估计协方差矩阵方面存在计算困难。

Method: 提出SOLVAR方法，利用协方差矩阵的低秩假设，将其转化为可快速准确求解的优化问题，并能够同时优化输入粒子图像的姿态。

Result: 在合成和实验数据集上的数值实验表明，算法能准确捕捉主要变异成分并保持计算效率，在最近的异质性基准测试中达到最先进性能。

Conclusion: SOLVAR为冷冻电镜连续异质性分析提供了一种高效准确的协方差估计方法，代码已开源。
...
### [51] [Guiding LLM-Based Human Mobility Simulation with Mobility Measures from Shared Data](https://arxiv.org/abs/2602.16726)
*Hua Yan,Heng Tan,Yu Yang*

Main category: cs.MA

TL;DR: M2LSimu：一个基于移动度量指导的多提示调整框架，通过共享数据衍生的移动度量作为指导来优化个体级提示，实现更真实的人群移动模拟，解决了现有LLM方法无法捕捉集体行为涌现的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型人类移动模拟方法虽然能模拟个体认知过程，但缺乏群体层面的协调机制，无法捕捉集体行为的涌现，这限制了大规模人群移动模拟的实用性。

Method: 设计M2LSimu框架，利用共享数据衍生的移动度量作为指导，通过粗粒度调整策略逐步实现细粒度个体级适配，在有限预算下满足多个群体层面的移动目标。

Result: 在两个公开数据集上的实验表明，M2LSimu显著优于最先进的基于LLM的方法。

Conclusion: M2LSimu通过移动度量指导的多提示调整框架，成功解决了LLM方法在人群移动模拟中缺乏群体协调机制的问题，能够更真实地模拟集体行为的涌现。
...
### [52] [Self-Evolving Multi-Agent Network for Industrial IoT Predictive Maintenance](https://arxiv.org/abs/2602.16738)
*Rebin Saleh,Khanh Pham Dinh,Balázs Villányi,Truong-Son Hy*

Main category: cs.MA

TL;DR: SEMAS是一个自演化的分层多智能体系统，用于工业物联网预测性维护中的实时异常检测，通过边缘、雾和云计算层的分布式智能体协作，在保证可解释性的同时实现低延迟和高适应性。


<details>
  <summary>Details</summary>
Motivation: 工业物联网预测性维护需要能够实时检测异常的系统，同时不能牺牲可解释性或要求过多计算资源。传统方法依赖静态离线训练模型，无法适应变化的操作条件，而基于LLM的单体系统需要过多内存和延迟，不适合边缘部署。

Method: 提出SEMAS分层多智能体系统：边缘智能体进行轻量级特征提取和预过滤；雾智能体执行多样化集成检测和动态共识投票；云智能体通过近端策略优化（PPO）持续优化系统策略，同时保持异步非阻塞推理。框架包含基于LLM的响应生成和联邦知识聚合。

Result: 在两个工业基准测试（锅炉模拟器和风力涡轮机）上，SEMAS实现了卓越的异常检测性能和适应稳定性，在不同操作环境下保持预测准确性，并显著降低延迟实现真正实时部署。消融研究证实PPO驱动的策略演化、共识投票和联邦聚合都对系统有效性有重要贡献。

Conclusion: 研究表明，资源感知的自演化多智能体协调对于在严格延迟和可解释性约束下的生产就绪工业物联网预测性维护至关重要。
...
### [53] [AdaptOrch: Task-Adaptive Multi-Agent Orchestration in the Era of LLM Performance Convergence](https://arxiv.org/abs/2602.16873)
*Geunbin Yu*

Main category: cs.MA

TL;DR: 随着不同供应商的大语言模型在基准测试性能上趋同，传统的为每个任务选择单一最佳模型的范式收益递减。AdaptOrch框架通过任务自适应的多智能体编排拓扑（并行、顺序、分层、混合）来提升系统性能，证明编排设计已成为独立于模型扩展的一级优化目标。


<details>
  <summary>Details</summary>
Motivation: 当不同供应商的大语言模型在基准测试性能上趋于相似时，传统的为每个任务选择单一最佳模型的范式收益递减。作者认为编排拓扑（多个智能体如何协调、并行和合成的结构组成）现在主导系统级性能，超越了单个模型能力。

Method: 提出AdaptOrch框架，包含三个关键贡献：1) 性能收敛缩放定律，形式化编排选择优于模型选择的条件；2) 拓扑路由算法，将任务分解DAG映射到最优编排模式；3) 自适应合成协议，具有可证明的终止保证和并行智能体输出的启发式一致性评分。

Result: 在编码（SWE-bench）、推理（GPQA）和检索增强生成任务上验证，拓扑感知的编排比静态单拓扑基线实现了12-23%的性能提升，即使使用相同的底层模型。

Conclusion: 编排设计已成为独立于模型扩展的一级优化目标。随着模型性能趋同，编排拓扑的选择对系统性能的影响超过了单个模型的选择。
...
### [54] [AgentConductor: Topology Evolution for Multi-Agent Competition-Level Code Generation](https://arxiv.org/abs/2602.17100)
...
Method: 1) 设计拓扑密度函数，捕捉多智能体交互的通信感知数学特征；2) 采用难度区间划分，避免过度剪枝，实现更精细的密度控制；3) 基于强化学习优化，使用LLM编排器为核心，为每个查询推断智能体角色和任务难度，构建任务适应、密度感知的分层有向无环图拓扑。

Result: 在三个竞赛级和两个基础代码数据集上，AgentConductor在pass@1准确率上比最强基线提升高达14.6%，密度减少13%，token成本降低68%，达到最先进水平。

Conclusion: AgentConductor通过动态生成任务适应的交互拓扑，解决了现有多智能体系统在拓扑密度适应性和迭代优化方面的不足，显著提升了代码生成性能并降低了通信成本。
...
### [55] [Algorithmic Collusion at Test Time: A Meta-game Design and Evaluation](https://arxiv.org/abs/2602.17203)
*Yuhong Luo,Daniel Schoepflin,Xintong Wang*

Main category: cs.MA

TL;DR: 研究算法共谋风险，提出元博弈设计分析测试时间约束下的算法行为，评估强化学习和LLM策略在重复定价博弈中的共谋可行性


<details>
  <summary>Details</summary>
Motivation: 算法共谋的威胁及其是否需要监管干预仍存在争议，现有评估常依赖长学习周期、对手理性假设以及玩家间的对称性，需要研究实际测试环境下的共谋风险

Method: 引入元博弈设计，将智能体建模为具有不同策略特征（竞争性、天真合作、稳健共谋）的预训练策略，通过元策略选择结合初始策略和游戏内适应规则，在对称和非对称成本设置下评估强化学习和LLM策略

Result: 通过采样元策略配置的正规形式经验博弈，计算相关博弈统计数据（如个体收益和对手均衡混合的遗憾），构建经验最佳响应图揭示策略关系

Conclusion: 在重复定价博弈中评估算法共谋的可行性，展示实际"测试时间"环境中定价策略的有效性，为算法共谋风险提供实证分析框架
...
### [56] [MMCAformer: Macro-Micro Cross-Attention Transformer for Traffic Speed Prediction with Microscopic Connected Vehicle Driving Behavior](https://arxiv.org/abs/2602.16730)
*Lei Han,Mohamed Abdel-Aty,Younggun Kim,Yang-Jun Joo,Zubayer Islam*

Main category: cs.LG

TL;DR: 本文提出MMCAformer模型，通过交叉注意力机制融合宏观交通流特征与微观驾驶行为特征，提升交通速度预测精度并降低不确定性。
...
Method: 提出Macro-Micro Cross-Attention Transformer (MMCAformer)模型，使用自注意力学习宏观交通流内在依赖，通过交叉注意力捕捉宏观交通状态与微观驾驶行为的时空交互，采用Student-t负对数似然损失进行优化。
...
Conclusion: MMCAformer通过融合宏观交通特征与微观驾驶行为特征，显著提升了速度预测精度并降低了不确定性，特别是在拥堵条件下效果更明显，为主动交通管理提供了更可靠的预测工具。
...
### [57] [A Few-Shot LLM Framework for Extreme Day Classification in Electricity Markets](https://arxiv.org/abs/2602.16735)
...
<details>
  <summary>Details</summary>
Motivation: 在电力市场中，准确预测电价尖峰对市场参与者至关重要。传统机器学习方法需要大量历史数据，但在数据稀缺或市场结构变化时表现受限。本研究旨在探索大语言模型在少样本场景下的电价尖峰分类能力。

Method: 提出基于大语言模型的少样本分类框架：1) 聚合系统状态信息（电力需求、可再生能源发电、天气预报、近期电价）为统计特征；2) 将特征格式化为自然语言提示；3) 结合通用指令输入大语言模型；4) 模型输出次日是否为尖峰日的概率及置信度评分。

Result: 使用德克萨斯电力市场历史数据进行验证：1) 少样本方法达到与支持向量机和XGBoost等监督机器学习模型相当的性能；2) 在历史数据有限时，该方法优于传统机器学习模型；3) 大语言模型在数据稀缺场景下展现出更好的数据效率。

Conclusion: 大语言模型可作为数据高效的工具，用于电力市场电价尖峰分类，特别是在数据稀缺的环境中。该方法为电力市场预测提供了新的技术路径，展示了大语言模型在专业领域应用的潜力。
...
### [58] [Real-time Secondary Crash Likelihood Prediction Excluding Post Primary Crash Features](https://arxiv.org/abs/2602.16739)
...
<details>
  <summary>Details</summary>
Motivation: 现有二次事故可能性预测方法主要依赖事故后特征（如事故类型和严重程度），这些特征在实时场景中难以获取，限制了实际应用。需要开发不依赖事故后特征的预测框架。
...
### [59] [Linear Convergence in Games with Delayed Feedback via Extra Prediction](https://arxiv.org/abs/2602.17486)
...
Method: 使用加权乐观梯度下降上升法(WOGDA)，将其解释为额外近端点法(EPP)的近似，分析标准乐观主义（预测下一步奖励）和额外乐观主义（预测更远未来奖励）

Result: 标准乐观主义收敛率为exp(-Θ(t/m^5))，额外乐观主义容忍更大步长，收敛率加速到exp(-Θ(t/(m^2 log m)))
...
### [60] [Quantifying LLM Attention-Head Stability: Implications for Circuit Universality](https://arxiv.org/abs/2602.16740)
*Karan Bali,Jack Stanley,Praneet Suresh,Danilo Bzdok*

Main category: cs.LG

TL;DR: 该研究系统分析了Transformer语言模型中注意力头在不同训练实例间的稳定性，发现中层注意力头最不稳定但表征最独特，深度模型在中间层表现出更强的发散性，不稳定的深层头功能更重要，权重衰减优化能显著提高注意力头稳定性。


<details>
  <summary>Details</summary>
Motivation: 当前机制可解释性研究中发现的Transformer"电路"（稀疏的子计算结构）很少在不同架构实例间进行稳定性测试。如果不验证这些电路是否在不同训练实例中普遍存在，就无法确定它们是普遍规律还是特定训练实例的偶然产物，这会限制在安全关键场景中对这些发现的可信度。

Method: 系统研究不同复杂度、不同规模的Transformer语言模型在不同独立初始化训练运行间的稳定性。逐层量化注意力头在不同训练实例中学习表征的相似性，进行严格的实验分析。

Result: 研究发现：(1) 中层注意力头最不稳定但表征最独特；(2) 更深层的模型在中间深度表现出更强的发散性；(3) 深层中不稳定的注意力头比同层的其他头功能更重要；(4) 应用权重衰减优化能显著提高注意力头在不同随机初始化模型间的稳定性；(5) 残差流相对稳定。

Conclusion: 跨实例电路鲁棒性是实现可扩展监督的重要但被低估的前提条件，为AI系统的白盒可监控性划定了可能边界。研究强调了验证机制可解释性发现在不同训练实例间稳定性的重要性。
...
### [61] [DeepVision-103K: A Visually Diverse, Broad-Coverage, and Verifiable Mathematical Dataset for Multimodal Reasoning](https://arxiv.org/abs/2602.16742)
*Haoxiang Sun,Lizhen Xu,Bing Zhao,Wotao Yin,Wei Wang,Boyu Yang,Rui Wang,Hu Wei*

Main category: cs.LG

TL;DR: DeepVision-103K是一个用于强化学习可验证奖励训练的大规模数据集，覆盖K12数学主题、广泛知识点和丰富视觉元素，旨在提升大型多模态模型的视觉反思和推理能力。
...
Method: 构建了DeepVision-103K数据集，包含103K个样本，涵盖多样化的K12数学主题、广泛的知识点和丰富的视觉元素，用于RLVR训练。
...
### [62] [PETS: A Principled Framework Towards Optimal Trajectory Allocation for Efficient Test-Time Self-Consistency](https://arxiv.org/abs/2602.16745)
*Zhangyi Liu,Huaizhi Qu,Xiaowei Yin,He Sun,Yanjun Han,Tianlong Chen,Zhun Deng*

Main category: cs.LG

TL;DR: PETS是一种通过优化框架进行轨迹分配的方法，旨在在有限预算下实现样本高效的测试时自一致性，相比均匀分配可减少75%（离线）和55%（在线）的采样预算。
...
Method: 提出了PETS方法，通过优化框架进行轨迹分配，引入自一致性率作为新度量标准。在离线设置中，将轨迹分配建模为众包问题；在在线流式设置中，提出基于离线框架的新方法，根据问题难度自适应分配预算。

Result: 在GPQA数据集上，PETS在两个设置中都实现了完美的自一致性，相比均匀分配减少了75%（离线）和55%（在线）的采样预算。

Conclusion: PETS为测试时自一致性提供了一个原则性且高效的框架，在有限预算下显著提升了样本效率，为测试时缩放的实际应用提供了理论基础和实用算法。
...
### [63] [Low-Dimensional and Transversely Curved Optimization Dynamics in Grokking](https://arxiv.org/abs/2602.16746)
*Yongzhong Xu*

Main category: cs.LG

TL;DR: 论文通过几何分析研究transformer在模运算任务中的grokking现象，发现训练轨迹主要局限在低维执行子空间，横向曲率增长先于泛化出现，且遵循幂律关系。


<details>
  <summary>Details</summary>
Motivation: grokking现象（从记忆到泛化的延迟转变）在小型算法任务中仍未被充分理解，作者希望通过几何分析揭示其背后的优化动力学机制。

Method: 1. 对transformer在模运算任务上的注意力权重轨迹进行PCA分析，识别低维执行子空间；2. 测量交换子缺陷（连续梯度步的非交换性）来探测损失景观几何；3. 将曲率投影到学习到的子空间；4. 进行因果干预实验验证几何解释。

Result: 1. 训练轨迹主要局限在低维执行子空间（单个主成分捕获68-83%的方差）；2. 横向（正交于执行子空间）曲率增长先于泛化出现，且领先时间遵循grokking时间尺度的幂律；3. 沿学习子空间的运动对grokking是必要的，而人为增加曲率不足以引发泛化；4. 结果在不同学习率、超参数和随机种子下具有可重复性。

Conclusion: grokking反映了从亚稳态机制的逃逸，该机制特征为低维局限和横向曲率积累。几何解释得到支持：横向梯度流对grokking是必要但不充分的，而沿学习子空间的运动是泛化的必要条件。
...
### [64] [LiveClin: A Live Clinical Benchmark without Leakage](https://arxiv.org/abs/2602.16747)
*Xidong Wang,Shuqi Guo,Yue Shen,Junying Chen,Jian Wang,Jinjie Gu,Ping Zhang,Lei Liu,Benyou Wang*

Main category: cs.LG

TL;DR: LiveClin是一个动态更新的医学LLM基准测试，从真实病例报告构建，每半年更新一次，包含1407个病例和6605个问题，用于评估模型在真实临床场景中的表现。


<details>
  <summary>Details</summary>
Motivation: 当前医学LLM评估面临数据污染和知识过时的问题，导致静态基准测试得分虚高，无法反映真实临床实践能力。

Method: 从当代同行评审的病例报告构建动态基准，采用239名医生参与的AI-人工验证工作流程，将真实患者病例转化为涵盖整个临床路径的复杂多模态评估场景。

Result: 评估26个模型发现，即使是表现最好的模型在病例准确率上也仅为35.7%。人类专家表现优于大多数模型，主任医师准确率最高，主治医师紧随其后。

Conclusion: LiveClin提供了一个持续演进的临床基础框架，能够指导医学LLM开发，缩小与人类专家的差距，提高可靠性和实际应用价值。
...
### [65] [Attending to Routers Aids Indoor Wireless Localization](https://arxiv.org/abs/2602.16762)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于机器学习的Wi-Fi定位方法在不同环境中难以取得突破性性能，主要原因是大多数算法在聚合不同路由器信息时未能适当加权，导致收敛不理想和精度降低。

Method: 受传统加权三角定位方法启发，引入了路由器注意力概念，在标准机器学习定位架构中加入注意力层，确保每个路由器在三角定位信息聚合中的贡献得到不同权重分配。

Result: 通过在开源数据集上的评估表明，路由器注意力方法相比基准架构在定位精度上提升了超过30%。

Conclusion: 通过强调每个路由器的相关性，采用注意力机制对路由器信息进行差异化加权，能够显著提升Wi-Fi定位系统的整体性能。
...
### [66] [ML-driven detection and reduction of ballast information in multi-modal datasets](https://arxiv.org/abs/2602.16876)
*Yaroslav Solovko*

Main category: cs.LG

TL;DR: 提出一个多模态框架用于检测和减少数据中的冗余信息（ballast），通过多种技术识别并消除低价值特征，显著降低维度、存储和计算成本。


<details>
  <summary>Details</summary>
Motivation: 现代数据集常包含大量冗余或低价值信息（ballast），这些信息增加维度、存储需求和计算成本，但不提供有意义的分析价值，需要系统化的方法来识别和减少这些冗余。
...
Conclusion: 该框架揭示了不同类型的ballast（统计、语义、基础设施等），为构建更精简、更高效的机器学习流程提供了实用指导。
...
TL;DR: 该论文提出一个框架，将语言模型中的分布偏移分为可观测和不可观测部分，通过建立不可观测变量强度与最坏情况泛化性能的边界，改进OOD评估和优化。
...
Method: 将分布偏移分解为可观测和不可观测部分，提出一个框架将不可观测变量的强度映射到分布偏移下语言模型最坏情况泛化性能的边界，并将这些边界直接用于语言模型评估和优化。

Result: 实验表明，使用这些边界进行语言模型评估和优化能提供更原则性的OOD性能度量，相比标准分布偏移调整方法改善了真实OOD性能，并在目标分布标签可用时能够推断不可观测变量的强度。

Conclusion: 该框架通过考虑不可观测分布偏移，为语言模型的分布偏移问题提供了更全面的解决方案，改进了OOD评估和优化，并能够量化不可观测变量的影响。
...
### [68] [Better Think Thrice: Learning to Reason Causally with Double Counterfactual Consistency](https://arxiv.org/abs/2602.16787)
*Victoria Lin,Xinnuo Xu,Rachel Lawrence,Risa Ueno,Amit Sharma,Javier Gonzalez,Niranjani Prasad*

Main category: cs.LG

TL;DR: 提出双反事实一致性(DCC)方法，无需标注数据即可评估和提升大语言模型的因果推理能力


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在推理基准测试中表现良好，但在面对反事实问题时表现出脆弱性，表明其因果推理能力存在弱点。现有方法需要大量标注的反事实数据，难以覆盖广阔的反事实空间。

Method: 引入双反事实一致性(DCC)，这是一种轻量级的推理时方法，通过验证模型执行因果干预和反事实预测的能力来评估因果推理能力，无需标注的反事实数据。

Result: 使用DCC评估了多种领先大语言模型在各种推理任务和干预中的因果推理能力，并证明DCC作为无需训练的训练时拒绝采样标准能有效提升多个模型家族在推理任务上的性能。

Conclusion: DCC提供了一种无需标注数据的有效方法来评估和提升大语言模型的因果推理能力，解决了反事实数据规模限制的问题。
...
TL;DR: radVI：一种基于径向轮廓优化的变分推断增强算法，可提升高斯分布对目标分布的径向拟合能力


<details>
  <summary>Details</summary>
Motivation: 传统变分推断中常用的高斯分布往往无法准确捕捉目标分布的径向轮廓，导致覆盖范围不足，需要改进径向拟合能力

Method: 提出radVI算法，专注于优化径向轮廓，可作为现有VI方案（如高斯VI、拉普拉斯近似）的廉价有效附加组件，利用Wasserstein空间优化理论和径向传输映射的规律性

Result: 提供了算法的理论收敛保证，基于Wasserstein空间优化理论的最新发展和Caffarelli风格的径向传输映射规律性

Conclusion: radVI是一种简单有效的变分推断增强方法，能够显著改善高斯分布对目标分布径向轮廓的拟合能力
...
TL;DR: 提出了一种低成本、高性能的IMO数学问题推理管道，通过认知陷阱识别和猜想提取技术，在IMO-ProofBench Advanced上达到67.1%的准确率，成本仅为31美元/题，远低于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有公开模型在IMO数学问题上达到金牌性能需要极高推理成本（约3000美元/题），而定制和未发布模型虽然性能好但不可及。需要开发一种既高性能又低成本的解决方案。

Method: 提出推理管道方法，识别并解决"认知陷阱"问题（迭代优化收敛到错误解但内部评分器认为正确）。通过猜想提取技术，从生成解中分离候选引理，在新鲜环境中独立验证引理及其否定（上下文分离）。

Result: 在IMO-ProofBench Advanced上使用Gemini 3.0 Pro达到67.1%的准确率，平均每题成本约31美元。这代表了当时PB-Adv上的最先进水平，是次优公开管道成功率的两倍以上，且成本仅为几分之一。

Conclusion: 通过解决认知陷阱问题并采用猜想提取技术，实现了在IMO数学问题上高性能、低成本的推理管道，为数学推理研究提供了实用且经济高效的解决方案。
...
### [71] [A Theoretical Framework for Modular Learning of Robust Generative Models](https://arxiv.org/abs/2602.17554)
*Corinna Cortes,Mehryar Mohri,Yutao Zhong*

Main category: cs.LG

TL;DR: 提出模块化LLM训练框架，通过门控机制组合领域专家模型，无需启发式数据混合调优，理论证明优于传统单模型训练


<details>
  <summary>Details</summary>
Motivation: 大规模生成模型训练资源密集且依赖启发式数据加权，需要解决两个问题：能否模块化训练LLM（组合小领域专家）达到单体性能？能否对任意数据混合鲁棒，消除启发式调优？

Method: 提出模块化生成建模理论框架：预训练专家通过门控机制组合；定义归一化门控函数空间G1；构建极小极大博弈寻找对最坏情况数据混合鲁棒的单门控；提出可扩展的随机原始对偶算法和结构蒸馏方法

Result: 理论证明：使用角谷不动点定理证明鲁棒门控存在；模块化作为强正则化器，泛化界与轻量级门控复杂度相关；模块化方法理论上优于聚合数据重训练模型，差距由Jensen-Shannon散度表征；实证验证在合成和真实数据集上有效缓解梯度冲突并超越单体基线

Conclusion: 模块化架构通过理论框架和算法实现，能够鲁棒地组合领域专家，无需启发式数据混合调优，在理论和实证上均优于传统单体训练方法
...
TL;DR: TFFT提出了一种基于CVaR的分布微调算法，能够分别控制高奖励尾部和低奖励尾部行为，计算成本与标准期望微调方法相当。


<details>
  <summary>Details</summary>
Motivation: 现有基于熵正则化的微调方法主要最大化期望奖励，但无法控制尾部行为。然而尾部控制在实际应用中至关重要：低奖励尾部决定了系统的可靠性（限制低奖励失败），高奖励尾部则有助于发现罕见的高奖励结果。

Method: 提出Tail-aware Flow Fine-Tuning (TFFT)算法，基于条件风险价值(CVaR)进行分布微调。利用CVaR的变分对偶公式，将其分解为解耦的两阶段过程：轻量级的一维阈值优化步骤，以及通过特定伪奖励的单一熵正则化微调过程。

Result: TFFT在多个实验中表现出有效性，包括说明性实验、高维文本到图像生成和分子设计。算法能够高效实现CVaR微调，计算成本与标准期望微调方法相当。

Conclusion: TFFT提供了一种原则性且高效的分布微调算法，能够分别控制高奖励和低奖励尾部行为，解决了现有方法在尾部控制方面的局限性，为实际应用提供了更可靠的微调方案。
...
TL;DR: 本文提出了一种基于e值的锚定水印框架，解决了现有LLM水印方法在采样分布选择和固定假设检验方面的局限性，实现了任意时间有效的推理。
...
Method: 开发了首个基于e值的水印框架Anchored E-Watermarking，通过锚定分布近似目标模型，构建检测过程的测试超鞅，统一了最优采样与任意时间有效推理。

Result: 理论分析表明该框架能显著提高样本效率，在标准基准测试中，检测所需的平均token预算比最先进基线减少13-15%。

Conclusion: 该框架解决了传统水印方法的局限性，实现了任意时间有效的推理，为机器生成内容检测提供了更高效可靠的方法。
...
### [74] [When to Trust the Cheap Check: Weak and Strong Verification for Reasoning](https://arxiv.org/abs/2602.17633)
*Shayan Kiyani,Sima Noorani,George Pappas,Hamed Hassani*

Main category: cs.LG

TL;DR: 论文提出弱-强验证策略框架，用于在LLM推理中平衡低成本弱验证（如自一致性检查）与高成本强验证（如人工检查），通过双阈值策略优化接受/拒绝决策，并开发了在线算法控制错误率。


<details>
  <summary>Details</summary>
Motivation: LLM推理过程通常包含验证环节：内部使用低成本但不可靠的弱验证（如自一致性检查），外部使用高成本但可靠的强验证（如人工检查）。这两种验证在成本和可靠性上存在显著差异，需要设计策略来平衡这种权衡。

Method: 提出弱-强验证策略框架，将验证决策形式化为基于弱验证信号的接受/拒绝/延迟决策问题。证明最优策略具有双阈值结构，并开发在线算法控制接受错误和拒绝错误，无需对查询流、语言模型或弱验证器做假设。

Result: 理论分析表明校准度和锐度决定弱验证器的价值，最优策略具有双阈值结构。开发的在线算法能够在不依赖任何假设的情况下，对接受错误和拒绝错误进行统计控制。

Conclusion: 弱-强验证策略框架为LLM推理中的验证权衡提供了系统化解决方案，通过理论分析和算法设计实现了在资源约束下的可靠验证，为实际应用中的验证决策提供了理论基础和实用工具。
...
TL;DR: HiVAE是一个分层变分架构，将心理理论推理扩展到现实时空领域，在校园导航任务中表现优异，但学习的潜在表征缺乏对实际心理状态的明确基础。


<details>
  <summary>Details</summary>
Motivation: 现有心理理论方法主要关注小型人类可理解的网格世界空间，需要将其扩展到现实时空领域。

Method: 提出HiVAE分层变分架构，受人类认知的信念-愿望-意图结构启发，采用三层VAE层次结构。

Result: 在3,185个节点的校园导航任务中实现了显著的性能提升，但发现学习的潜在表征缺乏对实际心理状态的明确基础。
...
### [76] [VAM: Verbalized Action Masking for Controllable Exploration in RL Post-Training -- A Chess Case Study](https://arxiv.org/abs/2602.16833)
*Zhicheng Zhang,Ziyan Wang,Yali Du,Fei Fang*

Main category: cs.LG

TL;DR: 论文提出Verbalized Action Masking (VAM)方法，通过在提示中语言化动作掩码，强制LLM从掩码集合中输出动作，结合迭代动作空间剪枝，改善大语言模型强化学习后训练中的探索问题。
...
Method: 提出Verbalized Action Masking (VAM)：1）在提示中语言化动作掩码；2）强制模型从掩码集合输出动作；3）引入迭代动作空间剪枝：如果未采样到目标动作，从掩码中移除已采样的有效动作，在减少的候选集上重新采样，直到采样到目标或达到预算上限。

Result: 在象棋任务中，VAM在两种训练机制下均表现优异：1）引擎对战机制；2）固定数据集机制。在保留的象棋谜题和完整对局中（通过平均百分兵损失ACPL衡量），VAM相比强基线提高了学习效率和最终性能。

Conclusion: 语言化掩码是LLM强化学习后训练中可控探索的实用机制，能有效改善探索效率和学习性能。
...
### [77] [A Residual-Aware Theory of Position Bias in Transformers](https://arxiv.org/abs/2602.16837)
*Hanna Herasimchyk,Robin Labryga,Tomislav Prusina,Sören Laue*

Main category: cs.LG

TL;DR: 该论文通过引入残差连接的理论分析，解决了因果Transformer中注意力机制会塌缩到第一个token的理论预测与实际观察不符的问题，解释了"迷失在中间"现象的结构性原因。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在特定token位置上存在系统性偏好，但造成这种位置偏见的架构根源尚未得到充分理解。先前关于注意力展开的理论分析预测，在无限深度和因果掩码条件下，注意力会不可避免地塌缩到第一个token，但这与实际情况不符。

Method: 提出了一个残差感知的累积注意力展开理论，通过纳入残差连接这一架构组件，证明了在现实条件下可以防止注意力塌缩。在有限深度条件下，证明了因果Transformer会诱导U形位置偏见，即注意力集中在早期和晚期token上。

Result: 研究结果表明，残差连接是防止注意力塌缩的关键架构组件，并解释了为什么实际Transformer模型不会出现理论预测的注意力完全塌缩到第一个token的现象。同时，有限深度下的U形位置偏见为"迷失在中间"现象提供了原则性的架构解释。

Conclusion: 该研究通过理论分析阐明了Transformer架构中残差连接的重要性，解决了理论预测与实际观察之间的差异，并为Transformer模型中的位置偏见现象提供了基于架构的合理解释，特别是解释了为什么模型在处理长序列时容易忽略中间部分信息。
...
### [78] [Training Large Reasoning Models Efficiently via Progressive Thought Encoding](https://arxiv.org/abs/2602.16839)
*Zeliang Zhang,Xiaodong Liu,Hao Cheng,Hao Sun,Chenliang Xu,Jianfeng Gao*

Main category: cs.LG

TL;DR: 提出渐进思维编码方法，通过将中间推理编码为固定大小向量表示，在有限缓存下提升大型推理模型的效率和性能


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂问题上表现优异，但强化学习训练需要长序列展开，自回归解码占用大量时间和内存。滑动窗口缓存策略虽然能限制内存，但会破坏长上下文推理并降低性能。

Method: 渐进思维编码：一种参数高效的微调方法，通过逐步将中间推理编码为固定大小的向量表示，消除对完整缓存展开的反向传播需求，减少内存使用，并在推理时保持恒定内存。

Result: 在三个模型（Qwen2.5-3B-Instruct、Qwen2.5-7B-Instruct、DeepSeek-R1-Distill-Llama-8B）和六个数学基准测试中，该方法比基于LoRA的微调平均提升19.3%，比未微调的LRMs提升29.9%，在相同严格缓存预算下AIME2024/2025准确率提升高达23.4%。

Conclusion: 渐进思维编码不仅提高了推理准确性，而且在真实世界内存约束下使LRMs的RL训练更加高效和可扩展。
...
### [79] [What is the Value of Censored Data? An Exact Analysis for the Data-driven Newsvendor](https://arxiv.org/abs/2602.16842)
*Rachitesh Kumar,Omar Mouchtaki*

Main category: cs.LG

TL;DR: 研究离线数据驱动报童问题中的需求截断问题，提出计算数据驱动库存策略最坏情况后悔值的通用方法，分析需求截断对学习能力的影响


<details>
  <summary>Details</summary>
Motivation: 传统报童问题假设需求完全可观测，但实际库存管理中需求常被库存水平截断，只能观察到销售数据而非真实需求，这限制了从被动销售数据中学习的能力

Method: 提出通用程序计算经典数据驱动库存策略在所有需求分布上的精确最坏情况后悔值，将无限维非凸优化问题简化为有限维问题，分析Kaplan-Meier策略和销售即需求启发式方法

Result: 需求截断从根本上限制了从被动销售数据中学习的能力，但在高库存水平进行少量针对性探索可显著改善最坏情况保证；销售即需求启发式方法在截断数据累积时性能严重下降

Conclusion: 销售点信息的质量对离线学习能力至关重要，需求截断限制了被动学习，但通过针对性探索可实现接近最优性能，而忽略截断的简单方法会导致严重性能退化
...
### [80] [Position: Why a Dynamical Systems Perspective is Needed to Advance Time Series Modeling](https://arxiv.org/abs/2602.16864)
*Daniel Durstewitz,Christoph Jürgen Hemmer,Florian Hess,Charlotte Ricarda Doll,Lukas Eisenmann*

Main category: cs.LG

TL;DR: 该论文主张将时间序列建模从传统统计方法转向动力系统视角，认为通过动力系统重构可以从数据中推断底层系统，实现更优的预测和更深刻的理论洞察。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列建模领域存在大量炒作和工业需求，但实际进展不明确。作者认为需要动力系统视角来推动时间序列预测和分析达到新水平，因为观测时间序列几乎都源自底层动力系统，理解其控制方程可实现理论最优预测。

Method: 论文提出采用动力系统重构方法，这是一类机器学习/人工智能方法，旨在从数据中推断底层动力系统的替代模型。该方法基于动力系统理论的核心概念、方法、度量和模型。

Result: 动力系统模型不仅提供短期预测，还能预测观测系统的长期统计特性，这在许多实际场景中更为重要。动力系统理论提供领域无关的理论洞察，揭示时间序列生成机制，为任何时间序列模型的性能上限、向未见状态（如临界点）的泛化以及潜在控制策略提供信息。

Conclusion: 动力系统重构的见解可以关键性地推进时间序列建模，实现更好的预测性能，同时大幅降低计算和内存开销。论文最后提出了将动力系统重构见解转化为时间序列建模的具体建议。
...
### [81] [Construction of a classification model for dementia among Brazilian adults aged 50 and over](https://arxiv.org/abs/2602.16887)
*F. S. Menezes,M. C. F. G. Barretto,E. Q. C. Garcia,T. A. E. Ferreira,J. G. Alvez*

Main category: cs.LG

TL;DR: 该研究为巴西中老年人开发了一个痴呆症分类模型，使用低成本可干预变量，通过随机森林和逻辑回归分析，发现文盲、高龄、低体重、握力弱、黑人肤色、缺乏运动、听力损失和抑郁症状是痴呆风险因素，而高等教育、生活满意度高和就业是保护因素。


<details>
  <summary>Details</summary>
Motivation: 建立针对巴西中老年人的痴呆症分类模型，使用低成本且具有干预潜力的变量，帮助识别高危人群，为公共卫生政策和资源分配提供依据。

Method: 采用观察性研究和预测建模方法，使用巴西老龄化纵向研究（ELSI-Brazil）的横断面数据，涉及9,412名参与者。痴呆症通过神经心理学评估和知情者认知功能评估确定。使用随机森林和多变量逻辑回归分析痴呆风险因素。
...
Conclusion: 研究结果强调了痴呆症的多维性质以及可及性因素在识别高危人群中的重要性。加强以促进脑健康为重点的公共卫生政策，可以显著促进巴西初级保健资源的有效分配和痴呆症预防。
...
TL;DR: 提出了一种去中心化多玩家随机老虎机算法，在连续Lipschitz动作空间中处理碰撞问题，实现与时间无关的协调成本，达到接近最优的遗憾界。


<details>
  <summary>Details</summary>
Motivation: 研究去中心化多玩家随机老虎机问题，在连续Lipschitz结构的动作空间中，硬碰撞产生零奖励。目标是设计无需通信的策略，最大化集体奖励，同时协调成本与时间范围T无关。

Method: 提出模块化协议：首先通过新颖的最大值导向搜索解决多智能体协调问题，识别并将玩家分配到不同的高价值区域；然后将问题解耦为N个独立的单玩家Lipschitz老虎机问题。

Result: 建立了接近最优的遗憾界 $\tilde{O}(T^{(d+1)/(d+2)})$ 加上与T无关的协调成本，匹配单玩家速率。这是第一个提供此类保证的框架，并可扩展到一般距离阈值碰撞模型。

Conclusion: 该框架成功解决了去中心化多玩家连续动作空间老虎机问题，实现了与单玩家相当的遗憾性能，同时协调成本与时间无关，具有理论保证和实际应用价值。
...
TL;DR: 该论文提出了一种新的多智能体强化学习方法，通过策略依赖的指数衰减特性分解，揭示了策略平滑性可以诱导局部性，即使环境存在强动作耦合，并建立了比现有方法更紧的光谱条件。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习的可扩展性受到维度诅咒的根本挑战。现有方法通常基于环境的最坏情况边界来保证指数衰减特性，这些条件过于保守，未能捕捉策略本身的调节效应。

Method: 提出了一种新颖的策略诱导相互依赖矩阵分解方法，将环境对状态和动作的敏感性与策略对状态的敏感性解耦。建立了光谱条件ρ(E^s+E^aΠ(π))<1来保证指数衰减，该条件比先前的基于范数的条件更严格。

Result: 证明了局部性可以是策略依赖的现象，平滑策略可以在环境存在强动作耦合时诱导局部性，揭示了局部性与最优性之间的基本权衡。基于该理论分析了一个可证明正确的局部化块坐标策略改进框架。

Conclusion: 通过策略依赖的指数衰减特性分解，为多智能体强化学习提供了更精确的局部性分析框架，揭示了策略平滑性在诱导局部性中的关键作用，并为设计可扩展的多智能体算法提供了理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索"顿悟"现象（从记忆到泛化的突然转变）是否仅限于模运算任务，以及是否存在通用的早期预警信号。

Method: 方法包括：1）在两个序列学习基准（SCAN组合泛化和Dyck-1深度预测）上训练Transformer模型；2）测量交换子缺陷（梯度更新的非交换性度量）；3）使用权重空间PCA分析谱集中；4）进行因果干预实验（放大非交换性和抑制正交梯度流）。

Result: 结果显示：1）交换子缺陷在泛化前显著上升，领先时间遵循超线性幂律（SCAN约1.18，Dyck约1.13）；2）谱集中不是通用前兆，交换子缺陷才是；3）放大非交换性加速顿悟（SCAN约32%，Dyck约50%），抑制正交梯度流延迟或阻止顿悟；4）三个任务家族形成因果敏感性谱系。
...
TL;DR: 该论文发现当前大语言模型对齐存在结构弱点：现代拒绝机制是"故障开放"的，通过提示注入攻击抑制单个主导特征会导致对齐崩溃。作者提出"故障封闭"对齐作为设计原则，并实现了一个渐进式对齐框架来创建冗余、独立的因果路径。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型对齐存在结构弱点：现代拒绝机制是"故障开放"的。虽然现有方法在多个潜在特征中编码拒绝行为，但通过提示注入攻击抑制单个主导特征会导致对齐崩溃，从而产生不安全生成。这促使研究者寻求更稳健的LLM安全设计原则。

Method: 提出"故障封闭"对齐作为设计原则：拒绝机制应在部分故障时仍保持有效，通过冗余、独立的因果路径实现。具体实现为渐进式对齐框架，迭代地识别和消融先前学习的拒绝方向，迫使模型在新的独立子空间中重建安全性。

Result: 在四种越狱攻击测试中，该方法实现了最强的整体鲁棒性，同时减轻了过度拒绝问题并保持了生成质量，计算开销较小。机制分析证实，使用该方法训练的模型编码了多个因果独立的拒绝方向，提示注入攻击无法同时抑制这些方向。

Conclusion: "故障封闭"对齐为稳健的LLM安全提供了原则性基础。通过创建冗余、独立的拒绝路径，可以防止单个特征被抑制导致的对齐崩溃，从而显著提升大语言模型对抗越狱攻击的鲁棒性。
...
TL;DR: UniLeak框架通过识别语言模型残差流中的通用激活方向，能够在不访问训练数据或真实PII的情况下，通过线性添加这些方向来一致地增加PII泄露概率。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型具有丰富的内部结构，但隐私敏感行为（如个人身份信息泄露）如何在其隐藏状态中表示和调节尚不清楚。研究者希望理解PII泄露在模型内部的具体表示机制。

Method: 提出UniLeak机制解释框架，识别模型残差流中的通用激活方向。这些方向在不同上下文中具有一致性，通过在线性推理时添加这些方向来增加PII生成概率。该方法仅依赖模型自生成文本，无需训练数据或真实PII。

Result: 在多个模型和数据集上，沿着这些通用方向进行"引导"显著增加了PII泄露，相比现有的基于提示的提取方法效果更好。这些方向对生成质量影响最小，但能有效放大PII生成概率。

Conclusion: PII泄露可以视为模型表示中潜在信号的叠加，这一发现为PII泄露提供了新视角，既支持风险放大也支持风险缓解。UniLeak框架为理解语言模型中的隐私泄露机制提供了新的工具。
...
<details>
  <summary>Details</summary>
Motivation: 多路径推测解码使用廉价草稿模型生成令牌树，然后通过验证算法接受子集。虽然已有多种针对独立同分布展开的验证算法，但在匹配设置下的相对性能尚不明确。

Method: 1) 系统评估不同验证策略；2) 提出延迟树扩展方法，推迟独立同分布分支点；3) 开发动态神经选择器，根据草稿和目标特征估计最优传输验证方法的预期块效率。

Result: 遍历验证始终优于最优传输方法；延迟树扩展保持目标分布并改进根节点独立同分布展开；神经选择器使最优传输方法首次超越遍历验证，在广泛模型、数据集和采样设置中实现平均5%的吞吐量提升。

Conclusion: 通过系统评估揭示了验证策略性能差异的原因，提出的延迟树扩展和神经选择器有效解决了最优传输方法在深层树节点性能不佳的问题，首次使OT方法超越遍历验证，显著提升了多路径推测解码的效率。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中，协调行动是最基本的合作形式。成功的分散决策不仅需要良好的个体行动，更需要智能体间选择兼容的行动以实现行为同步、避免冲突并满足全局约束。现有方法在处理协调任务时表现不佳。

Method: 提出动作图策略（AGP），建模智能体可用动作选择之间的依赖关系。该方法构建"协调上下文"，使智能体能够基于全局动作依赖关系来条件化其决策。

Result: 理论上证明AGP比完全独立策略具有更严格的表达能力，能够实现比贪婪执行更优的协调联合动作。实证结果显示，在部分可观测性和反协调惩罚的典型协调任务中，AGP达到80-95%成功率，而其他MARL方法仅10-25%。在多样化多智能体环境中，AGP始终优于基线方法。
...
TL;DR: 该论文建立了路径重参数化和分数函数梯度估计器之间的严格数学联系，基于Malliavin积分恒等式，提出了一个方差感知的混合估计器，在VAE和合成问题上实现了显著的方差降低。


<details>
  <summary>Details</summary>
Motivation: 现有随机梯度估计方法（如路径重参数化和分数函数估计器）在理论和实践上缺乏统一框架，需要一种能够自适应结合两者优势、降低方差的理论基础和方法。

Method: 基于Malliavin积分恒等式建立两种梯度估计器的数学等价性，提出方差感知的混合估计器，利用经验协方差结构自适应组合路径重参数化和Malliavin梯度，实现最小方差的无偏线性组合。

Result: 在VAE（CIFAR-10）上实现了9%的方差降低，在强耦合合成问题上达到35%的方差降低；获得了封闭形式的有限样本收敛界；在非平稳优化场景中混合方法面临挑战。

Conclusion: Malliavin微积分为随机梯度估计提供了概念统一和实践可解释的框架，明确了混合方法的适用场景和固有局限性，为未来研究指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: GRPO在复杂推理训练中有效，但基于相对轨迹的目标会导致过度思考和推理效率低下，而长度惩罚难以校准，且缺乏何时继续/停止的直接监督信号

Method: 提出弱监督GRPO，训练偏好模型从仅结果正确性产生前缀级信号，指示额外继续是否有益，提供结果衍生的继续/停止指导

Result: 理论上证明有效性，实验显示在推理基准上显著减少推理长度，同时保持与GRPO基线相当的准确性

Conclusion: WS-GRPO通过弱监督方式改进GRPO的推理效率，解决了过度思考和长度惩罚校准困难的问题，在保持准确性的同时减少冗余推理
...
TL;DR: 本文提出了一种AI增强的科学发现流水线，利用上下文学习范式帮助行为神经科学家自动化数据处理和模式识别，应用于小鼠恐惧泛化研究，相比传统方法显著提升了效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 科学发现流水线通常复杂、僵化且耗时，从数据准备到分析解释都需要大量人工干预。AI技术有望改变这一现状，让领域专家能专注于理解发现而非调试流水线或手动标注数据。特别是在行为神经科学领域，研究小鼠恐惧泛化对理解PTSD等临床疾病有重要意义。

Method: 采用"上下文学习"范式作为领域专家自动化流水线的接口，无需AI模型训练和微调的专业知识。同时引入了对张量分解模型的AI增强，以便从异构数据中更无缝地发现模式。该方法在行为神经科学的小鼠恐惧泛化研究中进行了应用。

Result: 实验评估表明，提出的流水线在性能上优于该领域的标准实践方法，也优于不使用上下文学习范式的合理机器学习基线。领域专家验证了发现结果的有效性，证明了该方法在保持性能的同时提供了无缝易用的接口。

Conclusion: 上下文学习范式为领域专家提供了无需AI专业知识的自动化流水线接口，结合增强的张量分解模型，能够有效加速科学发现过程，让专家专注于理解发现而非技术细节，在行为神经科学领域展示了显著的应用价值。
...
### [92] [Multi-Probe Zero Collision Hash (MPZCH): Mitigating Embedding Collisions and Enhancing Model Freshness in Large-Scale Recommenders](https://arxiv.org/abs/2602.17050)
...
TL;DR: MPZCH是一种基于线性探测的新型索引机制，通过辅助张量和高性能CUDA内核实现可配置探测和主动驱逐策略，有效缓解嵌入碰撞问题，在推荐系统中实现零碰撞嵌入。


<details>
  <summary>Details</summary>
Motivation: 随着推荐系统中唯一ID数量的增长，传统的基于哈希的索引方法面临碰撞问题，这会降低模型性能和个性化质量。哈希碰撞导致陈旧嵌入继承，影响新特征的学习效果。

Method: 提出Multi-Probe Zero Collision Hash (MPZCH)，一种基于线性探测的索引机制。使用辅助张量和高性能CUDA内核实现可配置探测和主动驱逐策略，通过淘汰过时ID和重置重新分配的槽位来防止陈旧嵌入继承。

Result: MPZCH实现了用户嵌入的零碰撞，显著提高了项目嵌入的新鲜度和质量。尽管有碰撞缓解开销，但系统保持了与现有方法相当的训练QPS和推理延迟。已在TorchRec开源库中发布。

Conclusion: MPZCH是一种有效缓解嵌入碰撞的索引机制，在保持生产规模效率的同时，通过消除碰撞提高了推荐系统的嵌入质量和个性化效果。
...
### [93] [Adam Improves Muon: Adaptive Moment Estimation with Orthogonalized Momentum](https://arxiv.org/abs/2602.17080)
...
Method: 提出了NAMO和NAMO-D两种算法：NAMO使用单一自适应步长缩放正交化动量，保持正交性；NAMO-D将正交化动量右乘一个带截断的对角矩阵，实现神经元级噪声适应，并与常见的近块对角Hessian结构对齐。

Result: 在标准假设下建立了两种算法在确定性设置中的最优收敛率，并证明在随机设置中其收敛保证能适应随机梯度的噪声水平。在GPT-2模型预训练实验中，NAMO和NAMO-D均优于AdamW和Muon基线，NAMO-D通过额外的截断超参数进一步超越NAMO。

Conclusion: NAMO和NAMO-D成功将正交化动量与基于范数的Adam型噪声适应原则性结合，提供了既能保持良好条件更新方向又能利用细粒度噪声适应的优化器，在大语言模型训练中表现出优越性能。
...
### [94] [MeGU: Machine-Guided Unlearning with Target Feature Disentanglement](https://arxiv.org/abs/2602.17088)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器学习遗忘方法存在基本权衡：激进擦除目标数据影响会降低模型在保留数据上的效用，而保守策略则残留目标信息。研究发现预训练模型中的语义类概念在特征模式层面存在纠缠，这从根本上限制了现有遗忘范式的有效性。

Method: 提出MeGU框架，利用多模态大语言模型为目标准样本分配有语义意义的扰动标签，确定重对齐方向；将MLLM估计的类间概念相似度编码为轻量级转移矩阵；引入正负特征噪声对，负噪声抑制目标特定特征模式，正噪声增强剩余关联特征并与扰动概念对齐。

Result: MeGU实现了可控和选择性遗忘，有效缓解了欠遗忘和过遗忘问题，能够在擦除目标数据影响的同时保持模型在保留数据上的效用。

Conclusion: 通过概念感知重对齐和特征噪声对解耦，MeGU框架克服了现有机器学习遗忘方法的基本权衡，实现了更有效的选择性遗忘，为训练数据隐私保护提供了新思路。
...
### [95] [Synergizing Transport-Based Generative Models and Latent Geometry for Stochastic Closure Modeling](https://arxiv.org/abs/2602.17089)
*Xinghao Dong,Huchen Yang,Jin-long Wu*

Main category: cs.LG

TL;DR: 该论文提出使用潜在空间中的流匹配方法替代扩散模型来学习随机闭合模型，实现了更快的采样速度（快两个数量级），同时通过显式和隐式正则化控制潜在空间失真，保持物理保真度。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能产生高质量且多样化的样本，但采样速度慢是其关键缺点。本文旨在寻找更适合学习随机闭合模型的生成AI方法，特别是需要快速采样的应用场景。

Method: 通过系统比较基于传输的生成模型，在2D Kolmogorov流的数值示例上，采用潜在空间中的流匹配方法实现单步采样。为控制潜在空间失真，比较了联合训练方案的隐式正则化与两种显式正则化：度量保持（MP）和几何感知（GA）约束。

Result: 潜在空间流匹配方法比迭代扩散方法快两个数量级。显式和隐式正则化的潜在空间都继承了原始复杂动力系统低维流形的关键拓扑信息，使得无需大量训练数据即可学习随机闭合模型。

Conclusion: 潜在空间中的流匹配方法特别适合随机闭合模型的快速采样，通过适当的正则化控制失真，在保持物理保真度的同时显著提高采样效率，为复杂动力系统的建模提供了有效工具。
...
### [96] [FLoRG: Federated Fine-tuning with Low-rank Gram Matrices and Procrustes Alignment](https://arxiv.org/abs/2602.17095)
*Chuiyang Meng,Ming Tang,Vincent W. S. Wong*

Main category: cs.LG

TL;DR: FLoRG：一种用于大语言模型联邦微调的高效框架，通过使用单低秩矩阵和Gram矩阵聚合，解决了传统LoRA在联邦学习中的聚合误差和分解漂移问题。


<details>
  <summary>Details</summary>
Motivation: 传统LoRA在联邦微调中使用两个独立的低秩矩阵会带来两个挑战：1) 分别聚合这两个矩阵会产生误差；2) 即使聚合矩阵乘积，服务器需要通过矩阵分解恢复因子，但分解不唯一且可能引入分解漂移。

Method: 提出FLoRG框架：1) 使用单个低秩矩阵进行微调；2) 聚合其Gram矩阵（列向量内积矩阵），消除聚合误差并减少通信开销；3) 引入Procrustes对齐方法，在连续微调轮次间对齐分解矩阵以确保更新一致性。

Result: 理论分析表明Procrustes对齐能获得更紧的收敛界。实验结果显示FLoRG在多个LLM微调基准测试中优于五种最先进的基线方案，在下游任务准确率上表现更好，通信开销最多可减少2041倍。

Conclusion: FLoRG通过单低秩矩阵和Gram矩阵聚合有效解决了联邦微调中的聚合误差和分解漂移问题，在保持准确率的同时显著降低了通信开销。
...
### [97] [Operationalization of Machine Learning with Serverless Architecture: An Industrial Operationalization of Machine Learning with Serverless Architecture: An Industrial Implementation for Harmonized System Code Prediction](https://arxiv.org/abs/2602.17102)
...
TL;DR: 论文提出了一个基于无服务器架构的MLOps框架，通过事件驱动管道管理完整的机器学习生命周期，并在海关HS编码预测的工业应用中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 解决传统ML部署中基础设施管理复杂、成本高昂的问题，同时应对HS编码预测这一合规关键任务中的挑战——产品描述简短、非结构化、频繁更新且分类模糊，错误会导致货运延误和财务损失。

Method: 采用无服务器架构构建事件驱动的MLOps管道，支持数据摄取、训练、部署、监控和再训练全流程。使用自定义文本嵌入编码器和多种深度学习架构（特别是Text-CNN），通过标准化接口支持多种推理模式，并实现自动A/B测试和动态模型选择。

Result: Text-CNN在真实数据上达到98%的准确率，管道确保了可重复性、可审计性和SLA遵守，通过自动扩展应对可变负载。相比Transformer模型，在保持相似准确率的同时显著降低了长期运营成本。

Conclusion: 该无服务器MLOps框架为机器学习工业化提供了可复制的蓝图，使企业能够在优化性能和经济性的同时实现规模化，优先考虑确定性分类、可预测延迟和可解释性，同时保持对Transformer变体和LLM推理的可扩展性。
...
### [98] [i-PhysGaussian: Implicit Physical Simulation for 3D Gaussian Splatting](https://arxiv.org/abs/2602.17117)
*Yicheng Cao,Zhuo Huang,Yu Yao,Yiming Ying,Daoyi Dong,Tongliang Liu*

Main category: cs.LG

TL;DR: i-PhysGaussian：将3D高斯泼溅与隐式物质点法结合，通过隐式牛顿优化解决显式方法的时间步长敏感性和精度退化问题


<details>
  <summary>Details</summary>
Motivation: 当前基于3D重建的物理模拟器通常依赖显式逐步更新方法，这些方法对时间步长敏感，在复杂场景（如高刚度材料或准静态运动）下精度会快速退化
...
Conclusion: 该隐式方法显著减少了时间步长敏感性，确保了物理一致性，为复杂物理场景提供了更稳定、更精确的模拟解决方案
...
### [99] [TIFO: Time-Invariant Frequency Operator for Stationarity-Aware Representation Learning in Time Series](https://arxiv.org/abs/2602.17122)
...
<details>
  <summary>Details</summary>
Motivation: 非平稳时间序列预测面临分布偏移问题，现有方法通常通过去除单个样本的低阶矩来缓解依赖，但未能捕捉样本间的时间演化结构，也无法建模复杂的时间结构。

Method: 提出时不变频率算子（TIFO），在频域空间学习整个数据集的平稳感知权重。该权重表示强调平稳频率分量，抑制非平稳分量，从而缓解时间序列的分布偏移。方法基于傅里叶变换在频域空间隐式诱导特征分解的理论基础。

Result: 在28个预测设置中取得18个top-1和6个top-2结果；在ETTm2数据集上平均MSE分别提升33.3%和55.3%；相比基线方法降低60%-70%计算成本，展示出强大的可扩展性。

Conclusion: TIFO是一种有效的即插即用方法，通过在频域学习时不变权重来缓解分布偏移问题，显著提升各种预测模型的性能并大幅降低计算复杂度。
...
### [100] [VP-VAE: Rethinking Vector Quantization via Adaptive Vector Perturbation](https://arxiv.org/abs/2602.17133)
*Linwei Zhai,Han Ding,Mingzhi Lin,Cui Zhao,Fei Wang,Ge Wang,Wang Zhi,Wei Xi*

Main category: cs.LG

TL;DR: VP-VAE通过解耦表示学习和离散化，用潜在扰动代替显式码本，解决了VQ-VAE训练不稳定和码本坍缩问题，并提出了轻量级变体FSP。


<details>
  <summary>Details</summary>
Motivation: VQ-VAE存在训练不稳定和"码本坍缩"问题，主要原因是表示学习和离散码本优化的内在耦合。需要一种能够解耦这两个过程的新方法。

Method: 提出VP-VAE，用分布一致且尺度自适应的潜在扰动替代不可微分的量化器，通过Metropolis-Hastings采样生成扰动。在近似均匀潜在变量假设下，推导出轻量级变体FSP，为FSQ式固定量化器提供理论解释和实践改进。

Result: 在图像和音频基准测试中，VP-VAE和FSP提高了重建保真度，实现了更平衡的token使用，同时避免了耦合码本训练固有的不稳定性。

Conclusion: VP-VAE通过解耦表示学习和离散化，用潜在扰动替代显式码本，提供了一种更稳定、更平衡的VQ-VAE训练方法，其理论框架也为现有量化方法提供了新的解释。
...
### [101] [TimeOmni-VL: Unified Models for Time Series Understanding and Generation](https://arxiv.org/abs/2602.17149)
*Tong Guan,Sheng Pan,Johan Barthelemy,Zhao Li,Yujun Cai,Cesare Alippi,Ming Jin,Shirui Pan*

Main category: cs.LG

TL;DR: TimeOmni-VL是首个基于视觉的统一时间序列理解与生成框架，通过双向映射和理解引导生成，显著提升了语义理解和数值精度。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列建模存在数值生成与语义理解之间的割裂：生成模型依赖表面模式匹配，理解模型难以实现高保真数值输出。尽管统一多模态模型在视觉领域已弥合此差距，但在时间序列领域尚未开发。

Method: 提出TimeOmni-VL框架，包含两个关键创新：1) 保真双向映射(Bi-TSI)：实现时间序列与图像之间的近无损转换；2) 理解引导生成：引入TSUMM-Suite数据集（包含6个理解任务和2个生成任务），通过校准思维链将时间序列理解作为生成控制信号。
...
### [102] [Powering Up Zeroth-Order Training via Subspace Gradient Orthogonalization](https://arxiv.org/abs/2602.17155)
...
<details>
  <summary>Details</summary>
Motivation: 零阶优化作为避免反向传播的内存高效范式，在微调大规模模型时面临准确性与查询效率之间的根本矛盾。现有方法需要在这两者之间权衡，限制了零阶优化的实际应用效果。

Method: 提出统一的子空间梯度正交化框架ZO-Muon，结合两个互补原则：(1) 基于投影的子空间视角，利用模型更新的内在低秩结构降低梯度估计方差；(2) Muon风格谱优化，通过梯度正交化从噪声零阶梯度中提取信息性谱结构。

Result: 在大语言模型和视觉Transformer上的实验表明，ZO-Muon显著加速收敛，在准确性和查询/运行时效率上实现双赢改进。相比MeZO基线，在LLM微调中仅需24.7%的查询达到相同SST-2性能，在ViT-B微调中CIFAR-100准确率提升25.1%。

Conclusion: 通过统一子空间视角和谱优化原则，ZO-Muon有效解决了零阶优化中准确性与查询效率的矛盾，为大规模模型的高效微调提供了有前景的解决方案。
...
### [103] [In-Context Learning in Linear vs. Quadratic Attention Models: An Empirical Study on Regression Tasks](https://arxiv.org/abs/2602.17171)
*Ayush Goel,Arjun Kohli,Sarvagya Somvanshi*

Main category: cs.LG

TL;DR: 对比研究transformer和线性注意力模型在线性回归任务上的上下文学习表现，分析两者在MSE、收敛性、泛化能力和深度影响方面的差异


<details>
  <summary>Details</summary>
Motivation: 已有研究表明transformer和线性注意力模型能在简单函数类（如线性回归）上实现上下文学习，但缺乏对这两种注意力机制在具体任务上表现差异的系统性实证研究

Method: 使用Garg等人提出的经典线性回归任务作为基准，实证评估两种架构的学习质量（MSE）、收敛性、泛化行为，并分析模型深度对上下文学习性能的影响

Result: 研究结果揭示了线性注意力相对于二次注意力在该设置下的相似性和局限性，具体表现在学习效果、收敛速度和泛化能力等方面的差异

Conclusion: 通过系统比较两种注意力机制在线性回归上下文学习中的表现，为理解不同注意力架构的能力边界提供了实证依据，有助于指导模型设计和应用选择
...
### [104] [Continual uncertainty learning](https://arxiv.org/abs/2602.17174)
...
<details>
  <summary>Details</summary>
Motivation: 机械系统多不确定性鲁棒控制是基本挑战，现有深度强化学习方法处理所有不确定性源时往往导致次优策略和学习效率低下

Method: 提出课程持续学习框架：1) 将多不确定性复杂控制问题分解为序列化持续学习任务；2) 逐步扩展和多样化动态不确定性；3) 结合模型基控制器保证基准性能；4) 残差学习方案实现任务特定优化

Result: 验证了所提方法在汽车动力总成主动振动控制中的应用，控制器对结构非线性和动态变化具有鲁棒性，成功实现了仿真到现实的迁移

Conclusion: 提出的课程持续学习框架能有效处理多不确定性鲁棒控制问题，通过分解复杂任务和结合模型基控制器，提高了学习效率和策略鲁棒性
...
### [105] [SoftDTW-CUDA-Torch: Memory-Efficient GPU-Accelerated Soft Dynamic Time Warping for PyTorch](https://arxiv.org/abs/2602.17206)
*Ron Shapira Weber,Oren Freifeld*

Main category: cs.LG

TL;DR: 软DTW-CUDA-Torch：一个开源的PyTorch库，用于在GPU上计算软动态时间规整（SoftDTW），解决了现有GPU实现的三个关键限制


<details>
  <summary>Details</summary>
Motivation: 现有GPU实现存在三个主要问题：序列长度限制在1024以内、小平滑参数下反向传播数值不稳定、以及存储成对距离张量导致GPU内存消耗过大

Method: 引入(1)分块反对角线核执行消除序列长度限制，(2)对数空间反向传播防止浮点溢出，(3)融合距离计算模式消除O(BNM)中间距离张量，减少高达98%内存使用

Result: 该库支持任意序列长度、完整的PyTorch自动微分集成以及软DTW重心计算，代码已在GitHub开源

Conclusion: 软DTW-CUDA-Torch提供了一个高效、稳定且内存优化的GPU实现，解决了现有实现的限制，为时间序列分析任务提供了更好的工具支持
...
### [106] [Structured Prototype-Guided Adaptation for EEG Foundation Models](https://arxiv.org/abs/2602.17251)
...
<details>
  <summary>Details</summary>
Motivation: EEG基础模型在全微调下表现良好，但在真实临床场景中，受试者级别的监督有限时泛化能力差。这种失败不仅源于监督有限，更源于噪声监督与模型高度可塑性参数空间之间的结构不匹配。

Method: 提出SCOPE框架，采用两阶段流程：第一阶段通过几何正则化的任务先验构建可靠的外部监督，在嵌入空间构建平衡的类级原型，并基于一致性生成置信度感知的伪标签以过滤不可靠信号；第二阶段引入ProAdapter，通过轻量级适配器在结构化原型条件下调整冻结的EEG基础模型。

Result: 在三个EEG任务和五个基础模型骨干上的实验表明，SCOPE在标签有限的跨受试者设置下始终实现强大的性能和效率。

Conclusion: SCOPE通过结构化原型引导的适配机制，有效解决了EEG基础模型在有限监督下的泛化挑战，为临床场景中的实际应用提供了可行方案。
...
### [107] [Learning a Latent Pulse Shape Interface for Photoinjector Laser Systems](https://arxiv.org/abs/2602.17263)
...
<details>
  <summary>Details</summary>
Motivation: 在自由电子激光器的光注入器中控制纵向激光脉冲形状是优化电子束质量的重要手段，但通过暴力脉冲传播模拟系统探索巨大设计空间成本过高

Method: 使用Wasserstein自编码器构建生成模型框架，学习脉冲整形与下游束流动力学之间的可微分潜在接口，通过主成分分析和高斯混合模型分析潜在空间几何结构

Result: 学习的潜在空间连续且可解释，保持高保真重建；脉冲族如高阶高斯沿相干轨迹分布；模型从模拟数据泛化到真实实验脉冲测量，准确重建脉冲并将其一致嵌入学习流形
...
### [108] [Unified Latents (UL): How to train your latents](https://arxiv.org/abs/2602.17270)
*Jonathan Heek,Emiel Hoogeboom,Thomas Mensink,Tim Salimans*

Main category: cs.LG

TL;DR: UL框架通过扩散先验和扩散模型联合正则化学习潜在表示，在ImageNet-512上达到1.4 FID，在Kinetics-600上创下1.3 FVD的新SOTA
...
Result: ImageNet-512上FID达1.4，重建质量高（PSNR），训练FLOPs少于基于Stable Diffusion潜在空间的模型；Kinetics-600上FVD达1.3，创SOTA

Conclusion: UL框架通过扩散先验和扩散模型的联合正则化，在图像和视频生成任务上都取得了优异性能，同时降低了训练成本
...
### [109] [RLGT: A reinforcement learning framework for extremal graph theory](https://arxiv.org/abs/2602.17276)
*Ivan Damnjanović,Uroš Milivojević,Irena Đorđević,Dragan Stevanović*

Main category: cs.LG

TL;DR: RLGT是一个新的强化学习框架，专门用于图论研究，支持有向/无向图、带/不带环、多边颜色，旨在系统化先前工作并促进极值图论的RL研究。


<details>
  <summary>Details</summary>
Motivation: Wagner等人开创性地将深度交叉熵强化学习方法应用于极值图论问题，将其重新表述为组合优化问题。随后许多研究者对此框架进行改进和扩展，创建了专门的图论RL环境，并解决了一些极值图论问题。然而，缺乏一个系统化的框架来整合这些工作并支持更广泛的图类型。

Method: 提出了RLGT（Reinforcement Learning for Graph Theory）框架，该系统支持有向图和无向图、带环和不带环、任意数量的边颜色。该框架采用高效的图表示方法，具有优化的计算性能和清晰模块化的设计。

Result: RLGT框架成功系统化了先前的工作，为极值图论的RL研究提供了统一平台。先前使用RL方法已取得的具体成果包括：反驳了关于图拉普拉斯谱半径的几个不等式、获得了某些拉姆齐数的新下界、对禁止三阶和四阶循环的Turán型极值问题做出了贡献。

Conclusion: RLGT是一个新颖的强化学习框架，旨在促进未来基于RL的极值图论研究。通过提供高效、模块化的系统设计，支持广泛的图类型，该框架有望推动图论与机器学习交叉领域的发展。
...
### [110] [Efficient privacy loss accounting for subsampling and random allocation](https://arxiv.org/abs/2602.17284)
*Vitaly Feldman,Moshe Shenfeld*

Main category: cs.LG

TL;DR: 该论文提出了一种高效计算随机分配采样方案隐私损失分布的方法，证明其在隐私-效用权衡上至少与泊松采样相当，特别适合DP-SGD训练。
...
Method: 开发了基于隐私损失分布实现概念的新工具，能够高效计算任何差分隐私算法在随机分配采样下的隐私损失分布，特别针对高斯机制进行了应用。

Result: 证明了随机分配采样在隐私-效用权衡上至少与泊松采样相当，特别适合DP-SGD训练，并实现了更准确的隐私损失计算。

Conclusion: 随机分配采样方案具有优于泊松采样的实用性优势，通过新的隐私损失分布计算方法，能够实现更精确的隐私损失核算，为差分隐私优化提供了更好的工具支持。
...
### [111] [SubQuad: Near-Quadratic-Free Structure Inference with Distribution-Balanced Objectives in Adaptive Receptor framework](https://arxiv.org/abs/2602.17330)
*Rong Fu,Zijian Zhang,Wenxin Zhang,Kun Liu,Jiekai Wu,Xianda Li,Simon Fong*

Main category: cs.LG

TL;DR: SubQuad是一个用于大规模适应性免疫库比较分析的高效、公平感知的端到端流程，通过近次二次检索、GPU加速亲和力内核和多模态融合解决计算成本和数据集不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 群体规模的适应性免疫库比较分析面临两个主要瓶颈：1) 成对亲和力评估的近二次成本；2) 数据集不平衡导致临床重要的少数克隆型被掩盖。这些限制阻碍了大规模免疫库分析和下游转化应用。

Method: SubQuad采用端到端流程，结合抗原感知的近次二次检索、GPU加速亲和力内核、学习型多模态融合和公平约束聚类。具体包括：紧凑MinHash预过滤大幅减少候选比较；可微分门控模块自适应加权互补对齐和嵌入通道；自动校准程序强制实施稀有抗原特异性亚群的比例表示。

Result: 在大型病毒和肿瘤免疫库上，SubQuad在吞吐量和峰值内存使用方面实现了可测量的增益，同时保持或提高了recall@k、聚类纯度和亚群公平性。通过协同设计索引、相似性融合和公平感知目标，系统提供了可扩展的解决方案。

Conclusion: SubQuad通过协同设计索引、相似性融合和公平感知目标，为免疫库挖掘和下游转化任务（如疫苗靶点优先排序和生物标志物发现）提供了一个可扩展、偏见感知的平台，解决了大规模免疫库分析中的计算和公平性挑战。
...
### [112] [From Subtle to Significant: Prompt-Driven Self-Improving Optimization in Test-Time Graph OOD Detection](https://arxiv.org/abs/2602.17342)
*Luzhi Wang,Xuanshuo Fu,He Zhang,Chuang Liu,Xiaobao Wang,Hongbo Liu*

Main category: cs.LG

TL;DR: SIGOOD是一种用于图异常检测的自改进框架，通过提示增强和能量偏好优化来放大异常信号，在21个真实数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的图异常检测方法大多采用单次推理范式，无法逐步纠正错误预测以放大异常信号，需要一种能够持续自我改进的检测框架。

Method: 提出SIGOOD框架：1）生成提示构建提示增强图以放大异常信号；2）引入能量偏好优化损失函数，利用原始测试图与提示增强图之间的能量变化；3）通过自改进循环迭代优化提示，最终使用最优提示增强图进行异常检测。

Result: 在21个真实世界数据集上的综合评估证实了SIGOOD方法的有效性和优越性能。

Conclusion: SIGOOD通过集成持续自学习与测试时训练，为图异常检测提供了一个有效的无监督框架，能够显著提升检测性能。
...
### [113] [2Mamba2Furious: Linear in Complexity, Competitive in Accuracy](https://arxiv.org/abs/2602.17363)
*Gabriel Mongaras,Eric C. Larson*

Main category: cs.LG

TL;DR: 该论文通过简化Mamba-2并改进其A-mask和隐藏状态阶数，开发出2Mamba方法，在保持线性注意力效率的同时，几乎达到softmax注意力的准确度


<details>
  <summary>Details</summary>
Motivation: 线性注意力变换器虽然效率高，但表达能力较弱，准确度低于softmax注意力。研究旨在缩小两者之间的准确度差距

Method: 首先简化Mamba-2到最基本组件（Mamba-2S），评估哪些选择最准确；然后改进A-mask并增加隐藏状态阶数，开发出2Mamba方法

Result: 2Mamba方法几乎达到softmax注意力的准确度，同时在长上下文长度下内存效率更高；还发现了能超越softmax注意力准确度的Mamba-2元素

Conclusion: 通过系统性地简化和改进Mamba-2，成功开发出2Mamba方法，在保持线性注意力效率优势的同时，显著缩小了与softmax注意力的准确度差距
...
### [114] [A feature-stable and explainable machine learning framework for trustworthy decision-making under incomplete clinical data](https://arxiv.org/abs/2602.17364)
...
Method: CACTUS框架集成了特征抽象、可解释分类和系统特征稳定性分析，量化信息特征在数据质量下降时的保持一致性。使用包含568名膀胱癌评估患者的真实血尿队列数据，在控制随机引入缺失数据水平下，与随机森林和梯度提升等常用方法进行基准比较。

Result: CACTUS在预测性能上达到竞争性或更优水平，同时在缺失数据增加时保持显著更高的顶级特征稳定性，包括在性别分层分析中。特征稳定性提供了与传统性能指标互补的信息。

Conclusion: 通过显式量化对缺失数据的鲁棒性并优先考虑可解释、稳定的特征，CACTUS为生物医学数据中可信的数据驱动决策支持提供了一个可推广的框架。
...
### [115] [Variational Grey-Box Dynamics Matching](https://arxiv.org/abs/2602.17477)
...
<details>
  <summary>Details</summary>
Motivation: 深度生成模型（如流匹配和扩散模型）能学习复杂分布但缺乏物理可解释性，而基于物理的仿真模型虽然可解释但可能包含未知项或不完整。需要一种方法既能利用观测数据学习动力学，又能保留物理模型的解释性。

Method: 在流匹配框架中构建结构化变分分布，使用两个潜在编码：一个建模缺失的随机性和多模态速度，另一个将物理参数编码为具有物理信息先验的潜在变量。还提出了处理二阶动力学的框架适配。

Result: 在代表性ODE/PDE问题上的实验表明，该方法性能与完全数据驱动方法相当或更优，同时保持了物理模型的可解释性，优于之前的灰盒基线方法。
...
### [116] [Retrospective In-Context Learning for Temporal Credit Assignment with Large Language Models](https://arxiv.org/abs/2602.17497)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于任务特定价值函数的信用分配方法存在样本效率低和泛化能力有限的问题，需要更有效的方法从稀疏环境反馈中学习。

Method: 提出RICL（回顾式上下文学习）利用LLMs将稀疏奖励转化为优势函数，并进一步提出RICOL在线学习框架，基于RICL的信用分配结果迭代优化策略。

Result: RICL能够用有限样本准确估计优势函数并识别环境中的关键状态；在四个BabyAI场景中，RICOL达到与传统在线RL算法相当的收敛性能，但样本效率显著更高。

Conclusion: 利用LLMs进行时间信用分配具有巨大潜力，为更样本高效和可泛化的RL范式开辟了新途径。
...
### [117] [LORA-CRAFT: Cross-layer Rank Adaptation via Frozen Tucker Decomposition of Pre-trained Attention Weights](https://arxiv.org/abs/2602.17510)
...
<details>
  <summary>Details</summary>
Motivation: 现有张量分解方法存在局限性：LoTR使用共享因子矩阵的Tucker分解，SuperLoRA对跨层梯度更新进行分解，PiSSA对每层独立进行SVD。需要一种能结合跨层张量分解优势的方法，直接对预训练权重进行全Tucker分解。

Method: CRAFT将预训练注意力权重矩阵跨层堆叠为3D张量，通过高阶奇异值分解(HOSVD)进行全Tucker分解，冻结所有分解因子，仅训练轻量级适配矩阵应用于每个因子矩阵。

Result: 在GLUE基准测试中，使用RoBERTa-base和RoBERTa-large模型，CRAFT仅需41K Tucker适配参数（与模型维度和深度无关），性能与现有方法相当。

Conclusion: CRAFT成功结合了跨层张量分解和预训练权重直接分解的优势，提供了一种参数高效且性能竞争的微调方法，参数数量固定且与模型规模无关。
...
TL;DR: 研究发现Transformer注意力头中存在专门的成员测试器，用于检测token是否在上下文中出现过，这些头形成了从高精度过滤器到经典Bloom过滤器的策略谱系


<details>
  <summary>Details</summary>
Motivation: 识别和分析Transformer模型中专门用于成员测试的注意力头，理解它们如何检测token重复出现，以及这些机制与经典数据结构的相似性

Method: 在四个语言模型（GPT-2小、中、大和Pythia-160M）中识别成员测试头，分析它们的假阳性率、容量曲线，使用混淆控制排除伪阳性，并进行消融实验

Result: 发现了三种真正的成员测试头：两个高精度过滤器（假阳性率0-4%），一个经典Bloom过滤器（R²=1.0，容量约5位），这些头集中在早期层（0-1层），形成多分辨率系统，对任何重复token类型都有响应

Conclusion: Transformer中存在专门的成员测试注意力头，它们形成从高精度过滤器到Bloom过滤器的策略谱系，这些机制具有距离敏感性，广泛泛化，并与更广泛的计算角色共存
...
TL;DR: 当前12导联心电图表示学习的基准测试实践存在缺陷，需要改进以确保进展可靠且与临床目标一致。研究发现随机初始化编码器在许多任务上与最先进的预训练方法表现相当，建议将其作为合理基线模型。


<details>
  <summary>Details</summary>
Motivation: 当前心电图表示学习领域主要依赖三个公开的多标签基准测试（PTB-XL、CPSC2018、CSN），这些基准主要关注心律失常和波形形态标签，但心电图实际上编码了更广泛的临床信息。需要扩展评估范围以包括结构性心脏病和患者层面预测等更相关的临床目标。

Method: 提出评估多标签、不平衡设置的最佳实践，并对三种代表性的心电图预训练方法在六个评估设置中进行实证评估：三个标准基准、一个结构性心脏病数据集、血流动力学推断和患者预测。

Result: 当应用最佳实践评估方法时，文献中关于哪种表示性能最佳的结论会发生改变。令人惊讶的是，随机初始化的编码器在许多任务上与最先进的预训练方法表现相当。随机编码器可以作为合理的基线模型。

Conclusion: 当前心电图表示学习的基准测试实践需要改革，应扩展评估范围以包括更广泛的临床目标，并采用更严格的评估方法。随机编码器应作为合理的基线模型，以确保进展的可靠性和临床相关性。
...
<details>
  <summary>Details</summary>
Motivation: 现有RLVR算法（如GRPO）依赖于刚性、均匀且对称的信任区域机制，这与LLMs复杂的优化动态不匹配。论文识别了三个关键挑战：硬截断导致的梯度利用效率低下、均匀比率约束忽略令牌分布的概率质量不敏感、以及正负样本信用分配模糊性差异导致的非对称信号可靠性问题。

Method: 提出Mass-Adaptive Soft Policy Optimization (MASPO)统一框架，包含三个核心组件：1）可微软高斯门控最大化梯度效用；2）质量自适应限制器平衡概率谱上的探索；3）非对称风险控制器使更新幅度与信号置信度对齐。

Result: 广泛评估表明MASPO作为稳健的一体化RLVR解决方案，显著优于强基线方法。

Conclusion: MASPO通过协调梯度利用、概率分布敏感性和信号可靠性三个维度，为RLVR提供了更有效的优化框架，解决了现有方法的局限性。
...
### [121] [Revisiting Weight Regularization for Low-Rank Continual Learning](https://arxiv.org/abs/2602.17559)
...
<details>
  <summary>Details</summary>
Motivation: 在参数高效持续学习（PECL）范式中，虽然任务特定模块（如低秩适配器）被广泛使用，但权重正则化技术（如EWC）在这一新范式下仍未得到充分探索。研究者希望重新审视低秩持续学习中的权重正则化，作为缓解PECL中任务干扰的新视角。

Method: 提出EWC-LoRA方法，通过EWC正则化共享的低秩更新来缓解任务干扰。该方法利用低秩表示在全维空间中估计参数重要性，保持存储需求和推理成本不随任务数量增加而变化。

Result: 在各种基准测试上的广泛实验表明，EWC-LoRA在稳定性和可塑性之间取得了优于现有低秩持续学习方法的平衡。结果表明，即使在低秩参数化下，权重正则化仍然是缓解任务干扰的有效机制。

Conclusion: EWC-LoRA为预训练模型的持续学习提供了一个实用、计算和内存高效的解决方案，并为权重正则化技术在PECL中的更广泛应用提供了见解。即使在低秩参数化条件下，权重正则化仍然是缓解任务干扰的有效方法。
...
### [122] [Be Wary of Your Time Series Preprocessing](https://arxiv.org/abs/2602.17568)
*Sofiane Ennadir,Tianze Wang,Oleg Smirnov,Sahar Asadi,Lele Cao*

Main category: cs.LG

TL;DR: 本文首次从理论角度分析了不同归一化策略（实例归一化和全局缩放）对时间序列Transformer模型表达能力的影响，提出了专门的时间序列表达能力框架，并通过理论推导和实验验证发现没有单一的归一化方法始终最优，有时甚至完全不用归一化效果更好。
...
Method: 提出了专门针对时间序列的新的表达能力框架，用于量化模型在表示空间中区分相似和不相似输入的能力。使用该框架推导了两种广泛使用的归一化方法（标准归一化和最小-最大缩放）的理论界限。通过多个基于Transformer的模型在分类和预测基准上进行实证验证。

Result: 分析表明归一化策略的选择会显著影响模型的表示能力，具体取决于任务和数据特征。实证结果显示没有单一的归一化方法始终优于其他方法，在某些情况下，完全省略归一化反而能获得更好的性能。

Conclusion: 预处理在时间序列学习中起着关键作用，需要针对特定任务和数据集设计更有原则的归一化策略。研究结果强调了归一化选择的重要性，并表明一刀切的归一化方法可能不是最佳选择。
...
### [123] [Guarding the Middle: Protecting Intermediate Representations in Federated Split Learning](https://arxiv.org/abs/2602.17614)
...
<details>
  <summary>Details</summary>
Motivation: 联邦分割学习(UFSL)虽然将部分计算卸载到服务器，但客户端与服务器共享的中间表示(破碎数据)容易暴露客户端的隐私数据，需要一种既能保护隐私又能保持模型效用的解决方案。

Method: 提出k-匿名差分隐私UFSL(KD-UFSL)方法，结合微聚合(k-匿名化)和差分隐私技术来最小化传输到服务器的破碎数据泄露风险。首先展示攻击者如何通过数据重建攻击获取隐私数据，然后提出KD-UFSL来缓解这种风险。

Result: 实验表明，KD-UFSL在某些情况下将实际图像与重建图像之间的均方误差增加高达50%，结构相似性降低高达40%(在四个基准数据集上)。更重要的是，KD-UFSL在保护隐私的同时保持了全局模型的效用。

Conclusion: KD-UFSL适合大规模大数据应用，能够在隐私保护和模型效用之间取得良好平衡，为联邦分割学习提供了有效的隐私增强解决方案。
...
### [124] [Stable Asynchrony: Variance-Controlled Off-Policy RL for LLMs](https://arxiv.org/abs/2602.17616)
...
<details>
  <summary>Details</summary>
Motivation: 异步强化学习训练能提高端到端吞吐量，但高异步性会导致策略梯度估计器方差显著增大，训练在陈旧轨迹上会产生重尾重要性比率，使少数样本主导更新，导致梯度噪声大、学习不稳定。

Method: 提出VCPO（方差控制策略优化），包含两个核心组件：1）基于有效样本量（ESS）调整学习率以抑制不可靠更新；2）为离策略设置应用闭式最小方差基线，避免辅助价值模型且开销最小。

Result: VCPO在数学推理、通用推理和工具使用任务中显著提升异步训练的鲁棒性，优于多种基线方法（包括掩码/裁剪稳定器和算法变体），将长上下文多轮训练时间减少2.5倍，同时匹配同步训练性能。
...
### [125] [Catastrophic Forgetting Resilient One-Shot Incremental Federated Learning](https://arxiv.org/abs/2602.17625)
*Obaidullah Zaland,Zulfiqar Ahmad Khan,Monowar Bhuyan*

Main category: cs.LG

TL;DR: OSI-FL是一个一次性增量联邦学习框架，通过使用视觉语言模型生成类别特定嵌入，在单轮通信中传输这些嵌入，服务器端用扩散模型合成数据样本，并结合选择性样本保留机制来应对通信开销和灾难性遗忘问题。
...
Method: 提出OSI-FL框架：1) 客户端使用冻结的视觉语言模型生成类别特定嵌入，单轮通信传输到服务器；2) 服务器使用预训练扩散模型合成类似客户端数据分布的新数据；3) 引入选择性样本保留机制，基于样本损失识别并保留每个类别和任务对中最具信息量的top-p样本，在后续迭代中融入训练以限制遗忘。

Result: 实验结果表明，OSI-FL在三个基准数据集上，在类别增量和领域增量场景中，均优于传统联邦学习和一次性联邦学习方法。
...
### [126] [Reverso: Efficient Time Series Foundation Models for Zero-shot Forecasting](https://arxiv.org/abs/2602.17634)
*Xinghong Fu,Yanhong Li,Georgios Papaioannou,Yoon Kim*

Main category: cs.LG

TL;DR: 该论文提出了一种构建高效时间序列基础模型的方法，通过小型混合模型（结合长卷积和线性RNN层）替代大型Transformer，在零样本预测任务中实现相似性能但模型尺寸缩小百倍以上。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列基础模型通常采用大型Transformer架构，虽然性能良好但参数量巨大（数亿参数），在实际应用中效率低下且成本高昂。需要寻找更高效的模型架构来平衡性能与效率。

Method: 提出使用小型混合模型架构，交替使用长卷积和线性RNN层（特别是DeltaNet层），替代传统的大型Transformer。同时采用多种数据增强和推理策略进一步提升性能。

Result: 该方法构建的Reverso模型家族在零样本时间序列预测任务中，能够匹配甚至超越大型Transformer模型的性能，同时模型尺寸缩小了100倍以上，显著提升了性能-效率的帕累托前沿。

Conclusion: 大型Transformer并非时间序列基础模型的必要条件，小型混合模型通过合理的架构设计和训练策略，能够在保持高性能的同时大幅提升效率，为实际应用提供了更可行的解决方案。
...
### [127] [A.R.I.S.: Automated Recycling Identification System for E-Waste Classification Using Deep Learning](https://arxiv.org/abs/2602.17642)
*Dhruv Talwar,Harsh Desai,Wendong Yin,Goutam Mohanty,Rafael Reveles*

Main category: cs.LG

TL;DR: 提出A.R.I.S.系统，使用YOLOx模型实时分类电子垃圾中的金属、塑料和电路板，提高回收效率
...
Method: 开发低成本便携式电子垃圾分拣系统A.R.I.S.，采用YOLOx模型对金属、塑料和电路板进行实时分类

Result: 实验评估显示整体精度90%，平均精度82.2%，分拣纯度84%，推理延迟低且检测精度高

Conclusion: A.R.I.S.通过深度学习与传统分拣方法结合，提高了材料回收效率，降低了先进回收技术的应用门槛，支持产品生命周期延长和供应链环境影响的减少
...
### [128] [Pushing the Frontier of Black-Box LVLM Attacks via Fine-Grained Detail Targeting](https://arxiv.org/abs/2602.17645)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于迁移的黑盒攻击方法（如M-Attack）在局部裁剪匹配时产生高方差、近似正交的梯度，破坏了局部对齐的连贯性并导致优化不稳定，限制了攻击效果。

Method: 提出M-Attack-V2框架：1）多裁剪对齐（MCA）：每次迭代平均多个独立采样的局部视图梯度以降低方差；2）辅助目标对齐（ATA）：用语义相关分布的小型辅助集替代激进的目标增强，产生更平滑的目标流形；3）补丁动量：重放历史裁剪梯度；4）改进的补丁大小集成（PE+）。

Result: 在多个前沿LVLM上显著提升攻击成功率：Claude-4.0从8%提升至30%，Gemini-2.5-Pro从83%提升至97%，GPT-5从98%提升至100%，优于现有黑盒攻击方法。

Conclusion: M-Attack-V2通过解决梯度方差和结构不对称问题，为大型视觉语言模型的黑盒对抗攻击提供了简单而有效的模块化增强方案。
...
### [129] [Multi-Round Human-AI Collaboration with User-Specified Requirements](https://arxiv.org/abs/2602.17646)
*Sima Noorani,Shayan Kiyani,Hamed Hassani,George Pappas*

Main category: cs.LG

TL;DR: 本文提出一个原则性框架，通过反事实伤害和互补性两个原则来确保多轮对话AI可靠提升人类决策质量，并开发了具有有限样本保证的在线算法来执行用户定义的约束。


<details>
  <summary>Details</summary>
Motivation: 随着人类在高风险决策中越来越依赖多轮对话AI，需要原则性框架来确保这种互动能可靠提升决策质量。本文采用以人为中心的视角，关注AI不应削弱人类优势（反事实伤害），同时应在人类易出错处增加价值（互补性）。

Method: 通过用户定义的规则形式化反事实伤害和互补性概念，允许用户为具体任务定义伤害和互补性的含义。引入一个在线、无分布的算法，具有有限样本保证，能在协作动态中强制执行用户指定的约束。

Result: 在两个交互设置中评估框架：医疗诊断任务的LLM模拟协作和图像推理任务的人类众包研究。结果显示在线程序即使在非平稳交互动态下也能维持规定的反事实伤害和互补性违规率。收紧或放松这些约束会产生可预测的下游人类准确性变化。

Conclusion: 反事实伤害和互补性两个原则可作为实用杠杆，引导多轮协作向更好的决策质量发展，无需建模或约束人类行为。该框架为高风险的AI-人类协作提供了可验证的保障。
...
### [130] [MARS: Margin-Aware Reward-Modeling with Self-Refinement](https://arxiv.org/abs/2602.17658)
*Payel Bhattacharjee,Osvaldo Simeone,Ravi Tandon*

Main category: cs.LG

TL;DR: MARS提出了一种基于边界的自适应数据增强和采样策略，专注于奖励模型最不确定的低边界偏好对，通过迭代增强困难样本来改进奖励建模。
...
Method: MARS框架采用自适应、边界感知的增强和采样策略，专注于奖励模型最不确定的低边界（模糊）偏好对，通过迭代增强困难样本来精炼训练分布。

Result: 理论保证显示该策略增加了损失函数的平均曲率，从而增强了信息并改善了条件化；实证结果表明相比均匀增强，MARS在鲁棒奖励建模方面取得了持续增益。

Conclusion: MARS通过专注于奖励模型最不确定的模糊偏好对进行自适应数据增强，为奖励建模提供了一种更有效的训练策略，能够提高模型鲁棒性。
